...
def remove_authorized_user(tg_id: int):
    """Remove user from authorized_users table"""
    with get_conn() as conn:
        cur = conn.cursor()
...
async def login_username(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['ig_login_username'] = update.message.text.strip()
...
async def login_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['ig_login_password'] = update.message.text.strip()
    await update.message.reply_text(
...
async def login_passphrase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            # 👇 Agar OTP/challenge aata hai
            if "challenge_required" in str(e).lower():
                await update.message.reply_text("🔐 Instagram is asking for OTP. Please enter the code:")
                context.user_data["awaiting_otp"] = {
                    "username": ig_user,
                    "password": ig_pass,
...
async def otp_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    otp_info = context.user_data.get("awaiting_otp")
    if not otp_info:
        await update.message.reply_text("⚠️ No OTP requested.")
        return
    ...
    try:
        cl.challenge_code(otp_code)
        cl.dump_settings(otp_info["session_file"])

        # ✅ Save encrypted account
        blob_data = {
            "username": otp_info["username"],
            "password": otp_info["password"],
            "session_file": otp_info["session_file"]
        }
        encrypted = encrypt_blob(tg_id, otp_info["passphrase"], blob_data)
        ...
        with get_conn() as conn:
            ...
            cur.execute("""
                INSERT INTO ig_accounts(tg_id, ig_username, encrypted_blob, session_file, default_account)
                VALUES(?,?,?,?,?)
            """, (tg_id, otp_info["username"], encrypted, otp_info["session_file"], 0))
            save_passphrase_for_account(tg_id, otp_info["username"], otp_info["passphrase"])
            conn.commit()

        await update.message.reply_text("✅ Login successful with OTP. Account saved.")
    except Exception as e:
...
async def viewmyac(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        return
    s = "📂 *Your saved Instagram accounts:*\n"
    for r in rows:
        s += f"{r[0]}. {r[1]} {'(default)' if r[2] else ''}\n"
    s += "\nUse /setig <id> to set default account."
...
async def start_sending_messages(update: Update, context: ContextTypes.DEFAULT_TYPE, passphrase: str = None):
    ...
    thread = threading.Thread(
        ...
        args=(tg_id, cl_inst, msgs, choice, context.user_data, stop_event)
    )
...
def send_messages_worker(tg_id: int, cl_inst: Client, msgs: List[str], choice: str, user_data: Dict, stop_event: threading.Event):
    speed = get_user_speed(tg_id)
    ...
    try:
        for msg in msgs:
            ...
            try:
                if choice == 'dm':
                    if dm_target_username:
                        cl_inst.direct_send(msg, [dm_target_username])
                    else:
                        cl_inst.direct_send(msg, [target])
                elif choice == 'gc' and selected_thread_id:
                    cl_inst.direct_send(msg, [], selected_thread_id)
                time.sleep(speed)
            except Exception as e:
        ...
        print(f"Worker error for user {tg_id}: {e}")
    finally:
        # Clean up
        if tg_id in stop_flags:
            del stop_flags[tg_id]
        if tg_id in sending_threads:
            del sending_threads[tg_id]
...
def main():
    ...
    # Handle OTP messages outside conversation
