...
HANDLERS.setclan = async function ({ sock, jid, sender, args }) {
  try {
    if (!args[0]) {
      return await sock.sendMessage(jid, { text: "❌ Please provide a clan tag.\nExample: setclan #CLANTAG" });
    }
    ...
    // ✅ Use the new setClan logic
    const msg = await setClan(jid, clanTag);
...
HANDLERS.removeclan = async function ({ sock, jid }) {
  try {
    // ✅ Use the new removeClan logic
    const msg = await removeClan(jid);
