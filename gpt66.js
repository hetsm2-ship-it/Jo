...
// ------------------------- ⚔️ War Notifications (full updated) -------------------------
// This implementation prefers DB.groupMemberClans mapping, and falls back to
// cached groupMetadata lookup if mapping is not available for a group.

// Simple cache for group participants to avoid repeated metadata calls (fallback only)
const _groupParticipantsCache = new Map(); // key: groupJid, value: {ts: epochMs, participants: [jid,...] }
...
async function getGroupParticipantsCached(sock, groupJid) {
  ...
  try {
    ...
    // Could not fetch metadata (bot not in group / permission) => return null to indicate failure
    return null;
  }
}
...
async function handleWarNotifications(sock) {
  try {
    ...
    // Collect all unique clan tags from every user/group's saved clan
    ...
    // iterate per clan so group gets each clan's notification separately
    for (const clanTag of uniqueClanTags) {
      try {
        ...
        // 1) Gather direct targets from DB.userClans (users and groups who explicitly set this clan)
        ...
        // 2) Start with deduped groupTargets/userTargets from explicitTargets
        ...
        // 3) AUTO-INCLUDE authorised groups when ANY of their participants have this clan set
        //    Prefer DB.groupMemberClans mapping (no metadata). If absent for a group, fallback to cached metadata lookup.
        const authorisedGroups = Object.keys(DB.authorisedGroups || {});
        for (const gJid of authorisedGroups) {
          try {
            // skip if already included explicitly
            if (groupTargets.includes(gJid)) continue;

            let anyMemberHasClan = false;

            // Preferred: use DB.groupMemberClans mapping if available
            if (DB.groupMemberClans && DB.groupMemberClans[gJid]) {
              ...
              // Fallback: try reading group metadata (cached)
              ...
              if (participants) {
                ...
                // Could not fetch participants; skip this group for safety.
                anyMemberHasClan = false;
              }
            }
            ...
            console.error("auto-include-group error for", gJid, err && err.message ? err.message : err);
        ...
        // If nothing to send to after all, continue
        if (groupTargets.length === 0 && userTargets.length === 0) continue;
        ...
        if (war.state === "inWar") {
          ...
          for (const ms of milestones) {
            ...
            if (!DB.lastWarNotificationSent[notifyKey] && diffMs <= targetMs) {
              ...
              // send to groups and authorised users according to rules
        ...
        if (war.state === "warEnded") {
          ...
          if (!DB.lastWarNotificationSent[endedKey]) {
            ...
            // For ended war we want same rules: groups always get group message; DMs only to authorised users (or owner)
        ...
        console.error(`notify error for ${clanTag}:`, err && err.message ? err.message : err);
      }
    }
  } catch (err) {
    console.error("handleWarNotifications failed:", err && err.message ? err.message : err);
  }
}

// ------------------------- Helper to send notifications with auth-aware DM filtering -------------------------
async function sendWarNotificationToTargets(sock, war, userTargets = [], groupTargets = [], timeLeft = null, overrideMsg = null) {
  // Prepare message (if overrideMsg given use it)
  ...
  // Send to groups (deduped)
  for (const g of [...new Set(groupTargets || [])]) {
    try {
      await sock.sendMessage(g, { text: msgToSend }).catch(() => {});
    } catch (e) {
      // ignore per-group send errors
    }
  }

  // Send to users — but only those who are authorised for DM notifications OR the bot owner
  ...
  for (const u of [...new Set(userTargets || [])]) {
    try {
      ...
      if (isOwner || isUserAuthorised) {
        ...
        // user not in authorisedUsers -> skip DM (group may get it via participants / mapping logic)
      }
    } catch (e) {
