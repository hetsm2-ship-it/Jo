...
// ------------------------- ⚔️ War Notifications (Never Miss + Full Debug + Margin + Result Messages) -------------------------
...
async function getGroupParticipantsCached(sock, groupJid) {
  ...
  if (cached && (nowTs - cached.ts) < _GROUP_CACHE_TTL) {
...
async function handleWarNotifications(sock) {
  try {
    if (!DB.userClans || Object.keys(DB.userClans).length === 0) {
    ...
    for (const clanTag of uniqueClanTags) {
      try {
        ...
        let groupTargets = [...new Set(explicitTargets.filter(t => t.endsWith("@g.us")))];
        let userTargets = [...new Set(explicitTargets.filter(t => !t.endsWith("@g.us")))];

        // 2) auto-include groups
        ...
        for (const gJid of authorisedGroups) {
          ...
          if (DB.groupMemberClans && DB.groupMemberClans[gJid]) {
            const membersMap = DB.groupMemberClans[gJid] || {};
            anyMemberHasClan = Object.values(membersMap).some(tag => tag === clanTag);
          } else {
          ...
          if (anyMemberHasClan) {
            groupTargets.push(gJid);
          }
        }

        groupTargets = [...new Set(groupTargets)];
        userTargets = [...new Set(userTargets)];
        ...
        if (war.state === "inWar") {
          ...
          const timeLeftFormatted = formatTimeLeft(targetMs);

          for (const ms of milestones) {
            const targetMs = (ms.h ? ms.h * 3600 * 1000 : ms.m * 60 * 1000) + 16000; // +16 sec margin
            const notifyKey = `${baseKey}:${ms.key}`;

            if (!DB.lastWarNotificationSent[notifyKey] && diffMs <= targetMs) {
              console.log(`🚀 [FIRE] Sending ${ms.key} notification for ${clanTag}`);
              await sendWarNotificationToTargets(sock, war, userTargets, groupTargets, timeLeftFormatted);
              DB.lastWarNotificationSent[notifyKey] = now;
              if (typeof saveDB === "function") saveDB();
            } else if (DB.lastWarNotificationSent[notifyKey]) {
              console.log(`⏩ Already sent ${ms.key} for ${clanTag}`);
            }
          }
        }
        ...
        if (war.state === "warEnded") {
          ...
          if (!DB.lastWarNotificationSent[endedKey]) {
            ...
            if (myStars > oppStars) {
              resultMsg += `🎉 Congratulations! We won!\n`;
            } else if (myStars < oppStars) {
              resultMsg += `🥺 We lost the match!\n`;
            } else {
              resultMsg += `🙅🏻 Tie Match!\n`;
            }
...
async function sendWarNotificationToTargets(sock, war, userTargets = [], groupTargets = [], timeLeft = null, overrideMsg = null) {
  ...
  console.log("📤 Sending notification:", msgToSend);
...
setInterval(async () => {
  ...
  try {
    for (const jid in DB.userClans) {
      ...
      const maxAttacks = Number(warData.teamSize) || 2;

      for (const m of Array.isArray(warData.clan?.members) ? warData.clan.members : []) {
        ...
        const usedOrders = new Set(attacks.map(a => Number(a.order)));

        // --- Save actual attacks ---
        for (const atk of attacks) {
          const order = Number(atk.order) || 1;
          const warKey = `${warData.clan.tag}_${warData.opponent?.tag || "unknown"}_${playerTag}_${order}`;
          const exists = DB.playerWarLogs[playerTag].some(l => l && l.warKey === warKey);
          if (exists) continue;
          ...
          DB.playerWarLogs[playerTag].push({
            ...
            oppClanTag: warData.opponent?.tag || null,
            myClan: warData.clan.name || null,
            oppClan: warData.opponent?.name || null,
            myName: m.name || null,
            oppName: opponentName,
            stars: Number(atk.stars) || 0,
            destructionPercentage: Number(atk.destructionPercentage) || 0,
            order,
          ...
          console.log(`✅ Saved LIVE warlog for ${m.name || playerTag}: ${atk.stars}⭐ vs ${opponentName}`);
        }

        // --- Save unused slots up to maxAttacks ---
        for (let order = 1; order <= maxAttacks; order++) {
          if (!usedOrders.has(order)) {
            ...
            console.log(`⚪ Saved UNUSED slot for ${m.name || playerTag} (Attack ${order})`);
          }
        }
...
async function checkAndSaveWar(clanTag) {
  try {
    ...
    for (const m of Array.isArray(war.clan?.members) ? war.clan.members : []) {
      const playerTag = m.tag;
      if (!playerTag) continue;
      ...
      DB.playerWarLogs[playerTag] = DB.playerWarLogs[playerTag].map(l => {
        if (l.isFromLive && l.clanTag === clanTag) {
          const realAtk = attacks.find(a => Number(a.order) === Number(l.order));
          if (realAtk) {
            const opponent = (war.opponent?.members || []).find(o => o.tag === realAtk.defenderTag);
            return {
              ...l,
              oppName: opponent?.name || "Unknown",
              stars: Number(realAtk.stars) || 0,
              destructionPercentage: Number(realAtk.destructionPercentage) || 0,
              defenderTag: realAtk.defenderTag || null,
              unused: false,
              isFromLive: false
            };
          }
    ...
    console.error("checkAndSaveWar error:", e && e.message ? e.message : e);
  }
}


// ------------------------- helpers for 2-min finalizer -------------------------
function isWarAlreadyFinalized(clanTag, war) {
  try {
    if (!war || !Array.isArray(war.clan?.members)) return false;
    const maxAttacks = Number(war.teamSize) || 2;
    const oppTag = war.opponent?.tag || "unknown";
    const prefix = `${war.clan?.tag || clanTag}_${oppTag}_`;

    for (const m of war.clan.members) {
      const playerTag = m?.tag;
      if (!playerTag) return false; // cannot verify -> assume not finalized

      const logs = Array.isArray(DB.playerWarLogs[playerTag]) ? DB.playerWarLogs[playerTag] : [];

      for (let order = 1; order <= maxAttacks; order++) {
        const warKey = `${prefix}${playerTag}_${order}`;
        const found = logs.some(l => l && l.warKey === warKey && l.isFromLive === false);
    ...
    console.error("[isWarAlreadyFinalized] error:", e && e.message ? e.message : e);
    return false;
  }
}

async function forceFinalizeWar(clanTag, war) {
  try {
    if (!war) {
      console.log(`[forceFinalize] no war data for ${clanTag}`);
      return;
    }

    if (war.state === "inWar") {
      console.log(`[forceFinalize] ${clanTag} is inWar — skipping finalize.`);
      return;
    }

    console.log(`[forceFinalize] finalizing ${clanTag} (state=${war.state})`);

    const maxAttacks = Number(war.teamSize) || 2;
    const oppMembers = Array.isArray(war.opponent?.members) ? war.opponent.members : [];
    const oppMap = new Map(oppMembers.map(o => [o.tag, o.name || null]));
    const clanTagResolved = war.clan?.tag || clanTag;
    ...
    for (const m of Array.isArray(war.clan?.members) ? war.clan.members : []) {
      ...
      for (let order = 1; order <= maxAttacks; order++) {
        const realAtk = attacks.find(a => Number(a.order) === order);
        const warKey = `${clanTagResolved}_${war.opponent?.tag || "unknown"}_${playerTag}_${order}`;

        if (realAtk) {
          const oppName = oppMap.get(realAtk.defenderTag) || "Unknown";
          finalEntries.push({
            warKey,
            clanTag: clanTagResolved,
            oppClanTag: war.opponent?.tag || null,
            myClan: war.clan?.name || null,
            oppClan: war.opponent?.name || null,
            myName: m.name || null,
            oppName,
            stars: Number(realAtk.stars) || 0,
            destructionPercentage: Number(realAtk.destructionPercentage) || 0,
      ...
      // Replace any existing entries for this specific war (by prefix) with finalEntries
      DB.playerWarLogs[playerTag] = (DB.playerWarLogs[playerTag] || [])
        .filter(l => {
          const prefix = `${clanTagResolved}_${war.opponent?.tag || "unknown"}_`;
          return !l.warKey || !String(l.warKey).startsWith(prefix);
        })
        .concat(finalEntries);

      // limit per player to avoid unbounded growth
      if (DB.playerWarLogs[playerTag].length > 100) {
        DB.playerWarLogs[playerTag] = DB.playerWarLogs[playerTag].slice(-100);
      }
    } // members

    // Clear pendingFinalization flag if set
    if (DB.pendingFinalization && DB.pendingFinalization[clanTag]) {
      try { delete DB.pendingFinalization[clanTag]; } catch (_) {}
    }

    if (typeof saveDB === "function") {
      saveDB();
      console.log(`[forceFinalize] saved finalized logs for ${clanTag}`);
    }
  } catch (err) {
    console.error(`[forceFinalize] error for ${clanTag}:`, (err && err.message) ? err.message : err);
  }
}


// ------------------------- 2-MIN FINALIZER (skip inWar, finalize if needed) -------------------------
const __forceFinalizerInProgress = new Set();

async function finalizeWarsEvery2Min() {
  try {
    const clans = new Set();

    // collect clan tags from DB.userClans values
    for (const v of Object.values(DB.userClans || {})) {
      if (!v) continue;
      if (typeof v === "string") clans.add(v);
      else if (v && typeof v.clanTag === "string") clans.add(v.clanTag);
    }

    // include pendingFinalization keys too
    for (const k of Object.keys(DB.pendingFinalization || {})) {
      if (k) clans.add(k);
    }

    if (clans.size === 0) return;

    for (const clanTag of clans) {
      if (!clanTag) continue;
      if (__forceFinalizerInProgress.has(clanTag)) continue;
      __forceFinalizerInProgress.add(clanTag);

      (async (ct) => {
        try {
          const war = await cocFetch(`/clans/${encodeURIComponent(ct)}/currentwar`);
          if (!war) {
            console.log(`[2min-finalizer] No war data for ${ct}, skipping.`);
            return;
          }

          if (war.state === "inWar") {
            console.log(`[2min-finalizer] ${ct} is still inWar — skipping finalize.`);
          } else {
            // check if already finalized
            if (isWarAlreadyFinalized(ct, war)) {
              console.log(`[2min-finalizer] ${ct} already finalized previously — skipping.`);
            } else {
              console.log(`[2min-finalizer] ${ct} state=${war.state} — force finalizing.`);
              await forceFinalizeWar(ct, war);
            }
          }
        } catch (e) {
          console.error(`[2min-finalizer] error for ${ct}:`, (e && e.message) ? e.message : e);
        } finally {
          __forceFinalizerInProgress.delete(ct);
        }
      })(clanTag);
    }
  } catch (e) {
    console.error("[2min-finalizer] unexpected error:", (e && e.message) ? e.message : e);
  }
}

// run once immediately and then every 2 minutes
finalizeWarsEvery2Min();
setInterval(finalizeWarsEvery2Min, 2 * 60 * 1000);
...
// ensure global alias for removeClan
...
async function setClan(jid, clanTag) {
  ...
  // Backup old clan logs if switching
  ...
  // Set the new clan mapping
  DB.userClans[jid] = tag;

  // Restore logs if backup exists
...
// ensure global alias for setClan
...
setInterval(async () => {
  ...
  try {
    ...
    console.error("Ended war save loop error:", e && e.message ? e.message : e);
  }
}, 10 * 60 * 1000);
