...
// ------------------------- ⚔️ War Notifications (Never Miss + Full Debug) -------------------------
    ...
    for (const clanTag of uniqueClanTags) {
      try {
        ...
        if (war.state === "inWar") {
          ...
          // 🔥 Force fire: if not sent before & war still active -> send NOW
          for (const ms of milestones) {
            const notifyKey = `${baseKey}:${ms.key}`;
            if (!DB.lastWarNotificationSent[notifyKey]) {
              console.log(`🚀 [FORCE] Sending ${ms.key} notification for ${clanTag}`);
...
setInterval(async () => {
  ...
  try {
    for (const jid in DB.userClans) {
      ...
      // 🔑 Unique warId banayenge for this war
      ...
      for (const m of warData.clan.members) {
        ...
        for (const atk of attacks) {
          const warKey = `${warId}_${playerTag}_${atk.order}`;   // ab warId include ho gaya
        ...
        for (let order = 1; order <= 2; order++) {
          if (!usedOrders.has(order)) {
            const warKey = `${warId}_${playerTag}_${order}`;   // ab warId include ho gaya
            const exists = DB.playerWarLogs[playerTag].some(l => l.warKey === warKey);
            if (exists) continue;
  ...
  try {
    ...
    for (const m of war.clan.members) {
      ...
      DB.playerWarLogs[playerTag] = DB.playerWarLogs[playerTag].map(l => {
        if (l.isFromLive && l.clanTag === clanTag && l.warId === currentWarId) {
...
async function finalizeClanWar(clanTag) {
  try {
    ...
    // If opponent/clan data is missing (can happen in notInWar), avoid wrong finalization
    if (!war.clan || !war.opponent || !Array.isArray(war.clan.members)) {
      console.log(`ℹ️ No roster available for ${clanTag} [state=${war.state}] — skipping finalize this cycle.`);
      return;
    }

    const currentWarId = `${war.clan.tag}_${war.opponent.tag}_${war.startTime || war.preparationStartTime || war.endTime || "t"}`;

    console.log(`🏁 Finalizing war logs for ${clanTag} [state=${war.state}] (warId=${currentWarId})`);
    ...
    for (const m of war.clan.members || []) {
      ...
      DB.playerWarLogs[playerTag] = DB.playerWarLogs[playerTag].map(l => {
        // 🔐 Only touch logs from THIS warId (prevents prep-state mislabeling and cross-war edits)
        if (l.isFromLive && l.clanTag === clanTag && l.warId === currentWarId) {
          const realAtk = attacks.find(a => a.order === l.order);
          if (realAtk) {
            const opponent = war.opponent?.members?.find(o => o.tag === realAtk.defenderTag);
          ...
          // unused slot finalization (clean text)
          return {
            ...l,
            oppName: "Not used",
            stars: 0,
    ...
    if (typeof saveDB === "function") {
      saveDB();
      console.log(`💾 Finalized war logs for ${clanTag}`);
    }
  } catch (e) {
