...
`;
  ...
  try {
    // ✅ Public PushName fetch karo (fallback msg.pushName)
    let name = "Unknown";
    try {
      name = await sock.getName(sender) || msg?.pushName || "Unknown";
    } catch {
...
if (text.includes("instagram.com/")) {
  try {
    // ✅ Duplicate prevention (ignore if already processing same msg id)
    ...
    // ✅ First get caption (title) using yt-dlp
    let caption = text;
    try {
      caption = await new Promise((resolve, reject) => {
        execFile(ytDlpCommand, ["--get-title", text], { cwd: __dirname, timeout: 20000 }, (err, stdout) => {
          if (err) return resolve("Instagram Reel");
          resolve(stdout.trim() || "Instagram Reel");
        });
      });
    } catch {
