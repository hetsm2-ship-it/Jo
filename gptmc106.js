...
`;
  ...
  try {
    // ✅ Name logic (always try msg.pushName first)
    let name = msg?.pushName || "Not Available";
    try {
      const waName = await sock.getName(sender);
      if (waName && waName.trim().length > 0) {
        name = waName;
      }
    } catch {
      // ignore if getName fails
    }
