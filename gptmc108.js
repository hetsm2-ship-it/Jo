...
`;
  ...
  try {
    // ✅ Name fetching with full fallback chain
    let name = "Unknown";
    try {
      name =
        msg?.pushName ||                 // 1. msg se
        (await sock.getName(sender)) ||  // 2. WhatsApp roster se
        sender.split("@")[0] ||          // 3. Number se
        "Unknown";                       // 4. Final fallback
    } catch {
