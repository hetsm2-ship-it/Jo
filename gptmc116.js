...
// ✅ Minimal fix: CWL support for warData
...
if (warData?.state === "notInWar") {
  try {
    ...
    if (leagueGroup?.state === "inWar" || leagueGroup?.rounds?.length) {
      ...
      if (currentRound) {
        ...
        if (warTag) {
          ...
          if (leagueWar?.state === "inWar" || leagueWar?.state === "warEnded") {
            warData = leagueWar;
            warData.isCwl = true;
            console.log("[DEBUG] Using CWL war data instead of currentwar");
          }
        }
      }
    }
    console.log("=== CWL DEBUG START ===");
    console.log(`[CWL] Clan: ${clanTag}`);
    console.log(`[CWL] War State (after fetch): ${warData?.state}`);
    console.log(`[CWL] Is CWL War: ${warData?.isCwl || false}`);
    console.log(`[CWL] Members Found: ${warData?.clan?.members?.length || 0}`);
  } catch (e) {
    console.log("[DEBUG] CWL fetch failed:", e.message);
  }
}
 ...
 // console.log("[DEBUG] warState:", warState);
  ...
  for (const [i, m] of clanData.memberList.entries()) {
    ...
    if (!wm && warData?.isCwl) {
  console.log(`[CWL] ${m.name} (${m.tag}) => Not found in CWL lineup`);
}

    if (warState === "inWar") {
  if (!wm) {
    if (warData?.isCwl) {
      // ✅ Special case: CWL me member not in lineup
      status = "(not selected for CWL round)";
    } else {
    ...
    if (warData?.isCwl) {
     ...
     // console.log(`[DEBUG] Logs found in DB: ${logs.length}`);
      ...
      if (!logs.length) {
        ...
        // Fetch last 5 wars to include CWL also
const warLogsApi = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/warlog?limit=5`);

if (warLogsApi?.items?.length) {
  // Prefer last normal war, but if CWL war ended recently take that
  const lastRelevant = warLogsApi.items.find(w => w.isCwl || w.clan?.tag === clanTag);
  if (lastRelevant) {
    const playerInLastWar = lastRelevant.clan.members.find(x => x.tag === m.tag);
    // baaki aapka existing logic same
  }
}
  ...
  // ✅ Minimal fix: try to fetch last war (for CWL also)
  try {
    ...
    console.log("[DEBUG] Fallback warlog fetch failed:", e.message);
    status = "(no active war right now)";
  }
}
