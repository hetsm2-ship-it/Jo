...
// Agar normal war notInWar hai → CWL check karo
if (warData?.state === "notInWar") {
      ...
      if (currentRound) {
        ...
        if (warTag) {
          ...
          if (leagueWar?.state && leagueWar?.state !== "notInWar") {
            ...
            warData.isCwl = true;
            isCwl = true;
            console.log(`[DEBUG] CWL war data loaded for ${clanTag}`);
          }
        }
      }
    }
  } catch (err) {
    console.log("[DEBUG] CWL check failed:", err.message);
  }
}
  ...
  for (const [i, m] of clanData.memberList.entries()) {
    ...
    let status = "";
    // CWL ke liye sahi team identify karo
const isOurClan = warData?.clan?.tag === clanTag;
const teamData = isOurClan ? warData?.clan : warData?.opponent;
const wm = teamData?.members?.find(x => x.tag === m.tag);

    if (warState === "inWar") {
  if (!wm) {
    console.log(`[DEBUG] Player not found in current ${warData.isCwl ? "CWL" : "normal"} war members.`);
    status = warData.isCwl
      ? "(not selected for CWL round)"
      : "(not in current war)";
  } else {
    const used = wm?.attacks?.length || 0;
    console.log(`[DEBUG] Attacks used (live ${warData.isCwl ? "CWL" : "normal"} war): ${used}`);
    status = warData.isCwl
