...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    if (warData?.state === "notInWar") {
      try {
        ...
        if (leagueGroup?.rounds?.length) {
          const currentRound = leagueGroup.rounds.find(r => r.warTags.some(t => t !== "#0"));
          if (currentRound) {
            const warTag = currentRound.warTags.find(t => t !== "#0");
            const leagueWar = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
            if (leagueWar?.state && leagueWar?.state !== "notInWar") {
              warData = leagueWar;
              warData.isCwl = true;
              console.log("[DEBUG] Using CWL war data");
            }
          }
        }
      } catch (e) {
        console.log("[DEBUG] CWL fallback failed:", e.message);
      }
    }
    ...
    // Identify our clan in war data
    const ourClanData = (warData?.clan?.tag === clanTag) ? warData.clan : warData.opponent;

    // Member list based on war type
    const membersToCheck = warData?.isCwl
      ? (ourClanData?.members || [])
      : clanData.memberList;

    console.log("[DEBUG] Clan Tag:", clanTag);
    console.log("[DEBUG] War State:", warState);
    console.log("[DEBUG] Is CWL War:", warData?.isCwl || false);
    console.log("[DEBUG] Members Found:", membersToCheck.length);

    for (const [i, m] of membersToCheck.entries()) {
      console.log(`[DEBUG] Checking: ${m.name} (${m.tag})`);
      msg += `${i + 1}. ${m.name} ${m.tag}\n`;

      let status = "";
      let wm = warData?.isCwl
        ? ourClanData?.members?.find(x => x.tag === m.tag)
        : warData?.clan?.members?.find(x => x.tag === m.tag);

      // 🔍 Debug for member lineup
      if (!wm) {
        console.log(`[DEBUG] ${m.name} not in ${warData?.isCwl ? "CWL" : "Normal"} lineup`);
      }

      // 🟢 War is active
      if (warState === "inWar") {
        if (!wm) {
          status = warData?.isCwl
            ? "(not selected for CWL round)"
            : "(not in current war)";
        } else {
          const used = wm?.attacks?.length || 0;
          console.log(`[DEBUG] ${m.name} => Attacks Used: ${used}`);
          status = warData?.isCwl
            ? (used === 0
                ? "(unused CWL attack)"
                : "(used CWL attack)")
      ...
      // 🔴 War ended
        ...
        if (!logs.length) {
          try {
            const warLogsApi = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/warlog?limit=5`);
            if (warLogsApi?.items?.length) {
              const lastWar = warLogsApi.items.find(w => w.isCwl === !!warData?.isCwl);
              const player = lastWar?.clan?.members?.find(x => x.tag === m.tag);
              if (player) {
                const used = (player.attacks || []).filter(a => a.destructionPercentage > 0).length;
                status = warData?.isCwl
                  ? (used === 0
                      ? "(CWL unused attacks in last war)"
                      : "(CWL used attacks in last war)")
                  : (used === 0
              ...
              status = "(no warlog available)";
            }
          } catch (e) {
            console.log("[DEBUG] War log fetch failed:", e.message);
            status = "(error fetching last war data)";
          }
        } else {
          const recent = logs.filter(l => !l.isFromLive).slice(-2);
          const used = recent.filter(l => l.destructionPercentage > 0).length;
          status = warData?.isCwl
            ? (used === 0
                ? "(CWL unused attacks in last war)"
                : "(CWL used attacks in last war)")
    ...
    console.error("[ERROR] cminfo:", err);
    return await sock.sendMessage(jid, {
      text: `❌ Error fetching clan info: ${err.message}`
    });
  }
};
...
setInterval(async () => {
  ...
  try {
    for (const jid in DB.userClans) {
      ...
      const warData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
      if (!warData || (warData.state !== "inWar" && warData.state !== "warEnded")) {
  console.log(`⚠️ Skipped clan ${clanTag} (no active/ended war to save)`);
  continue;
}
