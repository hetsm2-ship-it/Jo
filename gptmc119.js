...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    if (warData?.state === "notInWar") {
      try {
        const lg = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
        if (lg?.rounds?.length) {
          const round = lg.rounds.find(r => r.warTags.some(t => t !== "#0"));
          if (round) {
            const warTag = round.warTags.find(t => t !== "#0");
    ...
    // ✅ 1. If args[0] is player number → show player details
    if (!isNaN(args[0])) {
      ...
      // 🔥 Current war
      if (warState === "inWar") {
        msg += `🏰 ${warData?.clan?.name} vs ${warData?.opponent?.name}\n\n`;
        const wm = (ourClan?.members || []).find(x => x.tag === player.tag);

        if (wm) {
          msg += `🔥 *Current Live War Attacks:*\n`;
          if (warData?.isCwl) {
            msg += wm.attacks?.[0]
              ? `CWL Attack → vs ${warData.opponent.members.find(o => o.tag === wm.attacks[0].defenderTag)?.name || "?"} → ${wm.attacks[0].destructionPercentage}% (${wm.attacks[0].stars}⭐)\n`
              : `CWL Attack → Unused\n`;
          } else {
            msg += wm.attacks?.[0]
              ? `Attack 1 → vs ${warData.opponent.members.find(o => o.tag === wm.attacks[0].defenderTag)?.name || "?"} → ${wm.attacks[0].destructionPercentage}% (${wm.attacks[0].stars}⭐)\n`
              : `Attack 1 → Unused\n`;
            msg += wm.attacks?.[1]
              ? `Attack 2 → vs ${warData.opponent.members.find(o => o.tag === wm.attacks[1].defenderTag)?.name || "?"} → ${wm.attacks[1].destructionPercentage}% (${wm.attacks[1].stars}⭐)\n`
              : `Attack 2 → Unused\n`;
          }
          msg += "\n";
        } else {
          msg += "🔥 This member is not selected in current war.\n\n";
        }
      }

      // 🔴 War ended or preparation
      else if (warState === "warEnded" || warState === "preparation") {
        const logs = DB.playerWarLogs?.[player.tag] || [];
        const maxAttacks = warData?.isCwl ? 1 : 2;
        const lastLogs = logs.filter(l => !l.isFromLive).slice(-maxAttacks);

        if (!logs.length) {
          msg += "🥲 No record found for last war\n\n";
        } else if (lastLogs.length === 0) {
          msg += "No attacks recorded in last war\n\n";
        } else {
          msg += "📜 Last War Attacks:\n";
          lastLogs.forEach((atk, i) => {
      ...
      if (warLogs.length) {
        ...
        for (const logs of Object.values(grouped)) {
          const sample = logs[0];
          msg += `No.${idx++} 🏰 ${sample.myClan} vs ${sample.oppClan}\n`;
          logs.slice(-2).forEach((atk, i) => {
    ...
    // ✅ 2. Otherwise → show clan member list
    const warState = warData?.state || "notInWar";
    const ourClan = (warData?.clan?.tag === clanTag) ? warData.clan : warData.opponent;
    const members = warData?.isCwl ? (ourClan?.members || []) : clanData.memberList;
    ...
    for (const [i, m] of members.entries()) {
      ...
      let status = "";
      const wm = (ourClan?.members || []).find(x => x.tag === m.tag);

      if (warState === "inWar") {
        if (!wm) {
          status = warData?.isCwl ? "(not selected for CWL round)" : "(not in current war)";
        } else {
          const used = wm?.attacks?.length || 0;
          status = warData?.isCwl
            ? (used === 0 ? "(unused CWL attack)" : "(used CWL attack)")
            : (used === 0
        ...
        status = warData?.isCwl ? "(CWL war preparation)" : "(normal war preparation)";
      } else if (warState === "warEnded") {
        status = warData?.isCwl
          ? "(CWL war ended - check history)"
          : "(normal war ended - check history)";
      } else {
