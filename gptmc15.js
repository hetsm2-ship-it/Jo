...
function makeWarId(war) {
  return `${war.clan.tag}_${war.opponent?.tag || "unknown"}_${war.startTime || war.preparationStartTime || war.endTime || "t"}`;
}
...
async function main() {
  ...
  const { exec } = require("child_process");
const cocFetch = require("./cocFetch"); // your cocFetch function
const DB = require("./db.json"); // your DB object
const fs = require("fs");
  ...
  try {
    ...
    for (const clanTag of clans) {
      ...
      if (warData.state === "inWar") {
        console.log(`⚠️ Skipping run getwarlogs.js for ${clanTag} (war LIVE)`);
        continue;
      }

      exec("node getlogwar.js", (error, stdout, stderr) => {
        if (error) return console.error(`❌ getlogwar.js error: ${error.message}`);
        if (stderr) return console.error(`❌ getlogwar.js stderr: ${stderr}`);
        console.log(`✅ getlogwar.js executed for ${clanTag}`);
        console.log(stdout);
      });
    }
  } catch (e) {
    console.error("Error in runGetWarLogs:", e.message);
  }
}
runGetWarLogs();
