...
if (isGroup) {
  // Group Authorisation
  if (!DB.authorisedGroups?.[jid]) {
    return await sock.sendMessage(jid, { text: "❌ This group is not authorised to use this command." });
  }

  const clanTag = args[0] || DB.userClans[sender] || DB.groupSettings?.[jid]?.defaultClan;
  if (!clanTag) {
    return await sock.sendMessage(jid, { text: "❌ Clan tag not set. Use setclan first." });
  }
} else {
  // DM Authorisation
  if (!(DB.userSettings?.[sender]?.authorized === true)) {
