...
HANDLERS.addadmin = async ({ sock, jid, args, sender }) => {
  if (!isOwner(sender)) return sock.sendMessage(jid, { text: "❌ Sirf owner hi `addadmin` chala sakta hai." });
...
HANDLERS.removeadmin = async ({ sock, jid, args, sender }) => {
  if (!isOwner(sender)) return sock.sendMessage(jid, { text: "❌ Sirf owner hi `removeadmin` chala sakta hai." });
  const adminJid = args[0];
