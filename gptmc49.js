...
async function handleWarNotifications(sock) {
  try {
    ...
    // Collect all unique clan tags from userClans DB
    const clanTagsSet = new Set(
      Object.values(DB.userClans)
        .map(d => (typeof d === "string" ? d : d?.clanTag)?.toUpperCase()?.trim())
        .filter(Boolean)
    );

    if (clanTagsSet.size === 0) return console.log("⏩ No clans in DB.userClans");

    // Track clans already notified in this invocation to prevent duplicates
    const notifiedClans = new Set();

    // Iterate over unique clan tags only
    for (const clanTag of clanTagsSet) {
      if (notifiedClans.has(clanTag)) {
        continue; // skip duplicate clan notifications
      }
      try {
        console.log("\n===============================");
        ...
        // Resolve targets (users and groups) authorized for this clan
        ...
        // Skip preparation phase notifications
        ...
        // Handle live war milestone notifications
        if (war.state === "inWar") {
        ...
        // Handle war-ended notification once
        ...
        // Mark clan as notified to avoid duplicate notifications for this run
        notifiedClans.add(clanTag);
...
async function sendWarNotificationToTargets(sock, war, userTargets = [], groupTargets = [], timeLeft = null, overrideMsg = null) {
  ...
  for (const g of uniq(groupTargets)) {
    ...
    try {
      ...
      sentGroups.add(g);
    } catch (e) { console.error("⚠️ Group send error:", g, e.message); }
  }
  ...
  for (const u of uniq(userTargets)) {
    ...
    } catch (e) { console.error("⚠️ User send error:", u, e.message); }
  }
}
...
HANDLERS.exitaimode = async ({ sock, jid, sender }) => {
  ...
  HANDLERS.add = async ({ sock, jid, args, isOwner }) => {
    if (!isOwnerOrAdmin(sender)) return
 await sock.sendMessage(jid, { text: '❌ You are not the owner of the bot.' });
 ...
 await sock.sendMessage(jid, { text: '❌ You are not the owner of the bot.' });
  ...
  HANDLERS.removegroup = async ({ sock, jid, sender, isOwner }) => {
  if (!isOwnerOrAdmin(sender)) {
