...
async function finalizeClanWar(clanTag) {
  try {
    ...
    // ✅ baseId (prefix only) for war matching
    const baseId = `${war.clan.tag}_${war.opponent.tag}_`;

    console.log(`🏁 Finalizing war logs for ${clanTag} [state=${war.state}] (baseId=${baseId})`);
    ...
    for (const m of war.clan.members || []) {
      ...
      DB.playerWarLogs[playerTag] = DB.playerWarLogs[playerTag].map(l => {
        // 🔐 Match all logs of this war (prefix check instead of exact)
        if (l.isFromLive && l.clanTag === clanTag && l.warId.startsWith(baseId)) {
          const realAtk = attacks.find(a => a.order === l.order);
          ...
          // ✅ Agar pehle se unused slot hai tabhi "Not used" karo
          if (l.unused) {
            return {
              ...l,
              oppName: "Not used",
