...
async function handleWarNotifications(sock) {
  try {
    ...
    // Collect all unique normalized clan tags from userClans DB
    ...
    for (const clanTag of clanTagsSet) {
      if (notifiedClans.has(clanTag)) continue;
      ...
      try {
        ...
        const { groupTargets, userTargets } = await resolveTargetsForClan(
          sock,
          clanTag
        );
        ...
        const baseKey = `${clanTag}:${war.preparationStartTime ||
          war.startTime ||
          "unknown"}`;
