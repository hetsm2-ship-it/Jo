...
async function startBot() {
  ...
  if (!msg || !msg.message) return; // Ignore empty
  ...
  // Lowercase, split into parts
  const commandText = eatPrefix(cleanedText).toLowerCase();
  const parts = commandText.split(/\s+/);
  const command = parts[0];
  const args = parts.slice(1);

  // Now you can use: command, args
...
const isAuthorized = isOwnerFlag || isAuthorizedForGroup || isAuthorizedForDM;
// ============================
...
if (!permitted) return;  // Agar permission nahi hai toh ruk jao


// Commands toggled off: suppress replies for non-admin/non-owner users
if (isGroup) {
  if (DB.offGroupCmds[jid] && !isOwnerFlag && !isAdmin(sender)) {
    console.log(`Commands suppressed in group ${jid} due to offgc active.`);
    return; // Return early; no chat reply
  }
} else {
  if (DB.offDmCmds[sender] && !isOwnerFlag && !isAdmin(sender)) {
    console.log(`Commands suppressed for DM user ${sender} due to offdm active.`);
    return; // Return early; no chat reply
  }
}

// Self-message fix: allow only owner self-messages to proceed
if (msg.key.fromMe) {
  if (sender !== CONFIG.OWNER_JID) return; // block non-owner self messages

  // Prevent spam loops by ignoring repeated identical messages
  if (DB.lastMessages[sender] && DB.lastMessages[sender] === text) return;

  // Store last message for sender
...
if (text.includes('instagram.com/')) {
  // Silent block if group or DM commands are disabled
  if ((isGroupChat && DB.offGroupCmds[jid]) || (!isGroupChat && DB.offDmCmds[sender])) {
    console.log(`[INFO] Skipped Instagram download in ${isGroupChat ? 'group' : 'DM'} ${jid || sender} due to commands disabled.`);
    return; // silently skip without reply
  }
  ...
  try {
    ...
    // Wrap exec in promise for await style
  ...
  return;  // End handling this message
}
...
const cocCommands = [
  'start', 'help', 'botinfo', 'authstatus', 'info', 'qr', 'enteraimode', 'exitaimode',
  'instagram', // to recognize Instagram reel downloads (assumed command or handler)
  'claninfo', 'player', 'playerstats', 'liveattack', 'warlog', 'warlogs', 'cminfo', 'cm',
  'attendance', 'capitalraids', 'clancapital', 'donations', 'goldpass', 'locations',
  'leagues', 'warleagues', 'topclans', 'topplayers', 'clanrankings', 'playerrankings',
  'setclan', 'removeclan', 'whenwar', 'warnotify'
];

const adminCommands = [
  'add', 'remove', 'addgroup', 'removegroup'
];

// ================== Permission and Authorization Check ==================

/**
 * Checks if a command is permitted for the sender based on authorization,
 * group/DM context, on/off toggles, and admin permissions.
 * 
 * @param {Object} params
 * @param {string} params.command - The command string, in lowercase.
 * @param {boolean} params.isGroupChat - True if message is in a group chat.
 * @param {string} params.jid - The jid of the chat (group or DM).
 * @param {string} params.sender - The sender's jid.
 * @param {boolean} params.isAuthorizedForGroup - Whether group is authorized.
 * @param {boolean} params.isAuthorizedForDM - Whether sender is authorized in DM.
 * @param {Object} params.DB - The database object with relevant toggles and auth lists.
 * @param {Object} params.sock - The Baileys socket instance used to send messages.
 * @param {Function} params.isOwnerOrAdmin - Function to check if sender is owner/admin.
 * @returns {Promise<boolean>} - Resolves true if permitted, false if blocked.
 */
async function checkCommandPermission({
  ...
  // Clash of Clans and general bot commands permission check
  if (cocCommands.includes(cmd)) {
    if (isGroupChat) {
      // Check if commands are toggled off for this group
      if (DB.offGroupCmds[jid]) {
        console.log(`[INFO] Command '${cmd}' blocked in group ${jid} due to offGroupCmds toggle.`);
        // Silently block without client message
        return false;
      }
      ...
      // DM (private chat) command handling
      if (DB.offDmCmds[sender]) {
        console.log(`[INFO] Command '${cmd}' blocked in DM from ${sender} due to offDmCmds toggle.`);
        return false; // silently block DM
      }
  ...
  // Admin commands require owner/admin permission
  ...
  // Passed all checks - permit command execution
  return true;
}
