...
async function checkAndSaveWar(clanTag) {
  try {
    ...
    const currentWarId = `${clanTag}_${war.opponent?.tag || "unknown"}_${war.startTime}`;
        ...
        if (l.isFromLive && l.clanTag === clanTag && l.warId === currentWarId) {
          ...
          if (realAtk) {
            ...
            return {
              ...
              unused: false,         // ✅ Attack mila → unused false
              isFromLive: false
            };
          }
          ...
          return {
            ...l,
            unused: true,            // ✅ Attack nahi mila → unused true
            isFromLive: false
          };
        }
        return l;
...
async function finalizeClanWar(clanTag) {
  try {
    ...
    // If opponent/clan data missing → skip
    ...
    // ✅ Use same warId formula as auto-save
    ...
    for (const m of war.clan.members || []) {
      ...
      const attacks = m.attacks || [];
      console.log(`🔎 Finalizing ${m.name} (${playerTag}) | Attacks=${attacks.length}`);
      ...
      DB.playerWarLogs[playerTag] = DB.playerWarLogs[playerTag].map(l => {
        if (l.isFromLive && l.clanTag === clanTag && l.warId === currentWarId) {
          ...
          if (realAtk) {
            const opponent = war.opponent?.members?.find(o => o.tag === realAtk.defenderTag);
            console.log(`✅ Finalized attack ${realAtk.order} for ${m.name} → ${realAtk.stars}⭐ ${realAtk.destructionPercentage}% vs ${opponent?.name || "Unknown"}`);
            return {
              ...l,
              oppName: opponent?.name || "Unknown",
              stars: realAtk.stars,
          ...
          // ⚪ Only if originally unused
          if (l.unused) {
            console.log(`⚪ Marked unused slot ${l.order} for ${m.name}`);
            return {
              ...l,
              oppName: "Not used",
              stars: 0,
          ...
          // ❌ Otherwise just clear live flag
          console.log(`⚠️ No attack found for ${m.name} [slot ${l.order}] → keeping as-is (not marking unused)`);
