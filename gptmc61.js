...
if (!permitted) {
  return; // Agar command allowed nahi, to yahin return kardo
}

      // permissions
      // ===================== Full Command Lists =====================

// Example command lists: adjust as needed


/**
 * Check if a command is allowed to be executed by the sender, given context.
 * Blocks silently for toggled-off commands, replies politely for unauthorized users.
 * 
 * @param {Object} params
 * @param {string} params.command - The command in lowercase.
 * @param {boolean} params.isGroupChat - True if message is in a group chat.
 * @param {string} params.jid - Chat jid (group or DM).
 * @param {string} params.sender - Sender jid.
 * @param {boolean} params.isAuthorizedForGroup - Group authorization flag.
 * @param {boolean} params.isAuthorizedForDM - DM user authorization flag.
 * @param {Object} params.DB - Database object containing toggles and authorization data.
 * @param {Object} params.sock - Baileys socket object for sending messages.
 * @param {Function} params.isOwnerOrAdmin - Function to check if sender is owner or admin.
 * 
...
async function checkCommandPermission({
  ...
  // Only allow known commands, otherwise ignore silently
  // (Optional: if you want to reject unknown commands explicitly, change here)
  if (![...cocCommands, ...adminCommands].includes(cmd)) {
    return false;  // silently block unknown commands
  }

  // For Clash of Clans and general commands
  if (cocCommands.includes(cmd)) {
    if (isGroupChat) {
      if (DB.offGroupCmds[jid]) {
        ...
        return false; // silently block toggled-off group commands
      }
      ...
      if (DB.offDmCmds[sender]) {
        ...
        return false; // silently block toggled-off DM commands
      }
  ...
  // Admin commands require special permission
  ...
  // If reached here, command is permitted
  return true;
}
