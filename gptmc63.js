...
const freeCommands = [
  'start', 'help', 'botinfo', 'authstatus', 'info', 'qr', 'enteraimode', 'exitaimode', 'instagram'
];
...
const adminCommands = ['add', 'remove', 'addgroup', 'removegroup', 'offgc', 'ongc', 'offdm', 'ondm'];

const adminCommands = Object.freeze(['offgc', 'ongc', 'offdm', 'ondm', 'add', 'remove', 'addgroup', 'removegroup']);
...
async function startBot() {
  ...
  try {
    ...
    // Context checks
    const isGroupChat = isJidGroup(jid); // Use isJidGroup for consistency
    ...
    // Async saveDB (writes to JSON file)
    const saveDB = async () => {
      try {
        await require('fs').promises.writeFile(CONFIG.DB_PATH || 'db.json', JSON.stringify(DB, null, 2));
        logger.info(`DB saved successfully for ${jid || sender}`);
      } catch (e) {
        logger.error('Failed to save DB:', e);
      }
    };
    ...
    // Avoid sending error to user to prevent spam; log only
  }
