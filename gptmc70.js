...
const fs = require('fs');           // sync functions ke liye
const fsp = require('fs').promises; // async functions ke liye
...
const saveDB = async (context = 'unknown') => {
  try {
    const tmpFile = CONFIG.DB_PATH + '.tmp';
    const data = JSON.stringify(DB, null, 2);
    await fs.writeFile(tmpFile, data, { encoding: 'utf-8' });
    await fs.rename(tmpFile, CONFIG.DB_PATH);
  } catch (e) {
    try {
      const tmpFile = CONFIG.DB_PATH + '.tmp';
      if (await fs.exists(tmpFile)) await fs.unlink(tmpFile);
    } catch (_) {}
...
const loadDB = async () => {
  try {
    if (!(await fs.exists(CONFIG.DB_PATH))) {
      ensureDBDefaults();
      return;
    }
...
try {
  ...
  parsed = {};
    ...
    for (const [key, defVal] of Object.entries(DEFAULT_DB)) {
      if (Object.prototype.hasOwnProperty.call(parsed, key)) {
        ...
        if (isPlainObject(defVal) && isPlainObject(parsedVal)) {
          DB[key] = { ...defVal, ...parsedVal }; // shallow merge objects
        } else if (Array.isArray(defVal) && Array.isArray(parsedVal)) {
          DB[key] = parsedVal.slice(); // copy array
        } else if (typeof defVal === typeof parsedVal) {
          DB[key] = parsedVal; // same type, take parsed
        } else {
    ...
    // Extra keys in parsed not in DEFAULT_DB (custom fields) → preserve
...
sock.ev.on('messages.upsert', async ({ messages }) => {
  try {
    ...
    // Async saveDB
    const saveDB = async (context = jid || sender) => {
      try {
        await fs.writeFile(CONFIG.DB_PATH, JSON.stringify(DB, null, 2));
        logger.info(`DB saved successfully for ${context}`);
      } catch (e) {
        logger.error(`Failed to save DB for ${context}:`, e);
      }
    };
    ...
    if (text.includes('instagram.com/')) {
      try {
        ...
        const existing = await fs.readdir(outputDir).then(files => files.filter(f => f.startsWith(msg.key.id)));
        ...
        await new Promise((resolve, reject) => {
          execFile(ytDlpCommand, cmdArgs, { cwd: __dirname, timeout: 180000 }, async (error) => {
            ...
            const files = await fs.readdir(outputDir).then(f => f.filter(f => f.startsWith(msg.key.id)));
            if (files.length > 0) {
    ...
    if (isKnown) {
      ...
      DB.aiChatHistory[sender] = DB.aiChatHistory[sender] || [
        {
          role: 'system',
          content: `You are Saad's helpful bot named ${CONFIG.BOT_NAME || 'Bot'}. Respond helpfully and conversationally in Hinglish where possible.`
        }
      ];
      ...
      // Get AI reply (placeholder; replace with actual AI integration)
      let aiResponse;
      try {
        const processAIRequest = async (history) => {
          // Example: Integrate with xAI's API (see https://x.ai/api)
          return `AI response to: ${history[history.length - 1].content}`; // Replace with actual logic
        };
        aiResponse = await processAIRequest(DB.aiChatHistory[sender]);
      } catch (err) {
