...
HANDLERS.enterterminal = async ({ sock, jid, sender }) => {
  ...
  await sock.sendMessage(jid, { text: "💻 Terminal mode ON. Ab aap commands bhej sakte ho.\nType `exitterminal` to exit." });
};
...
// ⚡ Add this block just after you define body
  if (body.startsWith("enterterminal")) 
    return HANDLERS.enterterminal({ sock, jid, sender });

  if (body.startsWith("exitterminal")) 
    return HANDLERS.exitterminal({ sock, jid, sender });

  // Agar user terminal mode me hai, toh uska input direct terminal ko bhej do
  if (activeTerminals[sender]) 
    return HANDLERS.terminalInput({ sock, jid, sender, body });
    // Prefix logic
    const eatPrefix = (str) => {
