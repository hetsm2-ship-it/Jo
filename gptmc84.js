...
let DB = { ...DEFAULT_DB };
const activeTerminals = {}; // { userJid: { cwd: "/root" } }
const logger = {
...
`;
  ...
  authstatus: async ({ sock, jid, isOwner, isAuthorizedForGroup, isAuthorizedForDM }) => {
    ...
    const msg =
      ...
      `├────────────────────────────┤\n` +
      `│ 👑 *Owner* : ${statusIcon(isOwner)}\n` +
      `│ 👥 *Group Access* : ${statusIcon(isAuthorizedForGroup)}\n` +
      `│ 💬 *Direct Access* : ${statusIcon(isAuthorizedForDM)}\n` +
      `╰────────────────────────────╯`;
    await sock.sendMessage(jid, { text: msg });
  } // No comma here because this is the last property
};

  HANDLERS.info = async ({ sock, jid, sender, msg }) => {
    try {
        // PushName ko safe tarike se lo
        const name = (msg && msg.pushName) ? msg.pushName : "Unknown";
...
HANDLERS.removeadmin = async ({ sock, jid, args, sender }) => {
  if (!isOwnerOrAdmin(sender)) return
 sock.sendMessage(jid, { text: "❌ Sirf owner hi `removeadmin` chala sakta hai." });
  const adminJid = args[0];
  if (!adminJid) return sock.sendMessage(jid, { text: `Usage: ${CONFIG.COMMAND_PREFIX}removeadmin 91XXXXXXXXXX@s.whatsapp.net` });
...
HANDLERS.enterterminal = async ({ sock, jid, sender }) => {
  if (!isOwnerOrAdmin(sender)) {
    return sock.sendMessage(jid, { text: "❌ Sirf owner aur admins hi terminal mode use kar sakte hain." });
  }

  if (activeTerminals[sender]) {
    return sock.sendMessage(jid, { text: "⚠️ Aap already terminal mode me ho. Exit karne ke liye `exitterminal` likho." });
  }

  activeTerminals[sender] = { cwd: "/root" }; // default directory
  await sock.sendMessage(
    jid,
    { text: "💻 Terminal mode ON. Ab aap commands bhej sakte ho.\nType `exitterminal` to exit." }
  );
};
...
HANDLERS.exitterminal = async ({ sock, jid, sender }) => {
  ...
  if (activeTerminals[sender]) {
    ...
    await sock.sendMessage(jid, { text: "🚪 Terminal mode exited." });
  } else {
    await sock.sendMessage(jid, { text: "⚠️ Aap terminal mode me nahi ho." });
  }
};

HANDLERS.terminalInput = async ({ sock, jid, sender, body }) => {
  if (!isOwnerOrAdmin(sender)) {
    return sock.sendMessage(jid, { text: "❌ Sirf owner aur admins hi terminal mode use kar sakte hain." });
  }

  const term = activeTerminals[sender];
  if (!term) return;
  ...
  if (cmd.startsWith("cd ")) {
    ...
    if (fs.existsSync(newCwd) && fs.lstatSync(newCwd).isDirectory()) {
      term.cwd = newCwd;
      return sock.sendMessage(jid, { text: `📂 Changed directory to: ${newCwd}` });
    } else {
      return sock.sendMessage(jid, { text: `❌ Directory not found: ${targetDir}` });
    }
  }

  // 🛠 Break command into binary + args
  const parts = cmd.split(/\s+/);
  const bin = parts[0];
  const args = parts.slice(1);

  execFile(bin, args, { cwd: term.cwd, timeout: 20 * 1000, maxBuffer: 1024 * 1024 }, async (error, stdout, stderr) => {
    let output = "";
    if (error) output = `❌ Error: ${error.message}`;
    else output = stdout || stderr || "⚠️ No output.";
...
HANDLERS.offgc = async ({ sock, jid, sender, isGroup, isOwner, isAdmin }) => {
  ...
  const saveDB = async () => {
    try {
      await require('fs').promises.writeFile(CONFIG.DB_PATH || 'db.json', JSON.stringify(DB, null, 2));
      logger.info(`DB saved successfully for ${jid}`);
    } catch (e) {
      logger.error(`Failed to save DB for ${jid}:`, e);
      throw e; // Propagate error to be caught by messages.upsert
    }
  };

  if (!isOwner && !isAdmin) {
    await sock.sendMessage(jid, { text: '❌ Sirf owner aur admins hi `offgc` chala sakte hain.' });
    return;
  }
  if (!isGroup) {
    await sock.sendMessage(jid, { text: '❌ Ye command sirf group me chalta hai.' });
    return;
  }
  DB.offGroupCmds = DB.offGroupCmds || {};
  DB.offGroupCmds[jid] = true;
  await saveDB();
  logger.info(`Group commands disabled for group ${jid} by ${sender}`);
...
HANDLERS.ongc = async ({ sock, jid, sender, isGroup, isOwner, isAdmin }) => {
  ...
  if (!isOwner && !isAdmin) {
    await sock.sendMessage(jid, { text: '❌ Sirf owner aur admins hi `ongc` chala sakte hain.' });
    return;
  }
  if (!isGroup) {
    await sock.sendMessage(jid, { text: '❌ Ye command sirf group me chalta hai.' });
  ...
  if (DB.offGroupCmds[jid]) {
    ...
    await saveDB();
    logger.info(`Group commands enabled for group ${jid} by ${sender}`);
    await sock.sendMessage(jid, { text: '✅ Group commands aur notifications enable kar diye gaye hain is group mein.' });
  } else {
    await sock.sendMessage(jid, { text: '✅ Commands already enabled in this group.' });
  }
};

// DM commands toggle OFF
HANDLERS.offdm = async ({ sock, jid, sender, isGroup, isOwner, isAdmin }) => {
  const logger = {
    info: (msg) => console.log(`[INFO] ${new Date().toISOString()} ${msg}`),
    error: (msg, err) => console.error(`[ERROR] ${new Date().toISOString()} ${msg}`, err)
  };
  const saveDB = async () => {
    try {
      await require('fs').promises.writeFile(CONFIG.DB_PATH || 'db.json', JSON.stringify(DB, null, 2));
      logger.info(`DB saved successfully for ${sender}`);
    } catch (e) {
      logger.error(`Failed to save DB for ${sender}:`, e);
      throw e; // Propagate error
    }
  };

  if (!isOwner && !isAdmin) {
    await sock.sendMessage(jid, { text: '❌ Sirf owner aur admins hi `offdm` chala sakte hain.' });
    return;
  }
  if (isGroup) {
    await sock.sendMessage(jid, { text: '❌ Ye command sirf DM me chalta hai.' });
    return;
  }
  DB.offDmCmds = DB.offDmCmds || {};
  DB.offDmCmds[sender] = true;
  await saveDB();
  logger.info(`DM commands disabled for user ${sender}`);
  await sock.sendMessage(jid, { text: '✅ DM commands aur notifications disable kar diye gaye hain aapke liye.' });
};

// DM commands toggle ON
HANDLERS.ondm = async ({ sock, jid, sender, isGroup, isOwner, isAdmin }) => {
  const logger = {
    info: (msg) => console.log(`[INFO] ${new Date().toISOString()} ${msg}`),
    error: (msg, err) => console.error(`[ERROR] ${new Date().toISOString()} ${msg}`, err)
  };
  ...
  if (!isOwner && !isAdmin) {
    await sock.sendMessage(jid, { text: '❌ Sirf owner aur admins hi `ondm` chala sakte hain.' });
    return;
  }
  if (isGroup) {
    await sock.sendMessage(jid, { text: '❌ Ye command sirf DM me chalta hai.' });
  ...
  if (DB.offDmCmds[sender]) {
    delete DB.offDmCmds[sender];
    await saveDB();
    logger.info(`DM commands enabled for user ${sender}`);
    await sock.sendMessage(jid, { text: '✅ DM commands aur notifications enable kar diye gaye hain aapke liye.' });
  } else {
    await sock.sendMessage(jid, { text: '✅ Commands already enabled in this DM.' });
  }
};
...
HANDLERS.remove = async ({ sock, jid, args, sender }) => {
  ...
  HANDLERS.addgroup = async ({ sock, jid, sender }) => {
  const normalizedSender = jidNormalizedUser(sender);
  if (!isOwnerOrAdmin(normalizedSender)) {
...
HANDLERS.removegroup = async ({ sock, jid, sender }) => {
  const normalizedSender = jidNormalizedUser(sender);
  if (!isOwnerOrAdmin(normalizedSender)) {
    return await sock.sendMessage(jid, { text: '❌ You are not the owner or admin of the bot.' });
  }
...
// ⚡ Add this block just after you define body
  if (text.startsWith("enterterminal")) 
  return HANDLERS.enterterminal({ sock, jid, sender });

if (text.startsWith("exitterminal")) 
  return HANDLERS.exitterminal({ sock, jid, sender });

if (activeTerminals[sender]) 
  return HANDLERS.terminalInput({ sock, jid, sender, body: text });
    // Prefix logic
    const eatPrefix = (str) => {
    ...
    const isOwnerFlag = isOwner(sender);
    const isAdminFlag = isAdmin(sender);
    ...
    // Command suppression (allow ongc/ondm for admins/owners)
    if (isGroupChat && DB.offGroupCmds[jid]) {
      if (command === 'ongc' && (isOwnerFlag || isAdminFlag)) {
        ...
        logger.info(`Commands blocked in group ${jid} due to offgc.`);
        return;
      }
    }
    if (!isGroupChat && DB.offDmCmds[sender]) {
      if (command === 'ondm' && (isOwnerFlag || isAdminFlag)) {
        ...
        logger.info(`Commands blocked for DM ${sender} due to offdm.`);
        return;
      }
    }
...
if (text.includes('instagram.com/')) {
  try {
    ...
    await fsp.mkdir(outputDir, { recursive: true }); // ✅ FIXED (fsp = fs.promises)
    ...
    const existing = (await fsp.readdir(outputDir))
      .filter(f => f.startsWith(msg.key.id)); // ✅ FIXED
    ...
    await new Promise((resolve, reject) => {
      execFile(ytDlpCommand, cmdArgs, { cwd: __dirname, timeout: 180000 }, async (error) => {
        ...
        if (files.length > 0) {
          ...
          await sock.sendMessage(jid, { video: { url: filePath }, caption: `✅ Downloaded:\n${text}` });

          // Cleanup after sending
          await fsp.unlink(filePath).catch(e => logger.error('Failed to clean up file:', e));
          resolve();
        } else {
  ...
  return;
    ...
    if (!isOwnerFlag && isOnCooldown(sender, CONFIG.AI_COOLDOWN_SEC || 10)) {
        await sock.sendMessage(jid, {
            text: `⏳ Thoda intezaar karein. (AI cooldown: ${CONFIG.AI_COOLDOWN_SEC || 10}s)`
        });
        return;
    }
    ...
    try {
        ...
        const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({
                model: "meta-llama/llama-4-maverick-17b-128e-instruct", // ✅ chosen model
    ...
    if (!aiResponse || aiResponse.trim() === '') {
        await sock.sendMessage(jid, { text: '❌ AI ka reply blank aaya.' });
        return;
    }
