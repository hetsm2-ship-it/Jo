...
HANDLERS.enterterminal = async ({ sock, jid, sender }) => {
  ...
  conn.on("ready", async () => {
    activeTerminals[sender] = { conn, cwd: "~" }; // start from home dir
    await sock.sendMessage(jid, { text: "💻 SSH Terminal mode ON. Ab aap commands bhej sakte ho.\nType `exitterminal` to exit." });
  }).on("error", async (err) => {
    await sock.sendMessage(jid, { text: `❌ SSH connection error: ${err.message}` });
  }).connect(SSH_CONFIG);
};
...
HANDLERS.terminalInput = async ({ sock, jid, sender, body }) => {
  ...
  if (cmd.startsWith("cd ")) {
    ...
    session.conn.exec(remoteCmd, (err, stream) => {
      if (err) return sock.sendMessage(jid, { text: `❌ Error: ${err.message}` });
      let output = "";
      stream.on("data", (data) => output += data.toString());
      stream.stderr.on("data", (data) => output += data.toString());
      stream.on("close", () => {
        if (output.startsWith("/")) {
          session.cwd = output.trim();
  ...
  // 🚀 Run remote command
  const remoteCmd = `cd "${session.cwd}" && ${cmd}`;
  session.conn.exec(remoteCmd, (err, stream) => {
    if (err) return sock.sendMessage(jid, { text: `❌ Error: ${err.message}` });
    let output = "";
    stream.on("data", (data) => output += data.toString());
    stream.stderr.on("data", (data) => output += data.toString());
    stream.on("close", () => {
      if (!output.trim()) output = "⚠️ No output.";
...
// ⚡ Add this block just after you define body
  if (text.toLowerCase().startsWith("enterterminal")) 
  return HANDLERS.enterterminal({ sock, jid, sender });

if (text.toLowerCase().startsWith("exitterminal")) 
