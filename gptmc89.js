...
`;
  ...
  authstatus: async ({ sock, jid, isOwner, isAdmin, isAuthorizedForGroup, isAuthorizedForDM }) => {
...
HANDLERS.offdm = async ({ sock, jid, sender, isGroup, isOwner, isAdmin, args }) => {
  ...
  await sock.sendMessage(jid, { text: `✅ DM commands aur notifications disable kar diye gaye hain *${target}* ke liye.` });
};


HANDLERS.ondm = async ({ sock, jid, sender, isGroup, isOwner, isAdmin, args }) => {
  if (!isOwner && !isAdmin) {
    return sock.sendMessage(jid, { text: "❌ Sirf owner aur admins hi `ondm` chala sakte hain." });
  }
  ...
  // Group me ho toh jid dena mandatory
  if (isGroup && !args?.[0]) {
    return sock.sendMessage(jid, { text: "❌ Group se sirf specific JID ke liye hi enable kar sakte ho.\nUsage: ondm <jid>" });
  }
  ...
  if (DB.offDmCmds[target]) {
    ...
    await sock.sendMessage(jid, { text: `✅ DM commands aur notifications enable kar diye gaye hain *${target}* ke liye.` });
  } else {
    await sock.sendMessage(jid, { text: `⚠️ Commands already enabled for *${target}*.` });
  }
};
