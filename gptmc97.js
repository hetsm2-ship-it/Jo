...
HANDLERS.remove = async ({ sock, jid, args, sender }) => {
  ...
  const normalizedSender = jidNormalizedUser(sender);
  const normalizedJid = jidNormalizedUser(jid); // ✅ yeh line add karo
  if (!isOwnerOrAdmin(normalizedSender)) {
    return await sock.sendMessage(jid, { text: '❌ You are not the owner or admin of the bot.' });
  }
  DB.authorisedGroups[normalizedJid] = true; // ✅ normalized JID store karo
  saveDB();
...
HANDLERS.removegroup = async ({ sock, jid, sender }) => {
  const normalizedSender = jidNormalizedUser(sender);
  const normalizedJid = jidNormalizedUser(jid); // ✅ yeh bhi add karo
  if (!isOwnerOrAdmin(normalizedSender)) {
    return await sock.sendMessage(jid, { text: '❌ You are not the owner or admin of the bot.' });
  }
  delete DB.authorisedGroups[normalizedJid]; // ✅ normalized JID se delete karo
  saveDB();
