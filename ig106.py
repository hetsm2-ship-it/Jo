...
def get_user_speed(tg_id: int) -> float:
    with get_conn() as conn:
        ...
        row = cur.fetchone()
        if row is not None and row[0] is not None:   # ✅ 0.0 bhi accept hoga
            return float(row[0])
...
def unlock_account_and_client(tg_id: int, passphrase: str = None) -> Client:
    # ❌ Purana client mat use karo
    if tg_id in UNLOCKED_CLIENTS:
        del UNLOCKED_CLIENTS[tg_id]

    row = get_default_account_row(tg_id)
    if not row:
        raise ValueError("⚠️ No default account set. Use /setig <id> first.")

    acc_id, ig_username, encrypted_blob, session_file = row
    if not session_file or not os.path.exists(session_file):
        raise ValueError(f"⚠️ Session expired. Logout your {ig_username} and retry with /login")

    if not passphrase:
        passphrase = get_passphrase_for_account(tg_id, ig_username)
        if not passphrase:
            raise ValueError("⚠️ Account locked. Please provide passphrase via /login")
...
async def msgspeed_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 1:
        await update.message.reply_text(
            f"⏱️ Current speed: *{get_user_speed(tg_id)}* seconds and ultra\n"
            f"Usage: /msgspeed 0.4 , ultra",
            parse_mode="Markdown"
        )
        return

    # ✅ Ultra mode
    if args[0].lower() == "ultra":
        set_user_speed(tg_id, 0.0)  # no delay
        await update.message.reply_text("🚀 Speed set to *Ultra* (no delay between messages)", parse_mode="Markdown")
        return

    # ✅ Normal numeric speed
    try:
        speed = float(args[0])
        if speed <= 0:
            await update.message.reply_text("❌ Speed must be greater than 0. Or use /msgspeed ultra")
            return
    except ValueError:
        await update.message.reply_text("❌ Invalid input. Usage: `/msgspeed 0.2` or `/msgspeed ultra`", parse_mode="Markdown")
        return
...
async def setig(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_conn() as conn:
        cur = conn.cursor()
        # Check if this account is already default
        cur.execute(
            "SELECT default_account FROM ig_accounts WHERE id=? AND tg_id=?",
            (acc_id, tg_id)
        )
        row = cur.fetchone()
        if row and row[0] == 1:
            await update.message.reply_text("ℹ️ This account is already set as default.")
            return

        # Otherwise reset and set default
        cur.execute("UPDATE ig_accounts SET default_account=0 WHERE tg_id=?", (tg_id,))
        cur.execute(
            "UPDATE ig_accounts SET default_account=1 WHERE id=? AND tg_id=?",
            (acc_id, tg_id)
        )
        conn.commit()

    await update.message.reply_text("✅ Default account set.")
    # ✅ Clear unlocked client cache for this user
    if tg_id in UNLOCKED_CLIENTS:
        del UNLOCKED_CLIENTS[tg_id]
...
async def massmsg_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if tg_id in sending_threads and sending_threads[tg_id].is_alive():
        await update.message.reply_text("⚠️ A process is already in progress. Type /stop to stop 🛑")
        return ConversationHandler.END
...
def main():
    ...
    login_conv = ConversationHandler(
        ...
        fallbacks=[
            ...
            CommandHandler('massmsg', massmsg_start),  # restart massmsg anytime
        ],
    )
