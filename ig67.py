...
def create_instaclient(sessionfile: Optional[str]=None) -> Client:
    ...
    def challenge_handler(username, choice):
        # Save pending OTP state
        PENDING_OTP[username] = {
    "client": cl,
    "password": ig_pass,       # login_password function se milega
    "passphrase": passphrase,  # user input se milega
}
        print(f"[DEBUG] OTP required for {username} via {choice}")
        # Don't return code here, just wait
        return None
...
async def login_passphrase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"[DEBUG] login_passphrase Exception: {e}")   # Debug
    if "challenge_required" in str(e).lower():
        await update.message.reply_text("🔐 Instagram is asking for OTP. Please enter: /otp <6digit>")
        PENDING_OTP[ig_user] = {
            "client": cl,
            "password": context.user_data.get("ig_login_password"),  # ✅ safe fix
            "passphrase": passphrase,
        }
        print(f"[DEBUG] OTP required for {ig_user}, stored in PENDING_OTP")  # Debug
        return ConversationHandler.END
    else:
        raise
...
blob_data = {
    "username": ig_user,
    "password": context.user_data.get("ig_login_password"),  # ✅ same fix here too
