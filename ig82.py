...
def create_instaclient(sessionfile: Optional[str] = None) -> Client:
    ...
    def challenge_handler(username, choice):
        print(f"[DEBUG] OTP required for {username} via {choice}")

        # Agar OTP pehle se pending hai to dobara notify na karo
        if username not in PENDING_OTP:
            PENDING_OTP[username] = {
                "client": cl,
                "password": None,
                "passphrase": None,
                "needs_otp": True
            }
            # 🔔 Notify bot owner on Telegram (sirf pehli baar)
            try:
                import asyncio
                ...
                asyncio.get_event_loop().create_task(
                    bot.send_message(
                        chat_id=BOT_OWNER_TELEGRAM_ID,
                        text=(
                            f"🔐 Instagram is asking OTP for *{username}* via {choice}.\n\n"
                ...
                print(f"[DEBUG] Failed to notify OTP in Telegram: {e}")
...
async def login_passphrase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ig_user = context.user_data.get("ig_login_username")
    ig_pass = context.user_data.get("ig_login_password")

    print(f"[DEBUG] login_passphrase by {tg_id}, user={ig_user}, pass={ig_pass}, passphrase={passphrase}")  # Debug

    if not ig_user or not ig_pass:
        await update.message.reply_text("❌ Missing username or password in session. Please /login again.")
        return ConversationHandler.END

    session_file = session_filename_for(ig_user, int(time.time()))
    ...
    try:
        ...
        try:
            ...
            print(f"[DEBUG] login_passphrase Exception: {e}")
            await update.message.reply_text(f"❌ Login failed: {e}")
            return ConversationHandler.END

        # ✅ Check if OTP is required (set by challenge_handler)
        if ig_user in PENDING_OTP and PENDING_OTP[ig_user].get("needs_otp"):
            PENDING_OTP[ig_user].update({
                "password": ig_pass,
                "passphrase": passphrase
            })
            await update.message.reply_text("🔐 Instagram is asking for OTP. Please enter: /otp <6digit>")
            print(f"[DEBUG] OTP required for {ig_user}, stored in PENDING_OTP with pass+passphrase")
            return ConversationHandler.END
        ...
        with get_conn() as conn:
            ...
            cur.execute(
                ...
                VALUES(?,?,?,?,?)
                """,
                (tg_id, ig_user, encrypted, session_file, 0),
            )
            save_passphrase_for_account(tg_id, ig_user, passphrase)
            conn.commit()

        await update.message.reply_text("✅ Account saved and encrypted successfully. Use /viewmyac to see.")
        print(f"[DEBUG] Account {ig_user} saved for Telegram user {tg_id}")  # Debug

    except Exception as e:
        print(f"[DEBUG] login_passphrase FAILED: {e}")  # Debug
...
async def otp_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    tg_id = update.effective_user.id
    print(f"[DEBUG] otp_handler called by {tg_id}, args={context.args}")   # Debug
    ...
    for username, otp_data in list(PENDING_OTP.items()):
        ...
        print(f"[DEBUG] Processing OTP for {username}, code={code}, pass={ig_pass}, passphrase={passphrase}")  # Debug

        # ✅ yeh block loop ke andar hona chahiye
        if not ig_pass or not passphrase:
            ...
            print(f"[DEBUG] OTP aborted for {username}: missing pass/passphrase")
            continue

        try:
            cl.challenge_code(code)

            session_file = session_filename_for(username, int(time.time()))
            cl.dump_settings(session_file)

            blob_data = {"username": username, "password": ig_pass, "session_file": session_file}
            encrypted = encrypt_blob(tg_id, passphrase, blob_data)
            ...
            with get_conn() as conn:
                ...
                cur.execute(
                    ...
                    VALUES(?,?,?,?,?)
                    """,
                    (tg_id, username, encrypted, session_file, 0),
                )
                save_passphrase_for_account(tg_id, username, passphrase)
                conn.commit()

            await update.message.reply_text(f"✅ OTP accepted, logged in and saved account: {username}")
            print(f"[DEBUG] OTP success for {username}, account saved.")   # Debug

            del PENDING_OTP[username]
            return

        except Exception as e:
            print(f"[DEBUG] OTP failed for {username}: {e}")   # Debug
            await update.message.reply_text(f"❌ OTP failed for {username}: {e}")
            return

    await update.message.reply_text("⚠️ No OTP request pending.")
    print("[DEBUG] otp_handler: No pending OTP found.")   # Debug
            ...
            if passphrase:
                try:
                    ...
                    for i, t in enumerate(gc_threads, start=1):
                        # Show title if available, otherwise first 3 usernames
    ...
    massmsg_conv = ConversationHandler(
        ...
        states={
            ...
            ASK_DM_MESSAGES: [
                ...
                MessageHandler(filters.Document.TEXT, massmsg_collect_messages_dm)
            ],
            ASK_GC_MESSAGES: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, massmsg_collect_messages_gc),
                MessageHandler(filters.Document.TEXT, massmsg_collect_messages_gc)
            ],
        },
        fallbacks=[CommandHandler('cancel', cancel)],
    )
