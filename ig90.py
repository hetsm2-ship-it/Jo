...
LAST_N_THREADS = 10   # <- isko aise rehne do ya 5 kar do
MAX_SAFE_GC = 5       # new line add
...
gc_threads = gc_threads[:MAX_SAFE_GC]  # sirf last 5 GC safe ke liye
                    if not gc_threads:
                    ...
                    for i, t in enumerate(gc_threads, start=1):
                        ...
                        if not title:
                            usernames = [getattr(u, "username", "?") for u in users[:3]]
title = ", ".join(usernames) if usernames else "Unnamed GC"
                        msg += f"{i}. {title}\n"
