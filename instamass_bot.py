...
async def login_passphrase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ Username aur password context se le rahe hain
    ig_user = context.user_data.get("ig_login_username")
    ig_pass = context.user_data.get("ig_login_password")   # <-- ERROR yahi line missing thi
    ...
    try:
        await update.message.reply_text(
            "⏳ Attempting Instagram login... (this may take a few seconds)"
        )
        ...
        try:
            ...
            if "challenge_required" in str(e).lower():
                await update.message.reply_text(
                    "🔐 Instagram is asking for OTP. Please enter: /otp <6digit>"
                )
                PENDING_OTP[ig_user] = {
                    "client": cl,
                    "password": ig_pass,
        ...
        await update.message.reply_text(
            "✅ Account saved and encrypted successfully. Use /viewmyac to see."
        )
