...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    function cwlUsedAttacksCurrent(playerTag, cwlData) {
      const player = cwlData?.rounds?.flatMap(r => r.clan.members).find(m => m.tag === playerTag);
      return player?.attacks?.length > 0 ? 1 : 0;
    }
    ...
    // CWL data bhi fetch karenge aur check karenge war state
    const cwlData = await fetchCWLData(clanTag);
    const cwlWarState = cwlData?.state || "notInWar";

    // Final: decide karo kis war ka data dikhana hai
    // Agar normalWarState notInWar hai to cwlWarState dekho
    // Agar cwlWarState 'inWar', 'warEnded' ya 'preparation' hai to cwl data dikhao
    // Nahi to normal war data dikhao

    let showCWL = false;
    if (normalWarState === "notInWar") {
      if (cwlWarState === "inWar" || cwlWarState === "preparation" || cwlWarState === "warEnded") {
        showCWL = true;
      }
    } else {
      // Normal war chal rahi hai, check karo agar cwl me aisa state hai to show karna chahiye kya
      if (cwlWarState === "inWar" || cwlWarState === "preparation" || cwlWarState === "warEnded") {
        showCWL = true;
      }
    }
    // Agar showCWL true hai tab sirf CWL ka dikhao
    // Nahi to normal war ka dikhao

    // Function to build and send normal war clan member or player info
      ...
      if (!args[0]) {
        ...
        for (const [i, m] of clanData.memberList.entries()) {
          ...
          if (normalWarState === "inWar") {
            ...
            if (!wm) status = "(not in current war)";
            else {
    ...
    // Function to build and send CWL war info
    async function sendCWLInfo() {
      if (!cwlData) {
        // Agar data hi nahi mila
        return await sock.sendMessage(jid, { text: "⚠️ CWL data unavailable." });
      }
      ...
      if (!args[0]) {
        ...
        if (cwlWarState === "inWar") {
          ...
          const playerCWL = cwlData.rounds.flatMap(r => r.clan.members).find(mem => mem.tag === player.tag);
