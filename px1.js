...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    async function fetchCWLData(clanTag) {
      try {
        return await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
      } catch {
        return null;
      }
    }
    ...
    async function fetchLastCWLWarLog(clanTag) {
      try {
        return await cocFetch(`/clans/${encodeURIComponent(clanTag)}/warlog?limit=10`);
      } catch {
        return null;
      }
    }
    ...
    // Pehle normal war aur clan info fetch karo
    const clanData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}`);
    ...
    // CWL data bhi fetch karo
    const cwlData = await fetchCWLData(clanTag);
    const cwlWarState = cwlData?.state || "notInWar";

    // Decide karo kis war ka data show karna hai
    let showCWL = false;
    if (normalWarState === "notInWar" && (cwlWarState === "inWar" || cwlWarState === "preparation" || cwlWarState === "warEnded")) {
      showCWL = true;
    }
    if ((normalWarState === "inWar" || normalWarState === "preparation" || normalWarState === "warEnded") && (cwlWarState === "inWar" || cwlWarState === "preparation" || cwlWarState === "warEnded")) {
      showCWL = true;
    }
    ...
    async function sendCWLInfo() {
      // Safe check for cwlData and clan
      if (!cwlData?.clan?.name || !Array.isArray(cwlData.clan.members)) {
      ...
      if (!args[0]) {
        ...
        // player detail with CWL attacks
        const playerNo = parseInt(args[0], 10);
        const player = (clanData?.memberList || [])[playerNo - 1];
        if (!player) return await sock.sendMessage(jid, { text: "❌ Invalid player number." });
