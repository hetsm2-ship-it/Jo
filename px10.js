...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  msg += "Leader wasn't selected this member in last war\n";
              ...
              if (warLogsApi?.reason === "accessDenied") status = "(clan is private can't show stats)";
              else {
                const lastWar = warLogsApi?.items?.[0];
                const playerInLastWar = lastWar?.clan?.members?.find(x => x.tag === m.tag);
                if (!playerInLastWar) status = "(not in last war)";
                else {
              ...
              status =
                ...
                usedCount === 0 && unusedCount === 2 ? "(unused attacks in last war)" :
                usedCount === 1 ? "(used 1 attack in last war)" :
                usedCount === 2 ? "(used 2 attacks in last war)" : "(can't show last stats)";
            }
          }
          else status = "(no active war right now)";
          msg += status + "\n\n";
        }
        return await sock.sendMessage(jid, { text: msg });
      }
  ...
  // Find defender name from opponent roster
  const opponentMembers = warData.opponent?.members || [];
  const defender = opponentMembers.find(mem => mem.tag === atk.defenderTag);
  const defenderName = defender?.name || atk.defenderTag;
