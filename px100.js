...
// --- War history with EXTREME DEBUG + FORCE CWL DETECTION ---
const playerLogsRaw = DB.playerWarLogs?.[player.tag] || [];
console.log(`🔍 [DEBUG] Player ${player.name} (${player.tag}) total logs:`, playerLogsRaw.length);

const warLogs = playerLogsRaw.slice(-50).reverse(); // last 50 logs
...
if (warLogs.length) {
  ...
  const grouped = {};
  for (const log of warLogs) {
    const key = (log.warId || `${log.clanTag}_${log.oppClanTag}` || "").trim();
    if (!grouped[key]) grouped[key] = [];
    grouped[key].push(log);
  }

  let idx = 1;
  for (const [warId, logs] of Object.entries(grouped)) {
    console.log(`\n🆔 [DEBUG] Checking warId=${warId} logs count=${logs.length}`);

    // Force detection: pick first with cwlRound present
    const cwlLog = logs.find(l => {
      const hasCWL = l.cwlRound !== undefined && l.cwlRound !== null && l.cwlRound !== "" && l.cwlRound !== "null";
      if (hasCWL) console.log(`✅ [DEBUG] Found CWL log in warId=${warId} → cwlRound=`, l.cwlRound, "isFromLive=", l.isFromLive);
      return hasCWL;
    });

    const isCWL = Boolean(cwlLog);
    const round = isCWL ? Number(cwlLog.cwlRound) || cwlLog.cwlRound : null;
    const sample = cwlLog || logs[0];

    console.log(`[DEBUG] warId=${warId} → isCWL=${isCWL} round=${round}`);

    // Build message header
    msg += `No.${idx++} ${isCWL ? "(CWL) " : ""}🏰 ${sample.myClan} (${sample.clanTag}) vs ${sample.oppClan} (${sample.oppClanTag})`;
    if (isCWL) msg += ` (Round ${round})`;
    msg += "\n";

    // Sort attacks
    logs.sort((a, b) => (a.order || 0) - (b.order || 0)).forEach((atk, i) => {
      console.log(`   🎯 [DEBUG] atk order=${atk.order} stars=${atk.stars} %=${atk.destructionPercentage} cwlRound=${atk.cwlRound}`);
      const notUsed = (atk.destructionPercentage === 0 && atk.stars === 0) || atk.unused;
      if (isCWL) {
        msg += `Attack → ${notUsed ? "Not used" : `vs ${atk.oppName || "Unknown"} → ${atk.destructionPercentage}% (${atk.stars}⭐)`}\n`;
      } else {
        msg += `Attack ${i + 1} → ${notUsed ? "Not used" : `vs ${atk.oppName || "Unknown"} → ${atk.destructionPercentage}% (${atk.stars}⭐)`}\n`;
      }
    });
