...
// --- EXTREME DEBUG: Print entire logs for this player ---
const playerLogsRaw = DB.playerWarLogs?.[player.tag] || [];
console.log(`\n🟡 [DEBUG] Player ${player.name} (${player.tag}) has ${playerLogsRaw.length} total logs:`);
playerLogsRaw.forEach((l, i) => {
  console.log(`   [${i}] warId=${l.warId} cwlRound=${l.cwlRound} isFromLive=${l.isFromLive} myClan=${l.myClan} opp=${l.oppClan} stars=${l.stars} %=${l.destructionPercentage}`);
});

const warLogs = playerLogsRaw.slice(-50).reverse();
...
if (warLogs.length) {
  ...
  for (const [warId, logs] of Object.entries(grouped)) {
    console.log(`\n🆔 [DEBUG] warId=${warId}, logsInThisGroup=${logs.length}`);
    logs.forEach((l) => console.log(`    🔹 log: order=${l.order} cwlRound=${l.cwlRound} isFromLive=${l.isFromLive}`));

    const cwlLog = logs.find(l => l.cwlRound !== undefined && l.cwlRound !== null && l.cwlRound !== "" && l.cwlRound !== "null");
    const isCWL = Boolean(cwlLog);
    const round = isCWL ? Number(cwlLog.cwlRound) || cwlLog.cwlRound : null;
    const sample = cwlLog || logs[0];

    console.log(`   ✅ [DEBUG] Detected ${isCWL ? "CWL" : "Normal"} War${isCWL ? ` Round=${round}` : ""}`);
    ...
    logs.sort((a, b) => (a.order || 0) - (b.order || 0)).forEach((atk, i) => {
      ...
      msg += isCWL
        ? `Attack → ${notUsed ? "Not used" : `vs ${atk.oppName || "Unknown"} → ${atk.destructionPercentage}% (${atk.stars}⭐)`}\n`
        : `Attack ${i + 1} → ${notUsed ? "Not used" : `vs ${atk.oppName || "Unknown"} → ${atk.destructionPercentage}% (${atk.stars}⭐)`}\n`;
    });

    msg += "\n";
  }
} else {
  console.log("🔴 [DEBUG] No logs found for this player!");
  msg += "📜 *War History:* No saved logs ❌\n";
}
