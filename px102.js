...
// --- War history with integrated CWL detection ---
const playerLogsRaw = DB.playerWarLogs?.[player.tag] || [];
console.log(`🔎 [DEBUG] Scanning ${player.name} (${player.tag}) logs:`, playerLogsRaw.length);

// Filter only last 50 logs to avoid spam
const warLogs = playerLogsRaw.slice(-50).reverse();

if (warLogs.length) {
  msg += "📜 *War History:*\n";

  // Group logs by warId (or clanTag+oppClanTag fallback)
  const grouped = {};
  for (const log of warLogs) {
    const key = log.warId || `${log.clanTag}_${log.oppClanTag}`;
    if (!grouped[key]) grouped[key] = [];
  ...
  for (const [warId, logs] of Object.entries(grouped)) {
    // --- CWL detection logic (same as detectCWL.js) ---
    const cwlLogs = logs.filter(
      (l) =>
    ...
    const round = isCWL ? Number(cwlLogs[0].cwlRound) || cwlLogs[0].cwlRound : null;

    console.log(
      `🆔 [DEBUG] warId=${warId} logs=${logs.length} isCWL=${isCWL} round=${round}`
    );
