...
HANDLERS.cminfo = async function ({ sock, jid, args, sender, isGroup }) {
  try {
    ...
    function normalizeKey(log) {
      // Prefer warId/warKey, fallbacks to clan/opp tags and time
      if (!log) return "unknown";
      if (log.warId) return String(log.warId).trim();
      if (log.warKey) return String(log.warKey).trim();
      if (log.clanTag && log.oppClanTag && log.time) return `${String(log.clanTag).trim()}_${String(log.oppClanTag).trim()}_${String(log.time).trim()}`;
      if (log.clanTag && log.oppClanTag) return `${String(log.clanTag).trim()}_${String(log.oppClanTag).trim()}`;
      return `${String(log.myClan || "")}_${String(log.oppClan || "")}`.trim() || "unknown";
    }

    function formatAttackLine(isCWL, atk, numberingIndex = 0) {
      const notUsed = (atk.destructionPercentage === 0 && atk.stars === 0) || atk.unused;
      const oppName = atk.oppName || atk.defenderName || atk.opp || "Unknown";
      if (isCWL) {
    ...
    /**
     * formatPlayerHistory
     * - Reads DB.playerWarLogs[playerTag]
     * - Groups logs by normalized war key
     * - If any log in a group has cwlRound -> treat whole group as CWL (show (CWL) and (Round n), no numbering)
     * - Else treat as normal war: show Attack 1/2 from last non-live logs (if available)
     */
    function formatPlayerHistory(playerTag, opts = { limit: 50 }) {
      const playerLogsRaw = DB.playerWarLogs?.[playerTag] || [];
      if (DEBUG) console.log(`🔎 [DEBUG] formatPlayerHistory for ${playerTag}: total logs =`, playerLogsRaw.length);

      if (!playerLogsRaw.length) return "📜 *War History:* No saved logs ❌\n";

      // We'll use the last N logs (preserve chronological order reversed so newest first visually)
      const warLogs = playerLogsRaw.slice(-opts.limit).reverse(); // newest first in array

      // Group by normalized key
      const grouped = {};
      for (const log of warLogs) {
        const key = normalizeKey(log);
      ...
      for (const [groupKey, logs] of Object.entries(grouped)) {
        // Determine if CWL: any log in group has cwlRound present (accept numbers and strings)
        const cwlLogs = logs.filter((l) => hasCwl(l));
        const isCWL = cwlLogs.length > 0;

        // Choose sample for header
        const sample = isCWL ? cwlLogs[0] : logs[0];

        // If multiple cwlRound values exist, pick the most common or numeric max
        let round = null;
        if (isCWL) {
          const roundVals = cwlLogs.map((l) => {
            const v = l.cwlRound;
            if (typeof v === "number") return v;
            const n = Number(v);
            return Number.isNaN(n) ? v : n;
          });
          // prefer numeric max if numeric; else first non-empty
          const numericRounds = roundVals.filter((r) => typeof r === "number");
          if (numericRounds.length > 0) round = Math.max(...numericRounds);
          else round = roundVals[0];
        }

        if (DEBUG) console.log(`🆔 [DEBUG] groupKey="${groupKey}" size=${logs.length} isCWL=${isCWL} round=${round}`);

        // Header
        msg += `No.${idx++} ${isCWL ? "(CWL) " : ""}🏰 ${sample.myClan} (${sample.clanTag}) vs ${sample.oppClan} (${sample.oppClanTag})`;
        if (isCWL && (round !== null && round !== undefined && round !== "")) msg += ` (Round ${round})`;
        msg += "\n";

        // Attacks output
        if (isCWL) {
          // show all attacks found in this group in order (no numbering)
          const sorted = logs.slice().sort((a, b) => (a.order || 0) - (b.order || 0));
          for (const atk of sorted) {
            msg += formatAttackLine(true, atk, null);
          }
        } else {
          // For normal war groups: show last two non-live attacks (same as old behavior)
          const nonLive = logs.filter((l) => !l.isFromLive).slice(-2).sort((a, b) => (a.order || 0) - (b.order || 0));
          if (!nonLive.length) {
            ...
            // if only one attack found, show Attack 2 → Not used
    ...
    // =====================================================================
    if (!args[0]) {
      // show clan members overview (normal war or CWL overview)
      const clanData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}`);
      ...
      if (warState !== "notInWar") {
        ...
        for (const [i, m] of clanData.memberList.entries()) {
          ...
          if (warState === "inWar") {
            ...
            // warEnded or preparation: fall back to DB logs if API not allowed
      ...
      // CWL overview mode (no normal war running)
      const league = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
      const rounds = league.rounds || [];
      let found = false,
        state = "",
      ...
      if (found && myClan) {
        msg += `🏆 *CWL State: ${
          state === "inWar" ? "Live War" : state === "warEnded" ? "War Ended" : "Preparation"
        } (Round ${roundIndex})*\nClan: ${myClan.name} (${myClan.tag})\n\n`;
    ...
    if (!isNaN(args[0])) {
      ...
      const playerMain = clanData.memberList[playerNo - 1];

      if (!playerMain) {
        return await sock.sendMessage(jid, { text: "❌ Invalid player number." });
      }

      let msg = `📌 *Player War Details for ${playerMain.name} (${playerMain.tag})*\n\n`;

      // --- Normal War Player ---
      if (warState && warState !== "notInWar") {
        if (warState === "inWar") {
          msg += `🏰 ${warData.clan.name} (${warData.clan.tag}) vs ${warData.opponent.name} (${warData.opponent.tag})\n\n`;
          const wm = warData.clan.members.find((x) => x.tag === playerMain.tag);
          if (wm) {
            msg += "🔥 *Current War:*\n";
            msg +=
              wm.attacks?.map(
                (atk, idx) =>
                  `Attack ${idx + 1} → vs ${warData.opponent.members.find((o) => o.tag === atk.defenderTag)?.name || "Unknown"} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`
        ...
        // Show player history (preserving old normal-war behavior where non-live last two attacks shown)
        msg += formatPlayerHistory(playerMain.tag, { limit: 50 });
        return await sock.sendMessage(jid, { text: msg.trim() });
      }

      // --- CWL Player (no normal war active) ---
      const league = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
      const rounds = league.rounds || [];
      let found = false,
        state = "",
      ...
      if (!playerCWL) {
        return await sock.sendMessage(jid, { text: "❌ Invalid player number for CWL roster." });
      }
      ...
      // After current attacks, show history (detect CWL groups and print accordingly)
      msg += formatPlayerHistory(playerMain.tag, { limit: 200 });
      return await sock.sendMessage(jid, { text: msg.trim() });
    }

    // If no branch matched (shouldn't happen)
