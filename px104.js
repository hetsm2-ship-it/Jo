...
HANDLERS.cminfo = async function ({ sock, jid, args, sender, isGroup }) {
  try {
     ...
     */
    function formatPlayerHistory(playerTag, opts = { limit: 10 }) {
  ...
  // Reverse chronological order, so newest first
  const warLogs = playerLogsRaw.slice().reverse();

  // Group logs by normalized key
  ...
  // Convert to array & sort by most recent log timestamp (or array order)
  const groupedArr = Array.from(groupedMap.entries())
    .map(([key, logs]) => {
      // Sort logs in this group by order ascending (attack order)
      logs.sort((a, b) => (a.order || 0) - (b.order || 0));
      // Use timestamp from last log as "date"
      const lastLog = logs[logs.length - 1];
      return { key, logs, lastTime: new Date(lastLog.time || 0).getTime() || 0 };
    })
    .sort((a, b) => b.lastTime - a.lastTime); // newest war first

  // Only keep last N groups (not last N logs)
  const recentGroups = groupedArr.slice(0, opts.limit);
  ...
  for (const group of recentGroups) {
    ...
    // Determine round properly (use first unique value)
    let round = null;
    if (isCWL) {
      const roundSet = [...new Set(cwlLogs.map((l) => String(l.cwlRound)))];
      if (roundSet.length === 1) round = roundSet[0];
      else {
        // multiple rounds found — pick last one (probably latest war)
        round = roundSet[roundSet.length - 1];
      }
    }

    if (DEBUG)
      console.log(
        `🆔 [DEBUG] groupKey="${group.key}" size=${logs.length} isCWL=${isCWL} round=${round}`
      );

    // Header
    msg += `No.${idx++} ${isCWL ? "(CWL) " : ""}🏰 ${sample.myClan} (${sample.clanTag}) vs ${sample.oppClan} (${sample.oppClanTag})`;
    if (isCWL && round) msg += ` (Round ${round})`;
    msg += "\n";
    ...
    if (isCWL) {
      ...
      const nonLive = logs.filter((l) => !l.isFromLive).slice(-2);
      if (!nonLive.length) {
