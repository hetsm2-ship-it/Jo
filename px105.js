...
HANDLERS.cminfo = async function ({ sock, jid, args, sender, isGroup }) {
  try {
    ...
    function pickBestAttacksForGroup(logs) {
      // group by order if present else use defenderTag
      const map = new Map();
      for (const l of logs) {
        const key = l.order !== undefined && l.order !== null ? `o:${l.order}` : `d:${(l.defenderTag || l.oppName || l.opp || l.defender || "unknown")}`;
      ...
      for (const [k, arr] of map.entries()) {
        // pick best: prefer higher stars, then higher destruction, then latest time
        arr.sort((a, b) => {
      ...
      // sort chosen by order numeric if available, otherwise by time desc (newest first)
      chosen.sort((a, b) => {
