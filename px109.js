...
// Minimal fix: save ended CWL war locally
DB.cwlLogs = DB.cwlLogs || {};
DB.cwlLogs[clanTagUC] = DB.cwlLogs[clanTagUC] || [];

// Check if this war is already saved (by endTime)
if (!DB.cwlLogs[clanTagUC].some(w => w.endTime === endTimeParsed && w.type === `CWL Round ${i + 1}`)) {
  DB.cwlLogs[clanTagUC].push(cwlWars[cwlWars.length - 1]);
}
...
const allWars = [...normalWars, ...cwlWars, ...savedCWL];
    if (!allWars.length) {
