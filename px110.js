...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
      ...
      for (let i = 0; i < rounds.length; i++) {
        for (const warTag of (rounds[i].warTags || [])) {
          ...
          try {
            ...
            cwlWars.push(warData);

            // ✅ Save locally for future use
            DB.cwlLogs = DB.cwlLogs || {};
            DB.cwlLogs[clanTagUC] = DB.cwlLogs[clanTagUC] || [];

            if (!DB.cwlLogs[clanTagUC].some(w => w.endTime === warData.endTime && w.type === warData.type)) {
              DB.cwlLogs[clanTagUC].push(warData);
              console.log(`💾 Saved CWL Round ${i + 1} to DB for future use`);
            }
    ...
    // ---------------- Merge Normal, Live CWL & Saved CWL ----------------
    const savedCWL = DB.cwlLogs?.[clanTagUC] || [];
    let allWars = [...normalWars, ...cwlWars];

    // Merge saved CWL if not already in live fetch
    for (const saved of savedCWL) {
      if (!allWars.some(w => w.type === saved.type && w.endTime === saved.endTime)) {
        allWars.push(saved);
      }
    }
