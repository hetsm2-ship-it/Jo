...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    // ---------------- Merge + Sort ----------------
    const savedCWL = (DB.cwlLogs && DB.cwlLogs[clanTagUC]) ? DB.cwlLogs[clanTagUC] : [];
    console.log("📂 Saved CWL wars found:", savedCWL.length);

    const allWars = [...normalWars, ...savedCWL, ...cwlWars];
    console.log("🟢 Wars before sort:", allWars.map(w => `${w.type} vs ${w.opponent}`));
