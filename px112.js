      ...
      for (let i = 0; i < rounds.length; i++) {
        for (const warTag of (rounds[i].warTags || [])) {
          ...
          try {
            ...
            // ✅ Save ended CWL war to DB (persistent)
            ...
            if (!DB.cwlLogs[clanTagUC].some(w => w.endTime === endTimeParsed && w.type === cwlData.type)) {
              ...
              console.log(`💾 Saved CWL Round ${i + 1} to DB`);
            }
...
// Push normal wars first
if (normalWars?.length) mergedWars.push(...normalWars);

// ✅ Always push saved CWL wars (even if live CWL is 0)
if (Array.isArray(savedCWL) && savedCWL.length) {
  console.log("✅ Appending saved CWL wars:", savedCWL.length);
  mergedWars.push(...savedCWL);
}

// Push live CWL wars if any
if (cwlWars?.length) mergedWars.push(...cwlWars);

if (!mergedWars.length) {
  return await sock.sendMessage(jid, { text: "❌ No war logs found (normal or CWL)." });
}

// Sort newest first
mergedWars.sort((a, b) => (b.endTime || 0) - (a.endTime || 0));
