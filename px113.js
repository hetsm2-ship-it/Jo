...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    // ---------------- Load saved CWL from DB ----------------
    DB.cwlLogs = DB.cwlLogs || {};
    const savedCWL = DB.cwlLogs[clanTagUC] || [];
    console.log(`💾 Saved CWL wars found: ${savedCWL.length}`);
        ...
        for (const warTag of (rounds[i].warTags || [])) {
          ...
          try {
            ...
            const cwlData = {
              ...
              destruction: `${(myClan?.destructionPercentage ?? 0).toFixed(2)}% : ${(enemyClan?.destructionPercentage ?? 0).toFixed(2)}%`,
            ...
            if (!savedCWL.some(w => w.endTime === endTimeParsed && w.type === cwlData.type)) {
              savedCWL.push(cwlData);
            }
    ...
    console.log("📊 Wars before sort:", mergedWars.map(w => `${w.type} vs ${w.opponent}`));
