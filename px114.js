...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    let savedCWL = DB.cwlLogs[clanTagUC] || [];
    console.log(`🗃️ Saved CWL wars found: ${savedCWL.length}`);

    // Fix invalid / missing endTime so they appear in correct order
    const now = Date.now();
    savedCWL.forEach((w, idx) => {
      if (!w.endTime || w.endTime < 1000000000000) {
        w.endTime = now - ((savedCWL.length - idx) * 60000); // distribute evenly
      }
    });
    ...
    // ---------------- Fetch CWL wars (if active season) ----------------
    let cwlWars = [];
    try {
      ...
      for (let i = 0; i < rounds.length; i++) {
        for (const warTag of rounds[i].warTags || []) {
          ...
          try {
            ...
            cwlWars.push(cwlEntry);

            // Save permanently if not already stored
            if (!savedCWL.some(x => x.type === cwlEntry.type && x.endTime === cwlEntry.endTime)) {
              savedCWL.push(cwlEntry);
              DB.cwlLogs[clanTagUC] = savedCWL;
            }
    ...
    // ---------------- Format Output ----------------
    const lines = warsToShow.map((w, idx) => `*No.${idx + 1}*  (${w.type}) ⚔️ vs ${w.opponent} | ${w.result} (${w.stars})`);
