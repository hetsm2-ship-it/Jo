...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
      ...
      for (let i = 0; i < rounds.length; i++) {
        for (const warTag of (rounds[i].warTags || [])) {
          ...
          try {
            ...
            const endTimeParsed = parseCoCTime(war.endTime);
            ...
            cwlWars.push(cwlData);

            // ✅ Save to DB if not already saved
            DB.cwlLogs = DB.cwlLogs || {};
            DB.cwlLogs[clanTagUC] = DB.cwlLogs[clanTagUC] || [];
            if (!DB.cwlLogs[clanTagUC].some(w => w.endTime === endTimeParsed && w.type === cwlData.type)) {
              DB.cwlLogs[clanTagUC].push(cwlData);
            }

            console.log(`   ✅ CWL Round ${i + 1} added vs ${enemyClan?.name}`);
          } catch (e) {
    ...
    // ---------------- Get Saved CWL From DB ----------------
    const savedCWL = DB.cwlLogs?.[clanTagUC] || [];
    console.log(`🗃️ Saved CWL wars found: ${savedCWL.length}`);

    // ---------------- Merge & Fix Ordering ----------------
    const maxNormalEndTime = Math.max(...normalWars.map(w => w.endTime), 0);
    savedCWL.forEach((w, idx) => {
      w.endTime = maxNormalEndTime + (idx + 1) * 60000; // Force CWL newer than normal
    });

    const mergedWars = [...normalWars, ...savedCWL];

    console.log("📊 Wars before sort:", mergedWars.map(w => `${w.type} vs ${w.opponent} @ ${w.endTime}`));

    mergedWars.sort((a, b) => b.endTime - a.endTime);

    // ---------------- Prepare Output ----------------
    const warsToShow = mergedWars.slice(0, 10);
    if (!warsToShow.length) {
      return await sock.sendMessage(jid, { text: "❌ No war logs found (normal or CWL)." });
    }

    const lines = warsToShow.map((w, idx) => 
      `*No.${idx + 1}*  (${w.type}) ⚔️ vs ${w.opponent} | ${w.result} (${w.stars})`
    );
...
setInterval(async () => {
  ...
  try {
    for (const jid in DB.userClans) {
      ...
      const warData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
      if (!warData) {
        console.log(`⚠️ No war data for ${clanTag}`);
        continue;
      }

      // ✅ Agar war inWar ya warEnded dono state me chalega
      if (warData.state !== "inWar" && warData.state !== "warEnded") {
        console.log(`⚠️ Skipped clan ${clanTag} (state: ${warData.state})`);
        continue;
      }
      ...
      for (const m of warData.clan.members) {
        ...
        // Filter old logs (remove only current war logs, keep previous wars)
        let oldLogs = DB.playerWarLogs[playerTag].filter(log => log.warId !== warId);
        let newLogs = [];

        // ✅ Save actual attacks (live + final snapshot)
        for (const atk of attacks) {
          ...
          newLogs.push({
            ...
            unused: false,
            isFromLive: warData.state === "inWar", // ✅ mark as live only if war ongoing
            time: new Date().toISOString()
          });

          console.log(`✅ Prepared ${warData.state === "inWar" ? "LIVE" : "FINAL"} log for ${m.name}: ${atk.stars}⭐ vs ${opponentName}`);
        }

        // ✅ Save unused slots ONLY if warEnded
