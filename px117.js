...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    // -------- 2️⃣ Try Fetch CWL Wars (Only If CWL Active) --------
    let cwlWars = [];
    try {
        ...
        for (let i = 0; i < rounds.length; i++) {
          for (const warTag of (rounds[i].warTags || [])) {
            ...
            try {
              const war = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
              if (!war || !war.state) continue;
              if (war.state !== "warEnded") continue; // sirf khatam huye CWL wars chahiye
              ...
              if (!DB.cwlLogs[clanTagUC].some(x => x.endTime === cwlData.endTime)) {
      ...
      console.log(`ℹ️ No active CWL found (using saved CWL logs only).`);
    }

    // -------- 3️⃣ Load Saved CWL Logs --------
    const savedCWL = DB.cwlLogs?.[clanTagUC] || [];
    console.log(`🗃️ Saved CWL wars found: ${savedCWL.length}`);

    // -------- 4️⃣ Merge & Sort --------
    const mergedWars = [...normalWars, ...savedCWL, ...cwlWars];
