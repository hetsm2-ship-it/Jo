...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    const parseCoCTime = (timeStr) => {
      ...
      try { return new Date(timeStr).getTime(); } catch { return Date.now(); }
    };
    ...
    try {
      ...
      normalWars = (normalWarlog.items || []).map(w => {
        ...
        let result = clanStars > oppStars ? "win" : clanStars < oppStars ? "loss" : "tie";
        return {
          type: "Normal",
    ...
    // Force CWL endTime > latest normal war
    const maxNormalEndTime = Math.max(...normalWars.map(w => w.endTime), 0);
    savedCWL.forEach((w, idx) => {
      if (!w.endTime || w.endTime <= maxNormalEndTime) {
        w.endTime = maxNormalEndTime + (idx + 1) * 60000;
      }
    });
    ...
    if (!warsToShow.length) {
      return await sock.sendMessage(jid, { text: "❌ No war logs found." });
    }
