...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    // ---------------- Normalize Clan Tag ----------------
    let clanTagUC = clanTag.toUpperCase().trim().replace(/O/g, "0");
    ...
    // ---------------- Merge Normal + Saved CWL ----------------
    const allWars = [...normalWars, ...savedCWL];

    // Ensure each war has valid endTime
    allWars.forEach((w, idx) => {
      if (!w.endTime) w.endTime = Date.now() - idx * 60000;
    });

    // Sort descending by endTime
    allWars.sort((a, b) => b.endTime - a.endTime);

    console.log("📊 Wars before slice:", allWars.map(w => `${w.type} vs ${w.opponent} @ ${w.endTime}`));
