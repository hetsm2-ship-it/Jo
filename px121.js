...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
      ...
      for (let i = 0; i < rounds.length; i++) {
        for (const warTag of (rounds[i].warTags || [])) {
          ...
          try {
            ...
            liveCWL.push(cwlData);

            // ✅ Save to DB if not already saved
            DB.cwlLogs = DB.cwlLogs || {};
            DB.cwlLogs[clanTag] = DB.cwlLogs[clanTag] || [];
            if (!DB.cwlLogs[clanTag].some(w => w.endTime === endTimeParsed && w.type === cwlData.type)) {
              DB.cwlLogs[clanTag].push(cwlData);
            }
    ...
    const savedCWL = DB.cwlLogs?.[clanTag] || [];
    ...
    const message = `📜 *Last 10 Wars for ${clanTag}:*\n\n${lines.join("\n")}`;
  ...
  try {
    for (const jid in DB.userClans) {
      ...
      for (const m of warData.clan.members) {
        ...
        const oldLogs = DB.playerWarLogs[playerTag].filter(log => log.warId !== warId);
        const newLogs = [];

        // ✅ Save all actual attacks
        for (const atk of attacks) {
        ...
        // ✅ Save placeholder UNUSED slots (so finalizer can detect 0 attacks)
        for (let order = 1; order <= 2; order++) {
          if (!usedOrders.has(order)) {
            ...
            console.log(`⚪ Placeholder UNUSED created for ${m.name} (Attack ${order})`);
          }
        }
