...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    try {
      ...
      normalWars = (normalWarlog.items || []).filter(w => !w.isCwl).map(w => {
    ...
    let message = `📜 *Last 10 Wars for ${clanTag}:*\n\n`;
  ...
  try {
    for (const jid in DB.userClans) {
      ...
      for (const m of warData.clan.members) {
        ...
        // Filter out old logs of this war (keep others)
        ...
        // ✅ Save placeholder UNUSED slots (TEMP only for tracking)
        for (let order = 1; order <= 2; order++) {
          if (!usedOrders.has(order)) {
            ...
            newLogs.push({
              ...
              defenderTag: null,
              unused: true,
              isTempUnused: true, // 🔥 mark temporary placeholder
              isFromLive: true,
              finalized: false,
              time: new Date().toISOString()
            });
