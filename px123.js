  ...
  try {
    for (const jid in DB.userClans) {
      ...
      for (const m of warData.clan.members) {
        ...
        let playerLogs = DB.playerWarLogs[playerTag].filter(log => log.warId !== warId);

        // 🧹 Also remove old placeholders of this war (we will recreate fresh ones)
        playerLogs = playerLogs.concat(
          DB.playerWarLogs[playerTag].filter(l => l.warId === warId && !l.isTempUnused) // keep real attacks only
        );
        ...
        // ✅ Create placeholder ONLY for missing orders
        for (let order = 1; order <= 2; order++) {
          if (!usedOrders.has(order)) {
            ...
            console.log(`⚪ TEMP UNUSED created for ${m.name} (Attack ${order})`);
          }
        }
