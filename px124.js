...
setInterval(async () => {
  ...
  try {
    for (const jid in DB.userClans) {
      ...
      // Only process if it is a regular clan war and in progress/finished
      if (!warData || (!["inWar", "warEnded", "ended"].includes(warData.state)) || warData.warTag) {
        console.log(`⚠️ Skipped clan ${clanTag} (not a regular war)`);
        continue;
      }

      const warId = `${warData.clan.tag}_${warData.opponent.tag}_${warData.endTime}`;
      const warIsOver = ["warEnded", "ended"].includes(warData.state);

      for (const m of warData.clan.members) {
        const playerTag = m.tag;
        if (!DB.playerWarLogs[playerTag]) DB.playerWarLogs[playerTag] = [];

        // Keep only logs for other wars, and real logs of this war
        let playerLogs = DB.playerWarLogs[playerTag].filter(
          log => log.warId !== warId || (log.warId === warId && !log.isTempUnused)
        );
        ...
        // Always save actual attacks (even during live war)
        for (const atk of attacks) {
          ...
          newLogs.push({
            ...
            unused: false,
            isTempUnused: false,
            finalized: warIsOver, // finalized if war is over
            time: new Date().toISOString(),
          });
          console.log(`✅ LIVE attack saved for ${m.name}: ${atk.stars}⭐ vs ${opponentName}`);
        }

        // Only create placeholders ("unused") if the war is *over*
        if (warIsOver) {
          for (let order = 1; order <= 2; order++) {
            if (!usedOrders.has(order)) {
              ...
              newLogs.push({
                ...
                unused: true,
                isTempUnused: true,
                finalized: true, // finalized after war
                time: new Date().toISOString(),
              });
              console.log(`⚪ FINAL UNUSED created for ${m.name} (Attack ${order})`);
            }
          }
        }

        DB.playerWarLogs[playerTag] = playerLogs.concat(newLogs);

        // Retain only latest 50 logs for this player
    ...
    saveDB(); // Custom function to persist updated DB
  } catch (e) {
