...
// ------------------------- AUTO-SAVE CURRENT NORMAL WAR -------------------------
setInterval(async () => {
  console.log("⏳ Auto warlog save check started...");
  try {
    if (!DB || !DB.userClans) return;
    ...
    for (const jid in DB.userClans) {
      ...
      for (const m of warData.clan.members) {
        ...
        // Remove old TEMP UNUSED of this war, keep only real attacks from previous saves
        let playerLogs = DB.playerWarLogs[playerTag].filter(
          l => l.warId !== warId || (!l.isTempUnused && l.isFromLive)
        );

        const newLogs = [];

        // ---------------- Save actual live attacks ----------------
        for (const atk of attacks) {
          ...
          console.log(`✅ LIVE attack saved for ${m.name}: ${atk.stars}⭐ vs ${oppName}`);
        }

        // ---------------- TEMP UNUSED placeholders (only internal, not for war history) ----------------
        for (let order = 1; order <= 2; order++) {
          if (!usedOrders.has(order)) {
            ...
            newLogs.push({
              ...
              defenderTag: null,
              unused: true,
              isTempUnused: true, // 🔹 clearly mark as placeholder
              isFromLive: true,
              finalized: false,
              time: new Date().toISOString()
            });

            console.log(`⚪ TEMP UNUSED placeholder for ${m.name} (Attack ${order})`);
          }
        }
    ...
    console.error("❌ Auto warlog save error:", err.message);
  }
}, 5 * 60 * 1000);
