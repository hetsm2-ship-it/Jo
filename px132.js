...
async function runNormalWar() {
  if (isNormalWarRunning) return console.log("⏩ Normal war already running, skipping...");
  isNormalWarRunning = true;

  try {
    await loadDB(); // ✅ just refresh global DB, don't re-declare it

    if (!DB.userClans || Object.keys(DB.userClans).length === 0) {
      console.log("⚠️ No clans found in DB, skipping normal war check.");
      return;
    }
    ...
    if (clans.length === 0) {
      console.log("⚠️ No valid clan tags found, skipping normal war.");
      return;
    }
...
async function runCwlWar() {
  if (isCwlWarRunning) return console.log("⏩ CWL finalizer already running, skipping...");
  isCwlWarRunning = true;

  try {
    await loadDB(); // ✅ refresh global DB

    if (!DB.userClans || Object.keys(DB.userClans).length === 0) {
      console.log("⚠️ No clans found in DB, skipping CWL check.");
      return;
    }
    ...
    if (clans.length === 0) {
      console.log("⚠️ No valid clan tags found, skipping CWL war.");
      return;
    }
