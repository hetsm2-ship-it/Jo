      ...
      for (const m of warData.clan.members) {
        ...
        for (const atk of attacks) {
          ...
          newLogs.push({
            ...
            unused: false,
            finalized: false,
            isTempUnused: false, // real attacks
        ...
        // ⚪ TEMP placeholders for live display ONLY (do NOT save in DB)
        const tempUnused = [];
        ...
        // Merge real attacks + temp for live display
        const liveDisplay = [...playerLogs, ...newLogs]; // DB safe
        if (tempUnused.length > 0) {
          // Temp placeholders can be used in messages, but not stored
          liveDisplay.push(...tempUnused.map(u => ({ ...u, isTempUnused: true })));
        }
...
async function runNormalWar() {
  ...
  try {
    await loadDB(); // ✅ just refresh DB, no overwrite
    ...
    if (clans.size === 0) {
      console.log("⚠️ No clans found in DB, skipping normal war.");
      return;
    }
    ...
    for (const clanTag of clans) {
      try {
        const warData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
        if (!warData) {
          console.log(`⚠️ No data for ${clanTag}, skipping...`);
          continue;
        }
...
async function runCwlWar() {
  ...
  try {
    ...
    if (clans.size === 0) {
      console.log("⚠️ No clans found in DB, skipping CWL.");
      return;
    }
    ...
    for (const clanTag of clans) {
      try {
        ...
        if (liveWars.length > 0) {
          console.log(`⏸️ Skipping CWL for ${clanTag}: live wars running ${liveWars.join(", ")}`);
          continue;
        }
...
// --- Scheduler ---
runNormalWar();
setInterval(runNormalWar, 3 * 60 * 1000); // every 3 minutes

runCwlWar();
setInterval(runCwlWar, 6 * 60 * 1000); // every 6 minutes
