...
// === Normal War: Last war history attacks ===

// ⚡ Player ke pure stored logs le lo
...
// get last N non-live logs but dedupe by attack order
const nonLiveAll = logs.filter((l) => !l.isFromLive);
const attacksByOrder = { 1: [], 2: [] };
for (const l of nonLiveAll) {
  const ord = Number(l.order) || (l.attackNumber || 0) || 0;
  if (ord === 1 || ord === 2) {
    attacksByOrder[ord].push(l);
  } else {
    if (!attacksByOrder[1].length) attacksByOrder[1].push(l);
    else if (!attacksByOrder[2].length) attacksByOrder[2].push(l);
  }
}
...
const atk1 = pickBest(attacksByOrder[1]);
const atk2 = pickBest(attacksByOrder[2]);

if (!atk1 && !atk2) {
  msg += "Attack 1 → Not used\nAttack 2 → Not used\n\n";
} else {
  msg += formatAttackLine(false, atk1 || { destructionPercentage: 0, stars: 0 }, 1);
  msg += formatAttackLine(false, atk2 || { destructionPercentage: 0, stars: 0 }, 2);
  msg += "\n";
}
          } else {
...
setInterval(async () => {
  ...
  try {
    for (const jid in DB.userClans) {
      ...
      // ❌ Skip agar CWL ya war active nahi hai
      ...
      for (const m of warData.clan.members) {
        ...
        // ❌ Agar ek bhi attack save nahi hua toh ek hi placeholder banao
        if (attacks.length === 0 &&
            !oldLogs.some(l => l.warId === warId && l.unused)) {
          newLogs.push({
            warKey: `${warId}_${playerTag}_unused`,
            warId,
            clanTag: warData.clan.tag,
            oppClanTag: warData.opponent.tag,
            myClan: warData.clan.name,
          ...
          console.log(`ℹ️ Placeholder saved for ${m.name} (no attacks yet)`);
        }
  ...
  try {
    for (const userKey in DB.userClans) {
      ...
      for (const m of myClan.members) {
        ...
        for (const atk of attacks) {
          ...
          if (atk.defenderTag) {
            try {
              ...
              console.log(`❌ Defender fetch error: ${atk.defenderTag}`);
            }
          }
        ...
        // ❌ Agar bilkul attack nahi hua toh ek hi placeholder banao
        if (attacks.length === 0 &&
            !oldLogs.some(l => l.warId === warId && l.unused)) {
          newLogs.push({
            warKey: `${warId}_${playerTag}_unused`,
          ...
          console.log(`⚪ ${m.name} did not attack (placeholder saved).`);
        }
