  ...
  try {
    ...
    async function fetchCWLData(clanTag) {
      try {
        ...
        console.log("DEBUG: CWL Data fetched:", JSON.stringify(data, null, 2));
        return data;
      } catch (err) {
    ...
    async function fetchLastCWLWarLog(clanTag) {
      try {
        ...
        console.log("DEBUG: Last CWL Warlog fetched:", JSON.stringify(data, null, 2));
        return data;
      } catch (err) {
    ...
    const clanData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}`);
    console.log("DEBUG: Clan Data fetched:", JSON.stringify(clanData, null, 2));

    const normalWarData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
    console.log("DEBUG: Normal War Data fetched:", JSON.stringify(normalWarData, null, 2));
    const normalWarState = normalWarData?.state || "notInWar";

    const cwlData = await fetchCWLData(clanTag);
    const cwlWarState = cwlData?.state || "notInWar";
    console.log("DEBUG: Normal War State:", normalWarState);
    console.log("DEBUG: CWL War State:", cwlWarState);

    let showCWL = false;
    if (normalWarState === "notInWar" && (cwlWarState === "inWar" || cwlWarState === "preparation" || cwlWarState === "warEnded")) {
      showCWL = true;
    }
    if ((normalWarState === "inWar" || normalWarState === "preparation" || normalWarState === "warEnded") &&
        (cwlWarState === "inWar" || cwlWarState === "preparation" || cwlWarState === "warEnded")) {
      showCWL = true;
    }
    ...
    async function sendCWLInfo() {
      if (!cwlData || !cwlData.state || cwlData.state === 'notInWar' ||
          !cwlData.clan || !Array.isArray(cwlData.clan.members)) {
        if (cwlData?.state === 'preparation') {
          return await sock.sendMessage(jid, { text: "ℹ️ CWL war preparation chal rahi hai, par detailed data abhi available nahi hai." });
        }
        return await sock.sendMessage(jid, { text: "⚠️ CWL data unavailable ya clan CWL league me nahi hai." });
      }

      if (!args[0]) {
        let msg = `🏆 *CWL War for ${cwlData.clan.name} (${cwlData.clan.tag}):*\n\n`;
