...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  msg += "The leader hasn't selected this member to be in current war\n";
      ...
      // ---- cwl -----
        ...
        let msg = `📌 *Player War Details for ${player.name} (${player.tag})*\n\n`;
msg += `🏰 ${myClan.name} (${myClan.tag}) vs ${warData.opponent.name} (${warData.opponent.tag})\n\n`;
...
// Fetch opponent clan members (live clan roster)
const opponentClanInfo = await cocFetch(`/clans/${encodeURIComponent(warData.opponent.tag)}`);

// Merge both sources for defender name: warData (for regular war), opponentClanInfo (for swapped/benched)
if (player.attacks?.length) {
  for (const atk of player.attacks) {
    // Try direct warData opponent members first
    let defenderName = '';
    const warRosterDefender = (warData.opponent?.members || []).find(mem =>
      normalizeTag(mem.tag) === normalizeTag(atk.defenderTag)
    );
    if (warRosterDefender?.name) {
      defenderName = warRosterDefender.name;
    } else {
      // Try full opponent clan live roster if not found above
      const liveClanDefender = opponentClanInfo.memberList?.find(
        mem => normalizeTag(mem.tag) === normalizeTag(atk.defenderTag)
      );
      defenderName = liveClanDefender?.name || "Unknown";
    }
