...
// Opponent clan info fetch for fallback (handles bench/swap)
const opponentClanInfo = await cocFetch(`/clans/${encodeURIComponent(warData.opponent.tag)}`);

// Always use IF/ELSE, do not close for-loop before else!
if (player.attacks?.length) {
  for (const atk of player.attacks) {
    let defenderName = "";
    // 1. Try war roster (best for normal war)
    const rosterDefender = (warData.opponent?.members || []).find(mem =>
      normalizeTag(mem.tag) === normalizeTag(atk.defenderTag)
    );
    if (rosterDefender?.name) {
      defenderName = rosterDefender.name;
    } else {
      // 2. Try live clan fallback (for swapped/bench, CWL)
      const liveClanDefender = opponentClanInfo.memberList?.find(mem =>
        normalizeTag(mem.tag) === normalizeTag(atk.defenderTag)
      );
