...
// ---------------------- AUTO-SAVE CURRENT CWL WAR ATTACKS ----------------------
setInterval(async () => {
  console.log("⏳ Auto CWL warlog save check started...");
  try {
    for (const jid in DB.userClans) {
      const clanData = DB.userClans[jid];
      if (!clanData) continue;
      ...
      // Recent active/ended CWL round finder
        ...
        for (const warTag of round.warTags || []) {
          if (warTag && warTag !== '#0') {
            ...
            if (war.state === 'inWar' || war.state === 'warEnded') {
              myClan = (war.clan && tagsEqual(war.clan.tag, clanTag)) ? war.clan
      ...
      if (!cwlWar || !myClan || !Array.isArray(myClan.members)) {
        console.log(`⚠️ Skipped clan ${clanTag} (no active/ended CWL war)`);
        continue;
      }

      // Save attacks for each CWL player (use /players/{defenderTag} for name)
      for (const m of myClan.members) {
        ...
        for (const atk of attacks) {
          ...
          // Defender name by player tag (most reliable)
          ...
          console.log(`✅ Prepared CWL warlog for ${m.name}: ${atk.stars}⭐ vs ${defenderName}`);
        }
        ...
        for (let order = 1; order <= 2; order++) {
          if (!usedOrders.has(order)) {
            ...
            console.log(`⚪ Prepared UNUSED CWL slot for ${m.name} (Attack ${order})`);
          }
        }
    ...
    console.error("Auto CWL warlog save error:", e.message);
  }
