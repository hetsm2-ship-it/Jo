...
// -------------------- DEBUG CWL SNAPSHOT (INWAR ONLY) --------------------
setInterval(async () => {
  console.log("⏳ [CWL-DEBUG] Snapshot save started");
  try {
    for (const jid in DB.userClans) {
      const clanData = DB.userClans[jid];
      if (!clanData) continue;
      ...
      const clanTag = typeof clanData === "string" ? clanData : clanData.clanTag;
      if (!clanTag) continue;
      console.log("➡️ Checking clan:", clanTag);

      // Fetch league group data
      let leagueGroup;
      try {
        leagueGroup = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
        console.log("LeagueGroup fetched", Array.isArray(leagueGroup.rounds) ? `Rounds: ${leagueGroup.rounds.length}` : "No rounds");
      } catch(e) {
        console.log("❌ LeagueGroup fetch error:", e.message);
        continue;
      }
      if (!leagueGroup || !Array.isArray(leagueGroup.rounds) || !leagueGroup.rounds.length) continue;

      let cwlWar = null, myClan = null, warId = '', roundIndex = -1;

      // Only INWAR rounds snapshot
      for (let i = leagueGroup.rounds.length - 1; i >= 0; i--) {
        ...
        for (const warTag of round.warTags) {
          if (warTag && warTag !== '#0') {
            let war;
            try {
              war = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
              console.log(`War fetched: ${warTag} state=${war.state}`);
            } catch(e) {
              console.log("❌ CWL war fetch error:", e.message);
              continue;
            }
            if (war.state === 'inWar') {
              // Side detect for your clan
              myClan =
                (war.clan && tagsEqual(war.clan.tag, clanTag)) ? war.clan :
                (war.opponent && tagsEqual(war.opponent.tag, clanTag)) ? war.opponent : null;
              if (!myClan || !myClan.members) continue;
              cwlWar = war;
              warId = `${war.clan.tag}_${war.opponent.tag}_${war.endTime}`;
              roundIndex = i + 1;
              console.log(`🟢 [CWL] INWAR found: ${warId} | members: ${myClan.members.length} | round: ${roundIndex}`);
              break;
            } else {
              console.log(`[SKIP] warTag ${warTag} state=${war.state}`);
            }
          }
        }
        if (cwlWar) break;
      ...
      if (!cwlWar || !myClan || !Array.isArray(myClan.members)) {
        console.log(`⚠️ Skipped ${clanTag} (no INWAR CWL found)`);
        continue;
      }
      ...
      for (const m of myClan.members) {
        ...
        const attacks = m.attacks || [];
        console.log(`🔎 Player=${m.name} (${playerTag}) | attacks=${attacks.length}`);
        ...
        for (const atk of attacks) {
          ...
          // Defender name by tag
          let defenderName = "Unknown";
          if (atk.defenderTag) {
            try {
              const playerData = await cocFetch(`/players/${encodeURIComponent(atk.defenderTag)}`);
              defenderName = playerData?.name || "Unknown";
              console.log(`✅ Defender: ${atk.defenderTag} -> ${defenderName}`);
            } catch(e) {
              console.log(`❌ Defender fetch failed for ${atk.defenderTag}: ${e.message}`);
            }
          }
          ...
          console.log(`📝 Snapshot log (CWL): ${m.name} vs ${defenderName} | stars=${atk.stars} destruct=${atk.destructionPercentage}`);
        }

        // If no attack, save unused
        if (!attacks.length) {
          ...
          console.log(`⚪ Unused CWL log: ${m.name} (attack 1)`);
        }
    ...
    saveDB();
    console.log("💾 [CWL] Snapshot logs saved");
  } catch (e) {
    console.error("CWL snapshot save error:", e.message);
  }
}, 6 * 60 * 1000); // Every 6 mins
