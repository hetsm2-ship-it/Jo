...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    // DEBUG: Helper function to dump JSON in terminal
    function debugDump(label, obj) {
      try { console.log("DEBUG:", label, JSON.stringify(obj, null, 2)); } catch {}
    }
    ...
    async function fetchLastCWLWarLog(clanTag) {
      try {
        const data = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/warlog?limit=10`);
        debugDump("CWL WarLog", data);
        return data;
      } catch (err) {
        console.error("DEBUG: Error fetching last CWL warlog:", err);
        return null;
      }
    }
    ...
    function detectCWLState(cwlData) {
      ...
      // Fallback guess: Agar rounds me warTags aa chuke hain but koi war/attack nahi huye, to "preparation"
      if (Array.isArray(cwlData.rounds) && cwlData.rounds.length && cwlData.rounds.some(r => Array.isArray(r.warTags) && r.warTags.length)) {
        return "preparation";
      }
      return "notInWar";
    }

    function getCWLUsedAttacksLastWar(playerTag, warlog) {
      if (!warlog?.items?.length) return 0;
      for (const war of warlog.items) {
        if (war.leagueGroup && war.clan && Array.isArray(war.clan.members)) {
          const player = war.clan.members.find(m => m.tag === playerTag);
          if (player) return player.attacks?.length > 0 ? 1 : 0;
        }
      }
      return 0;
    }

    function cwlUsedAttacksCurrent(playerTag, cwlData) {
      if (!cwlData?.rounds) return 0;
      const allMembers = cwlData.rounds.flatMap(r => (r.clan && Array.isArray(r.clan.members)) ? r.clan.members : []);
      const player = allMembers.find(m => m.tag === playerTag);
      return player?.attacks?.length > 0 ? 1 : 0;
    }
    ...
    // Show CWL only if normal war not in war & cwl in prep/inWar/warEnded
    let showCWL = false;
    ...
    // CWL INFO BLOCK (now robust)
    async function sendCWLInfo() {
      // Agar member list ya rounds available nahi bhi ho, fir bhi state ke hisaab se feedback do
      ...
      if (!args[0]) {
        ...
        if (cwlData.clan && Array.isArray(cwlData.clan.members)) {
          if (cwlWarState === "inWar") {
            for (const m of cwlData.clan.members) {
              const attacksUsed = cwlUsedAttacksCurrent(m.tag, cwlData);
              msg += `${m.name} ${m.tag} - ${attacksUsed === 1 ? "Used attack in current CWL" : "Unused attack in current CWL"}\n`;
            }
          }
          else if (cwlWarState === "warEnded" || cwlWarState === "preparation") {
            const warLog = await fetchLastCWLWarLog(clanTag);
            if (!warLog) {
              msg += "⚠️ CWL last war data unavailable\n";
            } else {
              for (const m of cwlData.clan.members) {
                const usedCount = getCWLUsedAttacksLastWar(m.tag, warLog);
                msg += `${m.name} ${m.tag} - ${usedCount === 1 ? "Used attack in last CWL" : "Unused attack in last CWL"}\n`;
              }
            }
          } else {
            msg += "(No active CWL war now)";
          }
        } else {
          // clan.members nahi mila, CWL prep/war phase show karo
          msg += `ℹ️ CWL war "${cwlWarState}" chal rahi hai, lekin members data abhi available nahi hai.\n`;
        }
        return await sock.sendMessage(jid, { text: msg });
      } else {
        const playerNo = parseInt(args[0], 10);
        ...
        if (cwlData.clan && Array.isArray(cwlData.clan.members)) {
          if (cwlWarState === "inWar") {
            msg += `🏆 *Current CWL Player Attack:*\n`;
            const allMembers = cwlData?.rounds?.flatMap(r => (r.clan && Array.isArray(r.clan.members)) ? r.clan.members : []);
            const playerCWL = allMembers.find(mem => mem.tag === player.tag);
            const usedAttack = playerCWL?.attacks?.find(a => a.destructionPercentage > 0);
            msg += usedAttack ?
              `Attack → vs ${usedAttack.defenderTag} → ${usedAttack.destructionPercentage}% (${usedAttack.stars}⭐)\n` :
            ...
            if (!warLog) {
              ...
              const usedCount = getCWLUsedAttacksLastWar(player.tag, warLog);
              msg += usedCount === 1 ? "Used attack in last CWL\n" : "Unused attack in last CWL\n";
            }
          } else {
            msg += "(No active CWL war now)";
          }
        } else {
          msg += `ℹ️ CWL war "${cwlWarState}" chal rahi hai, lekin player members data abhi available nahi hai.\n`;
        }
        return await sock.sendMessage(jid, { text: msg });
      }
    }
