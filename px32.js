...
async function runGetWarLogs() {
  if (isGetWarLogsRunning) return;
  isGetWarLogsRunning = true;
  try {
    // 1. Run getlogwar.js first (wait for finish)
    await new Promise((resolve, reject) => {
    ...
    // 2. Run cwlgetwar.js without any extra condition (wait for finish)
    await new Promise((resolve, reject) => {
    ...
    console.error(`Error while processing clan: ${e.message}`);
  }
