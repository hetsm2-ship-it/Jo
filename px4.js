...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    function detectCWLState(cwlData) {
      ...
      // If rounds w/ warTags present, assume prepping
    ...
    // CWL INFO BLOCK (members length > 0 check)
    async function sendCWLInfo() {
      ...
      if (!args[0]) {
        ...
        // Fixed: Only if members array exists and has actual players!
        if (cwlData.clan && Array.isArray(cwlData.clan.members) && cwlData.clan.members.length > 0) {
          ...
          msg += `ℹ️ CWL war "${cwlWarState}" chal rahi hai, lekin members data abhi available nahi hai (empty array).\n`;
        }
        return await sock.sendMessage(jid, { text: msg });
      } else {
        const playerNo = parseInt(args[0], 10);
        ...
        if (cwlData.clan && Array.isArray(cwlData.clan.members) && cwlData.clan.members.length > 0) {
          ...
          msg += `ℹ️ CWL war "${cwlWarState}" chal rahi hai, lekin player members data abhi available nahi hai (empty array).\n`;
        }
        return await sock.sendMessage(jid, { text: msg });
      }
    }
