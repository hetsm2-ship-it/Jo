...
// ------------------------- 🚀 Launch -------------------------
async function main() {
  await startBot(); // sock ko globalSock me assign kar raha hai startBot
  const fs = require("fs");
}
...
const loadDB = async () => {
  try {
    await fsp.access("db.json"); // check if file exists
    const data = JSON.parse(await fsp.readFile("db.json", "utf8"));
...
const saveDB = async () => {
  try {
    await fsp.writeFile("db.json", JSON.stringify(DB, null, 2));
...
// =============================================================================
// Clash of Clans War Log & Notifications Runner
// =============================================================================
...
// -----------------------------------------------------------------------------
// Helper: run child process sequentially
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Run get war logs for all clans
// -----------------------------------------------------------------------------
async function runGetWarLogs() {
  if (isGetWarLogsRunning) {
    console.log("⚠️ runGetWarLogs already running. Skipping this run.");
    return;
  }
  ...
  try {
    ...
    for (const clanTag of clans) {
      try {
        const warData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
        const state = warData?.state || "unknown";

        if (!warData || state === "inWar") {
          console.log(`⏩ Skipping finalize for clan ${clanTag}, state: ${state}`);
          continue;
        }

        console.log(`🏁 Finalizing war for clan ${clanTag}, state: ${state}`);
        ...
        console.error(`❌ Error processing clan ${clanTag}:`, err.message);
      }
    }
  } catch (err) {
    console.error("❌ runGetWarLogs error:", err.message);
  } finally {
...
async function main() {
  ...
  // Run every 2 minutes
  setInterval(runGetWarLogs, 2 * 60 * 1000);

  // War notifications every 1 minute
  setInterval(async () => {
    try {
      if (globalSock) {
        await handleWarNotifications(globalSock);
      }
    } catch (e) {
