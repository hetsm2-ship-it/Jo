...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    const key = isGroup ? sender : jid;
    const clanTag = DB.userClans[key];
    if (!clanTag) return await sock.sendMessage(jid, { text: "❌ Clan tag not set. Use setclan first." });

    // Debug dump
    function debugDump(label, obj) {
      try { console.log("DEBUG:", label, JSON.stringify(obj, null, 2)); } catch (e) {}
    }

    // CWL league group
    async function fetchCWLData(clanTag) {
      try {
        const data = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
        debugDump("CWL Group Data", data);
        return data;
      } catch (err) {
        console.error("DEBUG: Error fetching CWL data:", err);
        return null;
      }
    }

    // Get CWL warTag (active war) from rounds array
    function getActiveCwlWarTag(cwlData) {
      if (!cwlData?.rounds || !Array.isArray(cwlData.rounds)) return null;
      // try to pick latest war with valid tag (recent war first)
      for (let i = cwlData.rounds.length - 1; i >= 0; i--) {
        const warTags = cwlData.rounds[i]?.warTags;
        if (Array.isArray(warTags)) {
          // Remove '#0' (no war)
          const valid = warTags.filter(wt => wt && wt !== "#0");
          if (valid.length > 0) return valid[0]; // take first active war in round
        }
      }
      return null;
    }

    // Get CWL war by warTag (returns null if not found)
    async function fetchCWLWarByTag(warTag) {
      if (!warTag || warTag === "#0") return null;
      try {
        const data = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
        debugDump("CWL Actual War Data", data);
        return data;
      } catch (err) {
        console.error("DEBUG: Error fetching CWL war by tag:", err);
        return null;
      }
    }
    ...
    // Normal war logic
    const clanData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}`);
    debugDump("Clan Data", clanData);
    const normalWarData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
    debugDump("Normal War Data", normalWarData);
    const normalWarState = normalWarData?.state || "notInWar";

    // Find CWL state & war data
    const cwlData = await fetchCWLData(clanTag);

    // Detect state just for logic gating
    function detectCWLState(cwlData) {
      if (!cwlData) return "notInWar";
      if (cwlData.state) return cwlData.state;
      if (Array.isArray(cwlData.rounds) && cwlData.rounds.some(r => Array.isArray(r.warTags) && r.warTags.length))
        return "preparation";
      return "notInWar";
    }
    const cwlWarState = detectCWLState(cwlData);
    debugDump("CWL Detected State", { cwlWarState });

    // Show CWL if normalWar not active, CWL active
    let showCWL = false;
    if (normalWarState === "notInWar" && (cwlWarState === "inWar" || cwlWarState === "preparation" || cwlWarState === "warEnded")) showCWL = true;
    if ((normalWarState === "inWar" || normalWarState === "preparation" || normalWarState === "warEnded") &&
        (cwlWarState === "inWar" || cwlWarState === "preparation" || cwlWarState === "warEnded")) showCWL = true;

    // =========== MAIN CWL INFO BLOCK (with members from actual round) ==========
    async function sendCWLInfo() {
      if (!cwlData || !cwlWarState || cwlWarState === 'notInWar') {
        return await sock.sendMessage(jid, { text: "⚠️ CWL data unavailable ya clan CWL league me nahi hai." });
      }
      // Fetch actual cwl active round war members
      const currentWarTag = getActiveCwlWarTag(cwlData);
      let cwlActualWar = null;
      if (currentWarTag) cwlActualWar = await fetchCWLWarByTag(currentWarTag);

      // If actual CWL war found and clan + members exist, show those
      function findMyClanAndMembers(war, actualTag) {
        if (war && war.clan && war.clan.tag === actualTag) return war.clan;
        if (war && war.opponent && war.opponent.tag === actualTag) return war.opponent;
        return null;
      }
      // Pick the "side" of your clan in war
      const myCWLClanObj = cwlActualWar ? findMyClanAndMembers(cwlActualWar, clanTag) : null;

      // CLAN LISTING
      if (!args[0]) {
        let msg = `🏆 *CWL War State: ${cwlWarState}*\n`;
        // If actual war and members available use that
        if (myCWLClanObj && Array.isArray(myCWLClanObj.members) && myCWLClanObj.members.length > 0) {
          for (const m of myCWLClanObj.members) {
            const usedAttack = m.attacks?.length ? 1 : 0;
            msg += `${m.name} ${m.tag} - ${usedAttack === 1 ? "Used attack in current CWL" : "Unused attack in current CWL"}\n`;
          }
        }
        else if (cwlData.clan && Array.isArray(cwlData.clan.members) && cwlData.clan.members.length > 0) {
          // fallback (should not generally occur unless very first prep day)
          msg += `🔹 [Registered members only — actual war roster data unavailable]\n`;
          for (const m of cwlData.clan.members) {
            msg += `${m.name} ${m.tag}\n`;
          }
        } else {
          msg += `ℹ️ CWL war "${cwlWarState}" chal rahi hai, lekin war ke members data abhi available nahi hai.`;
        }
        return await sock.sendMessage(jid, { text: msg });
      }
      // PLAYER SPECIFIC
        ...
        let msg = `📌 *Player CWL Details for ${player.name}*\n\n`;
        if (myCWLClanObj && Array.isArray(myCWLClanObj.members) && myCWLClanObj.members.length > 0) {
          const member = myCWLClanObj.members.find(m => m.tag === player.tag);
          if (member) {
            const usedAttack = member.attacks?.length ? 1 : 0;
            msg += usedAttack
              ? `Attack → vs ${member.attacks[0].defenderTag} → ${member.attacks[0].destructionPercentage}% (${member.attacks[0].stars}⭐)\n`
              : "No attack used in current CWL yet.\n";
          } else {
            msg += "Not in this round's war roster.\n";
          }
        }
        else if (cwlData.clan && Array.isArray(cwlData.clan.members) && cwlData.clan.members.length > 0) {
          const regMember = cwlData.clan.members.find(m => m.tag === player.tag);
          msg += regMember
            ? "CWL Member (no active attack data available for current war yet).\n"
            : "Not registered in CWL this season.\n";
        } else {
          msg += `ℹ️ CWL war "${cwlWarState}" chal rahi hai, lekin war ke player data abhi available nahi hai.\n`;
        }
        return await sock.sendMessage(jid, { text: msg });
      }
    }

    // =========== NORMAL WAR INFO BLOCK =============
    async function sendNormalWarInfo() {
      if (!args[0]) {
        let msg = `🏰 *Clan Members for ${clanData?.name || "?"} (${clanData?.tag || "?"}):*\n\n`;
        for (const [i, m] of (clanData?.memberList || []).entries()) {
          msg += `${i + 1}. ${m.name} ${m.tag}\n`;
          let status = "";
          if (normalWarState === "inWar") {
            const wm = normalWarData?.clan?.members?.find(x => x.tag === m.tag);
            status = !wm ? "(not in current war)" :
              wm?.attacks?.length === 0 ? "(unused attacks in current war)" :
              wm?.attacks?.length === 1 ? "(used 1 attack in current war)" :
              "(used 2 attacks in current war)";
          }
          else if (normalWarState === "warEnded" || normalWarState === "preparation") {
            ...
            if (!logs.length) {
              ...
              status =
                lastLogs.length === 0 ? "(not in last war)" :
                usedCount === 0 ? "(unused attacks in last war)" :
        ...
        let warLogs = (DB.playerWarLogs?.[player.tag] || []).filter(l => !l.isFromLive).slice(-10).reverse();
        let msg = `📌 *Player War Details for ${player.name}*\n\n`;

        if (normalWarState === "inWar") {
          msg += `🏰 Clan: ${normalWarData?.clan?.name || "?"} (${normalWarData?.clan?.tag || "?"}) vs ${normalWarData?.opponent?.name || "?"} (${normalWarData?.opponent?.tag || "?"})\n\n`;
          const wm = normalWarData?.clan?.members?.find(x => x.tag === player.tag);
          if (wm) {
            msg += `🔥 *Current Live War Attacks:*\n`;
            msg += wm.attacks?.[0]
              ? `Attack 1 → vs ${normalWarData?.opponent?.members?.find(o => o.tag === wm.attacks[0].defenderTag)?.name || "Unknown"} → ${wm.attacks[0].destructionPercentage}% (${wm.attacks[0].stars}⭐)\n`
              : `Attack 1 → Unused\n`;
            msg += wm.attacks?.[1]
              ? `Attack 2 → vs ${normalWarData?.opponent?.members?.find(o => o.tag === wm.attacks[1].defenderTag)?.name || "Unknown"} → ${wm.attacks[1].destructionPercentage}% (${wm.attacks[1].stars}⭐)\n`
              : `Attack 2 → Unused\n`;
            msg += "\n";
          } else {
          ...
          if (!logs.length) msg += ` 🥲 The player record is unavailable because he wasn't in last war\n\n`;
          else {
            const lastLogs = logs.filter(l => !l.isFromLive).slice(-2);
            if (lastLogs.length === 0) msg += `1. The user wasn't attacked in last war\n\n`;
          }
        }
        if (warLogs.length) {
          msg += `📜 *War History:*\n`;
          ...
          for (const [warKey, logs] of Object.entries(grouped)) {
            ...
            if (lastTwo.length === 0) {
              ...
              if (atk1.destructionPercentage === 0 && atk1.stars === 0) msg += `Attack 1 → Not used\n`;
              else msg += `Attack 1 → vs ${atk1.oppName} → ${atk1.destructionPercentage}% (${atk1.stars}⭐)\n`;
              msg += `Attack 2 → Not used\n`;
            } else {
              lastTwo.forEach((atk, i) => {
                if (atk.destructionPercentage === 0 && atk.stars === 0) msg += `Attack ${i + 1} → Not used\n`;
                else msg += `Attack ${i + 1} → vs ${atk.oppName} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`;
              });
            }
            msg += "\n";
          }
    ...
    if (showCWL) {
      return await sendCWLInfo();
    } else {
      return await sendNormalWarInfo();
    }
  } catch (err) {
