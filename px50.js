  ...
  if (leagueGroup && Array.isArray(leagueGroup.rounds)) {
    for (const round of leagueGroup.rounds) {
      ...
      for (const warTag of round.warTags) {
        ...
        const cwlWar = await cocFetch(`/warleagues/wars/${encodeURIComponent(warTag)}`);
...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    const key = isGroup ? sender : jid;
    const clanTag = (DB.userClans && DB.userClans[key]);
    if (!clanTag) {
      return await sock.sendMessage(jid, { 
        text: `❌ Clan tag not set. Use ${CONFIG.COMMAND_PREFIX}setclan #CLAN_TAG first.` 
      });
    }
    const clanTagUC = clanTag.toUpperCase().startsWith('#') ? clanTag.toUpperCase() : '#' + clanTag.toUpperCase();

    // Fetch normal warlog
    const normalWarlog = await cocFetch(`/clans/${encodeURIComponent(clanTagUC)}/warlog`);
    const normalWars = (normalWarlog.items || []).map(w => ({
      isCwl: false,
      war: w,
      endTime: new Date(w.endTime).getTime()
    }));

    // Fetch CWL warlog rounds
    let cwlWars = [];
    try {
      const leagueGroup = await cocFetch(`/clans/${encodeURIComponent(clanTagUC)}/currentwar/leaguegroup`);
      if (leagueGroup?.rounds) {
        for (let i = 0; i < leagueGroup.rounds.length; i++) {
          ...
          for (const warTag of round.warTags) {
            ...
            try {
              const war = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
              // Check if this clan is involved
              if (war && tagsEqual(war.clan.tag, clanTagUC) || tagsEqual(war.opponent.tag, clanTagUC)) {
                cwlWars.push({
                  isCwl: true,
                  roundNumber: i + 1,
                  war,
                  endTime: new Date(war.endTime).getTime()
                });
              }
            } catch {
              // skip invalid war tags
            }
          }
        }
      }
    } catch {
      // ignore errors in fetching cwl
    }

    // Merge both arrays
    const allWars = normalWars.concat(cwlWars);
    ...
    const lines = warsToShow.map((entry, idx) => {
      ...
      if (entry.isCwl) {
        return `*No.${idx + 1}*  (CWL) Round ${entry.roundNumber} ⚔️ vs ${opponentName} | Result: ${result} (${clanStars}⭐ : ${oppStars}⭐)`;
      } else {
        return `*No.${idx + 1}*  ⚔️ vs ${opponentName} | Result: ${result} (${clanStars}⭐ : ${oppStars}⭐)`;
      }
    });

    const clanName = normalWarlog?.items?.[0]?.clan?.name || clanTagUC;
        ...
        if (leagueGroup && Array.isArray(leagueGroup.rounds)) {
          for (let i = leagueGroup.rounds.length - 1; i >= 0; i--) {
            ...
            for (const warTag of round.warTags) {
              ...
              const cwlWar = await cocFetch(`/clanleagues/wars/${encodeURIComponent(warTag)}`);
