...
async function updateAttendanceLogs(clanTag) {
  try {
    ...
    const normalInWar = liveWar.state === "inWar";
let cwlInWar = false;
...
try {
  ...
  console.error('CWL attendance fetch failed:', error.message);
}
// --- End CWL attendance ---
...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    const clanTagUC = clanTag.startsWith("#")
      ? clanTag.toUpperCase()
      : "#" + clanTag.toUpperCase();
    ...
    let normalWars = [];
    try {
      const normalWarlog = await cocFetch(
        `/clans/${encodeURIComponent(clanTagUC)}/warlog`
      );
      normalWars =
        (normalWarlog?.items || []).map((w) => ({
          isCwl: false,
          war: w,
          endTime: w.endTime ? new Date(w.endTime).getTime() : Date.now(),
        })) || [];
    } catch (err) {
      console.error("Normal warlog fetch failed:", err.message);
    }

    // --- CWL warlog fetch ---
    let cwlWars = [];
    try {
      // 1st try currentwar/leaguegroup (agar CWL hi current war hai)
      let leagueGroup = await cocFetch(
        `/clans/${encodeURIComponent(clanTagUC)}/currentwar/leaguegroup`
      );

      // fallback to leaguegroup (hamisha available hota hai jab clan CWL me hai)
      if (!leagueGroup?.rounds) {
        leagueGroup = await cocFetch(
          `/clans/${encodeURIComponent(clanTagUC)}/leaguegroup`
        );
      }

      if (Array.isArray(leagueGroup?.rounds)) {
        for (let i = 0; i < leagueGroup.rounds.length; i++) {
          for (const warTag of leagueGroup.rounds[i].warTags || []) {
            if (!warTag || warTag === "#0") continue;

            try {
              const war = await cocFetch(
                `/clanwarleagues/wars/${encodeURIComponent(warTag)}`
              );

              if (
                war &&
                (tagsEqual(war.clan?.tag, clanTagUC) ||
                  tagsEqual(war.opponent?.tag, clanTagUC))
              ) {
                cwlWars.push({
                  isCwl: true,
                  roundNumber: i + 1,
              ...
              console.error("CWL war fetch failed:", err.message);
            }
          }
        }
      }
    } catch (err) {
      console.error("CWL group fetch failed:", err.message);
    }
    ...
    if (!allWars.length) {
      return await sock.sendMessage(jid, {
        text: "❌ No war logs found (normal or CWL).",
      });
    }
    ...
    // --- Format message ---
    const lines = warsToShow.map((entry, idx) => {
      const w = entry.war;
      const opponentName = w.opponent?.name || "Unknown";
      const state = w.state || "unknown";
      const result =
        w.result || (w.state === "inWar" ? "in progress" : "pending");
      const clanStars = w.clan?.stars ?? 0;
      const oppStars = w.opponent?.stars ?? 0;

      if (entry.isCwl) {
        return `*No.${idx + 1}*  (CWL Round ${entry.roundNumber}) ⚔️ vs ${opponentName} | ${state} | ${result} (${clanStars}⭐ : ${oppStars}⭐)`;
      } else {
        return `*No.${idx + 1}*  ⚔️ vs ${opponentName} | ${state} | ${result} (${clanStars}⭐ : ${oppStars}⭐)`;
      }
    });

    const clanName =
      normalWars?.[0]?.war?.clan?.name || cwlWars?.[0]?.war?.clan?.name || clanTagUC;

    const message =
      `📜 *Last 10 Wars for ${clanName}:*\n\n` + lines.join("\n");

    await sock.sendMessage(jid, { text: message });
  } catch (e) {
    console.error("Warlog handler failed:", e.message);
    await sock.sendMessage(jid, {
