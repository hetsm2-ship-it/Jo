...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    try {
      ...
      console.error("CWL fetch failed:", err.message);
    }
    ...
    const lines = warsToShow.map((entry, idx) => {
      ...
      if (entry.type === "normal") {
        ...
        return `*No.${idx + 1}*  (Normal) ⚔️ vs ${opponentName} | ${result} (${clanStars}⭐ : ${oppStars}⭐)`;
      }

      if (entry.type === "cwl") {
        let resultText = "";
        if (w.state === "inWar") resultText = "⏳ in progress";
        else if (w.state === "preparation") resultText = "🛡️ preparation";
        else if (w.state === "warEnded") resultText = w.result || "ended";
        else resultText = "unknown";

        return `*No.${idx + 1}*  (CWL Round ${entry.round}) ⚔️ vs ${opponentName} | ${resultText} (${clanStars}⭐ : ${oppStars}⭐)`;
      }

      return `*No.${idx + 1}*  ⚔️ vs ${opponentName}`;
    });

    const clanName =
      normalWars?.[0]?.war?.clan?.name ||
      cwlWars?.[0]?.war?.clan?.name ||
      clanTagUC;
