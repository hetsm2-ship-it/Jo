  ...
  try {
    ...
    try {
      ...
      normalWars = (normalWarlog?.items || []).map((w) => ({
        type: "normal",
        war: w,
        endTime: w.endTime ? new Date(w.endTime).getTime() : 0,
      }));
    } catch (err) {
    ...
    let cwlWars = [];
    try {
      const leagueGroup = await cocFetch(
    ...
    const lines = warsToShow.map((entry, idx) => {
      ...
      if (entry.type === "cwl") {
        const result = w.result || "ended";
        return `*No.${idx + 1}*  (CWL Round ${entry.round}) ⚔️ vs ${opponentName} | ${result} (${clanStars}⭐ : ${oppStars}⭐)`;
      }
