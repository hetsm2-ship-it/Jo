...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    let cwlWars = [];
    try {
      const leagueGroup = await cocFetch(`/clans/${encodeURIComponent(clanTagUC)}/leaguegroup`);
              ...
              if (
                ...
                if (war.state === "warEnded") {
                  cwlWars.push({
                    ...
                    round: i + 1,
                    war,
                    endTime: war.endTime ? new Date(war.endTime).getTime() : 0
                  });
                }
              }
            } catch {}
    ...
    const lines = warsToShow.map((entry, idx) => {
      const w = entry.war;
      const opp = w.opponent?.name || "Unknown";
      ...
      if (entry.type === "normal") {
        return `*No.${idx + 1}*  (Normal) ⚔️ vs ${opp} | ${result} (${clanStars}⭐ : ${oppStars}⭐)`;
      } else {
        return `*No.${idx + 1}*  (CWL Round ${entry.round}) ⚔️ vs ${opp} | ${result} (${clanStars}⭐ : ${oppStars}⭐)`;
      }
    });
