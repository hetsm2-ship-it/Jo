...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    const clanTagUC = clanTag.startsWith('#') ? clanTag.toUpperCase() : '#' + clanTag.toUpperCase();
    ...
    try {
      ...
      normalWars = (normalWarlog.items || []).map(w => {
        ...
        const result = w.result || (clanStars > oppStars ? "win" : clanStars < oppStars ? "loss" : "tie");
        return {
          type: "Normal",
      ...
      console.log("⚠️ Normal warlog fetch failed:", e.message);
    }
        ...
        for (let i = 0; i < league.rounds.length; i++) {
          ...
          for (const warTag of (round.warTags || [])) {
            ...
            try {
              const war = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
              if (!war || !war.state) continue;
              if (!(tagsEqual(war.clan?.tag, clanTagUC) || tagsEqual(war.opponent?.tag, clanTagUC))) continue;

              const clanStars = war.clan?.stars ?? 0;
              const oppStars = war.opponent?.stars ?? 0;
              const result = war.result || (clanStars > oppStars ? "win" : clanStars < oppStars ? "loss" : "tie");

              cwlWars.push({
                type: `CWL Round ${i + 1}`,
                opponent: war.opponent?.name || "Unknown",
                result,
                stars: `${clanStars}⭐ : ${oppStars}⭐`,
                endTime: war.endTime ? new Date(war.endTime).getTime() : Date.now()
              });
            } catch { /* skip invalid war */ }
          }
        }
      }
    } catch (e) {
      console.log("⚠️ CWL warlog fetch failed:", e.message);
    }
    ...
    const lines = warsToShow.map((entry, idx) => {
      return `*No.${idx + 1}*  (${entry.type}) ⚔️ vs ${entry.opponent} | ${entry.result} (${entry.stars})`;
    });

    const clanName = normalWars[0]?.clanName || clanTagUC;
    const message = `📜 *Last 10 Wars for ${clanName}:*\n\n` + lines.join("\n");

    await sock.sendMessage(jid, { text: message });

  } catch (e) {
    console.error("❌ Warlog handler error:", e);
