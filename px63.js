...
// ------------------------- 🧮 Attendance Logs System (Full Recode) -------------------------

// Auto-update every 90 seconds (1.5 min)
setInterval(async () => {
  try {
...
async function updateAttendanceLogs(clanTag) {
  try {
    ...
    let inWarActive = false; // track if any war is active
      ...
      if (leagueGroup?.rounds?.length) {
        for (const round of leagueGroup.rounds) {
          for (const warTag of round.warTags || []) {
            ...
            if (!cwlWar || cwlWar.state !== "inWar") continue; // only if inWar

            inWarActive = true;

            // check which side is my clan
            const isClanSide = tagsEqual(cwlWar.clan?.tag, clanTag);
    ...
    DB.attendanceLogs[clanTag].records = Array.isArray(DB.attendanceLogs[clanTag].records) 
      ...
      : [];
    DB.attendanceLogs[clanTag].lastSnapshot = DB.attendanceLogs[clanTag].lastSnapshot || { lastMembers: [] };
    ...
    DB.attendanceLogs[clanTag].records = DB.attendanceLogs[clanTag].records.filter(r => r.timestamp >= oneMonthAgo);
