...
// ------------------------- 🧮 Attendance Logs System (DEBUG Version) -------------------------

// Auto-update every 90 sec
setInterval(async () => {
  try {
...
async function updateAttendanceLogs(clanTag) {
  try {
    ...
    console.log(`\n🔎 Checking attendance for clan: ${clanTag}`);

    // ---------------- Normal War ----------------
    const liveWar = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
    console.log("📡 currentwar state:", liveWar?.state);
    ...
    if (liveWar?.state === "inWar" && liveWar.clan?.members) {
      ...
      console.log(`⚔️ Normal war active: ${presentPlayers.size} present so far`);
    }

    // ---------------- CWL Wars ----------------
    try {
      const leagueGroup = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
      console.log("📡 leagueGroup rounds:", leagueGroup?.rounds?.length);
      ...
      if (leagueGroup?.rounds?.length) {
        for (const round of leagueGroup.rounds) {
          for (const warTag of round.warTags || []) {
            ...
            console.log("➡️ Checking CWL warTag:", warTag);
            const cwlWar = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
            console.log("   ↪️ cwlWar state:", cwlWar?.state);

            if (!cwlWar || cwlWar.state !== "inWar") continue;

            inWarActive = true;

            const isClanSide = cwlWar.clan?.tag?.toUpperCase() === clanTag.toUpperCase();
            const members = isClanSide ? (cwlWar.clan?.members || []) : (cwlWar.opponent?.members || []);
            console.log(`   👥 Members in this CWL war: ${members.length}`);

            for (const member of members) {
              if (member.attacks?.length > 0) {
                presentPlayers.add(member.tag);
              }
            }
            console.log(`⚔️ CWL war active: ${presentPlayers.size} present so far`);
          }
        }
      }
    } catch (err) {
    ...
    console.log("👥 Total clan members fetched:", clanMembers?.items?.length);
    ...
    console.log(`📊 Attendance calc => Present: ${presentCount}, Absent: ${absentCount}, Total: ${total}`);
...
async function attendance({ sock, jid, args, sender, isGroup }) {
  try {
    ...
    if (!logs?.lastSnapshot) {
      return await sock.sendMessage(jid, {
        text: "📭 Attendance logs not ready yet. They update automatically every 15 mins (only during wars)."
      });
    }
    ...
    if (latest.joined?.length) rep += `\n🆕 Joined: ${latest.joined.join(", ")}`;
    if (latest.leaved?.length) rep += `\n👋 Left: ${latest.leaved.join(", ")}`;
