...
// ------------------------- 🧮 Attendance Logs System (FINAL PATCH) -------------------------

// ✅ Safe DB Save (prevent race conditions)
let isSaving = false;
...
// Auto-update every 90 sec (ONLY if inWar)
setInterval(async () => {
  try {
    for (const [key, clanTag] of Object.entries(DB.userClans || {})) {
      if (!clanTag) continue;
      await updateAttendanceLogs(normalizeTag(clanTag));
    }
  } catch (e) {
...
async function updateAttendanceLogs(clanTag) {
  try {
    ...
    await safeSaveDB("attendance"); // ✅ no overwrite
