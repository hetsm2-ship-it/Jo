...
// Prefer loadDB/saveDB, else fallback to other possible names
const loadDB = dbMod.loadDB || dbMod.load || dbMod.read || dbMod.get;
const saveDB = dbMod.saveDB || dbMod.save || dbMod.write || dbMod.set;

// Safety checks
if (typeof loadDB !== "function") {
  throw new Error("db.js must export one of: loadDB / load / read / get");
}
if (typeof saveDB !== "function") {
  throw new Error("db.js must export one of: saveDB / save / write / set");
}
...
async function updateAttendanceForClan(rawTag) {
  ...
  const inWarActive = presentPlayers.size > 0 || normalPresent.size > 0 || cwlPresent.size > 0;

  if (!inWarActive) {
    console.log(`⏸️ No active inWar for ${tag}, skipping write.`);
    return;
  }
...
// ------------------------- 📌 Attendance Command -------------------------
// helpers.js (or same file above this command)
function canonTag(tag) {
  if (!tag) return null;
  let t = String(tag).trim().toUpperCase().replace(/O/g, "0");
  if (!t.startsWith("#")) t = "#" + t;
...
// ------------------------- 📌 Attendance Command (fresh read) -------------------------
async function attendance({ sock, jid, args, sender, isGroup }) {
  try {
    const key = isGroup ? sender : jid;

    const raw = args?.[0] || DB?.userClans?.[key];
    const tag = canonTag(raw);

    if (!tag) {
      return sock.sendMessage(jid, {
        text: `✅ Usage: ${CONFIG.COMMAND_PREFIX}attendance #CLANTAG\nOr set default: ${CONFIG.COMMAND_PREFIX}setclan #CLANTAG`
      });
    }

    // 🔥 IMPORTANT: force a FRESH read so you don’t show stale memory
    const dbMod = require("./db");
    const loadDB =
    ...
    const logs = fresh?.attendanceLogs?.[tag];
    ...
    const s = logs.lastSnapshot;
    const lastUpdated = s.timestamp ? require("moment")(s.timestamp).fromNow() : "just now";

    let rep = `🧮 *Attendance Report (Last Month)*\n`;
    rep += `🏰 Clan: ${s.clanName}\n`;
    rep += `⏳ Last Updated: ${lastUpdated}\n`;
    rep += `👥 Current Members: ${s.total}\n`;
    rep += `✅ Present: ${s.present} (${s.percentPresent}%)\n`;
    rep += `❌ Absent: ${s.absent} (${s.percentAbsent}%)\n`;

    if (Array.isArray(s.joined) && s.joined.length) {
      rep += `\n🆕 Joined: ${s.joined.join(", ")}`;
    }
    if (Array.isArray(s.leaved) && s.leaved.length) {
      rep += `\n👋 Left: ${s.leaved.join(", ")}`;
    }
