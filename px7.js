...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    // Local API helper (NO global.cocFetch!)
    async function cocFetch(endpoint) {
      try {
        const apiKey = process.env.COC_API_KEY || (CONFIG && CONFIG.COC_API_KEY) || '';
        if (!apiKey) return { error: true, message: 'COC_API_KEY not set' };
    ...
    // ========== NORMAL WAR ==========
    async function formatNormalWar(playerNo) {
      const clanData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}`);
      ...
      if (playerNo !== null) {
        ...
        if (!player) return "❌ Invalid player number.\n";
        let msg = `📌 *Normal War: ${player.name}*\n\n`;
        const warMem = warData?.clan?.members?.find(x => x.tag === player.tag);
        if (warState === "inWar") {
          msg += warMem && warMem.attacks?.length
            ? warMem.attacks.map(
                (atk, idx) =>
                  `Attack ${idx + 1}: vs ${warData.opponent.members.find(o => o.tag === atk.defenderTag)?.name || "Unknown"}, ${atk.destructionPercentage}% (${atk.stars}⭐)`
              ).join("\n") + "\n"
            : "No live attacks in current war.\n";
        } else if (warState === "warEnded" || warState === "preparation") {
          msg += "(Not in live war, see warlog/history below)\n";
        } else {
          msg += "(Player not in any current war)\n";
        }
        // Show warlog/history if available
        ...
        if (logs.length) {
          ...
          let idx = 1;
          for (const [warKey, groupLogs] of Object.entries(grouped)) {
            const war = groupLogs[0];
            msg += `*${idx++}*. 🏰 ${war.myClan} vs ${war.oppClan}\n`;
            const attacks = groupLogs.filter(l => !l.isFromLive).slice(-2).sort((a, b) => a.order - b.order);
            attacks.length ? attacks.forEach((atk, i) => {
      ...
      for (const [i, m] of clanData.memberList.entries()) {
        ...
        if (warState === "inWar") {
          ...
          msg += "(not in live war, check warlog/player for stats)\n";
        } else {
          msg += "(no active war)\n";
        }
        msg += "\n";
    ...
    // ========== CWL WAR ==========
    async function formatCWLWar(playerNo) {
      const cwlData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
      if (!cwlData?.rounds) return null;

      // Latest round: pick latest active war (side detect)
      let found = false, state = '', myClan = null, warData = null, roundIndex = -1;
      for (let i = cwlData.rounds.length-1; i >= 0; i--) {
        let round = cwlData.rounds[i];
        if (!Array.isArray(round.warTags)) continue;
        for (const warTag of round.warTags) {
          if (warTag && warTag !== '#0') {
            const war = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
            if (war?.clan && tagsEqual(war.clan.tag, clanTag)) {
              found = true; state = war.state; myClan = war.clan; warData = war; roundIndex = i+1; break;
            }
            if (war?.opponent && tagsEqual(war.opponent.tag, clanTag)) {
              found = true; state = war.state; myClan = war.opponent; warData = war; roundIndex = i+1; break;
            }
          }
        }
        if (found) break;
      }
      if (!found || !myClan) return null;

      // PLAYER DETAILS
      if (playerNo !== null) {
        const player = (myClan.members || [])[playerNo - 1];
        if (!player) return "❌ Invalid player number.\n";
        let msg = `📌 *CWL War: ${player.name}*\n\n`;
        if (player.attacks?.length) {
          player.attacks.forEach((atk, idx) => {
            msg += `Attack ${idx+1}: vs ${atk.defenderTag}, ${atk.destructionPercentage}% (${atk.stars}⭐)\n`;
          });
        } else {
          msg += "(No attack used in this CWL round)\n";
        }
        return msg;
      }

      // FULL CLAN LIST
      let msg = `🏆 *CWL State: ${state.toUpperCase()} (Round ${roundIndex})*\nClan: ${myClan.name} (${myClan.tag})\n\n`;
      for (const [i, m] of myClan.members.entries()) {
        msg += `${i + 1}. ${m.name} ${m.tag}\n`;
      ...
      // Last war detail if prep
      if (state === 'preparation') {
        let lastAttacks = '';
        for (let j = cwlData.rounds.length - 1; j >= 0; j--) {
          let round = cwlData.rounds[j];
          if (!Array.isArray(round.warTags)) continue;
          for (const warTag of round.warTags) {
            if (warTag && warTag !== '#0') {
              ...
              if (
                ...
                lastAttacks += `--- Last CWL War Attacks ---\n`;
                c.members.forEach(mem => {
                  lastAttacks += `${mem.name} ${mem.tag} `;
                  lastAttacks += mem.attacks?.length
                    ? `(USED attack)\n`
                    : "(UNUSED attack)\n";
                });
                break;
          ...
          if (lastAttacks) break;
        }
        msg += "\n" + (lastAttacks || "--- Last CWL War Attacks Not found ---\n");
      }
      return msg;
    }

    // ========== COMBINED PRIORITY LOGIC ==========
    const playerNo = args[0] && !isNaN(args[0]) ? parseInt(args[0], 10) : null;
    ...
    if (normalWarState !== "notInWar") {
      msg = await formatNormalWar(playerNo);
    } else {
      msg = await formatCWLWar(playerNo) || await formatNormalWar(playerNo);
    }
