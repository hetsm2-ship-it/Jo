...
// Safe fallback for records
const recordsCount = Array.isArray(logs.records) ? logs.records.length : 0;
const totalMembers = latest.total ?? 0;
const presentCount = latest.present ?? 0;
const absentCount = latest.absent ?? 0;
...
rep += `📅 Records stored: ${recordsCount}\n`;
rep += `⏳ Last Updated: ${lastUpdatedText}\n`;
rep += `👥 Current Members: ${totalMembers}\n`;
rep += `✅ Present: ${presentCount} (${percentPresent}%)\n`;
rep += `❌ Absent: ${absentCount} (${percentAbsent}%)\n`;

if (Array.isArray(latest.joined) && latest.joined.length) {
  rep += `\n🆕 Joined: ${latest.joined.join(", ")}`;
}
if (Array.isArray(latest.leaved) && latest.leaved.length) {
