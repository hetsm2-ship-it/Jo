...
async function attendance({ sock, jid, args, sender, isGroup }) {
  try {
    // 1. Har run me fresh DB load karo
    const DB = await loadDB();

    // 2. User ke lie relevant clanTag lo (args se ya default se)
    const key = isGroup ? sender : jid;
    let clanTag = args[0] || DB.userClans?.[key];
    clanTag = (clanTag || "").toUpperCase().trim();

    // 3. Agar clanTag hi nahi mila, to usage reply bhejo
    if (!clanTag) {
    ...
    // STEP 3: lastSnapshot existence check
    if (!logs.lastSnapshot || typeof logs.lastSnapshot !== "object") {
      return await sock.sendMessage(jid, {
    ...
    // STEP 5: Build the reply with all the details
    let rep = "";
    rep += `🧮 *Attendance Report (Last Month)*\n`;
    rep += `🏰 Clan: ${latest.clanName || "Unknown Clan"}\n`;
    rep += `📅 Records stored: ${records.length}\n`;
    ...
    // STEP 6: Show joined/leaved if present
