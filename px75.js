...
// ------------------------- 🧮 Attendance Logs System (FINAL RECODE) -------------------------

// ✅ Global lock (to avoid double execution)
let isAttendanceRunning = false;

// ✅ Safe DB Save (prevents race conditions)
let isSaving = false;
async function safeSaveDB(section) {
  if (isSaving) return;
  isSaving = true;
  try {
    await saveDB(section);
    console.log("💾 Attendance saved successfully.");
  } catch (e) {
    console.error("❌ safeSaveDB error:", e.message);
  } finally {
    isSaving = false;
  }
}

// ✅ Normalize clanTag always
...
setInterval(async () => {
  if (isAttendanceRunning) {
    console.log("⚠️ Attendance updater already running, skipping...");
    return;
  }
  ...
  try {
    ...
    console.log(`🔎 Auto attendance check started for ${clans.length} clans...`);

    for (const clanTag of clans) {
      await updateAttendanceLogs(clanTag);
    }

    await safeSaveDB("attendance");
  } catch (err) {
    console.error("❌ Attendance auto-save error:", err.message);
  } finally {
    isAttendanceRunning = false;
  }
}, 90 * 1000); // runs every 90 sec
...
async function updateAttendanceLogs(clanTag) {
  try {
    ...
    let inWarActive = false;

    // ---------------- Normal War ----------------
    const liveWar = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
    if (liveWar?.state === "inWar" && liveWar.clan?.members) {
      inWarActive = true;
      for (const m of liveWar.clan.members) {
        if (m.attacks?.length > 0) presentPlayers.add(m.tag);
      }
      console.log(`⚔️ Normal war active for ${clanTag}: ${presentPlayers.size} present so far`);
    }
      ...
      if (leagueGroup?.rounds?.length) {
        for (const round of leagueGroup.rounds) {
          for (const warTag of round.warTags || []) {
            ...
            if (!cwlWar || cwlWar.state !== "inWar") continue;

            inWarActive = true;

            const isClanSide = cwlWar.clan?.tag?.toUpperCase() === clanTag;
            const members = isClanSide ? (cwlWar.clan?.members || []) : (cwlWar.opponent?.members || []);

            for (const member of members) {
              if (member.attacks?.length > 0) presentPlayers.add(member.tag);
            }
            console.log(`⚔️ CWL war active for ${clanTag}: ${presentPlayers.size} present so far`);
          }
        }
      }
    } catch (err) {
      console.error("❌ CWL fetch error:", err.message);
    }

    // ---------------- Skip if no war ----------------
    if (!inWarActive) {
      console.log(`⏸️ No active war for ${clanTag}, skipping attendance update.`);
      return;
    }
    ...
    const presentCount = clanMembers.items.filter(p => presentPlayers.has(p.tag)).length;
    const absentCount = total - presentCount;

    // ✅ Safe initialization
    DB.attendanceLogs = DB.attendanceLogs || {};
    DB.attendanceLogs[clanTag] = DB.attendanceLogs[clanTag] || {};
    DB.attendanceLogs[clanTag].records = Array.isArray(DB.attendanceLogs[clanTag].records)
      ? DB.attendanceLogs[clanTag].records
      : [];
    ...
    console.error(`❌ updateAttendanceLogs failed for ${clanTag}:`, e.message);
  }
}
...
async function attendance({ sock, jid, args, sender, isGroup }) {
  try {
    ...
    // Step 3: Basic checks for clanTag
    if (!clanTag) {
    ...
    // Step 4: Defensive check for DB structure
    const logs = DB.attendanceLogs && DB.attendanceLogs[clanTag] && typeof DB.attendanceLogs[clanTag] === "object"
      ? DB.attendanceLogs[clanTag]
      : null;

    if (!logs) {
      return await sock.sendMessage(jid, {
        text: "📭 Attendance logs not found for this clan. Wait for auto-attendance to update during the next war."
      });
    }

    // Step 5: Defensive check for records
    let records = [];
    ...
    if (records.length === 0) {
      return await sock.sendMessage(jid, {
        text: "🕒 No attendance records yet, wait until a war is in progress and logs update."
      });
    }

    // Step 6: lastSnapshot
    const latest = logs.lastSnapshot && typeof logs.lastSnapshot === "object"
      ? logs.lastSnapshot
      : null;
    if (!latest) {
      return await sock.sendMessage(jid, {
        text: "🕒 Attendance snapshot not available yet, will appear after next auto-update."
      });
    }

    // Step 7: All stats with fallback/defaults
    ...
    rep += `🏰 Clan: ${latest.clanName || "Unknown Clan"}\n`;
    rep += `📅 Records stored: ${records && Array.isArray(records) ? records.length : 0}\n`;
    console.log('DEBUG records:', records); // Should always show [] or array
    rep += `⏳ Last Updated: ${lastUpdatedText}\n`;
    rep += `👥 Current Members: ${total}\n`;
    rep += `✅ Present: ${present} (${percentPresent}%)\n`;
    rep += `❌ Absent: ${absent} (${percentAbsent}%)\n`;
    if (joined.length) rep += `\n🆕 Joined: ${joined.join(", ")}`;
    if (leaved.length) rep += `\n👋 Left: ${leaved.join(", ")}`;

    await sock.sendMessage(jid, { text: rep });
  } catch (e) {
    await sock.sendMessage(jid, { text: `❌ Error fetching attendance: ${e?.stack || e?.message}` });
  }
}
...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  try {
    ...
    // ---------------- Helper: Normalize clan tag ----------------
    let clanTagUC = clanTag.toUpperCase().trim();
    clanTagUC = clanTagUC.replace(/O/g, "0");
    if (!clanTagUC.startsWith("#")) clanTagUC = "#" + clanTagUC;

    console.log("🔎 Fetching warlog for:", clanTagUC);

    // ---------------- Helper: Parse CoC timestamp ----------------
    ...
    let normalWars = [];
    try {
      const normalWarlog = await cocFetch(`/clans/${encodeURIComponent(clanTagUC)}/warlog`);
      normalWars = (normalWarlog.items || []).map(w => {
        ...
        return {
          ...
          endTime: parseCoCTime(w.endTime)?.getTime() || Date.now()
        };
      });
    ...
    let cwlWars = [];
    try {
      const league = await cocFetch(`/clans/${encodeURIComponent(clanTagUC)}/currentwar/leaguegroup`);
      const rounds = league.rounds || [];
      ...
      for (let i = 0; i < rounds.length; i++) {
        for (const warTag of (rounds[i].warTags || [])) {
          ...
          try {
            ...
            // ✅ Skip anything not ended
            if (war.state !== "warEnded") {
              console.log(`⏩ Skipping CWL Round ${i + 1} (state=${war.state})`);
              continue;
            }

            // ✅ Identify which side is my clan
            let myClan, enemyClan;
            if (tagsEqual(war.clan?.tag, clanTagUC)) {
              myClan = war.clan;
              enemyClan = war.opponent;
            } else if (tagsEqual(war.opponent?.tag, clanTagUC)) {
              myClan = war.opponent;
              enemyClan = war.clan;
            } else continue;
            ...
            const endTimeParsed = parseCoCTime(war.endTime)?.getTime() || Date.now();
            ...
            console.log(`   ✅ CWL Round ${i + 1} war added vs ${enemyClan?.name}, endTime: ${war.endTime}`);
          } catch (e) {
            console.log("⚠️ CWL war fetch failed:", warTag, e.message);
          }
        }
      }
      console.log(`✅ CWL wars fetched: ${cwlWars.length}`);
    } catch (err) {
      console.log("❌ CWL warlog fetch failed:", err.message);
    }

    // ---------------- Merge + Sort ----------------
    const allWars = [...normalWars, ...cwlWars];
    if (!allWars.length) {
      return await sock.sendMessage(jid, { text: "❌ No war logs found (normal or CWL)." });
    }

    allWars.sort((a, b) => b.endTime - a.endTime);
    const warsToShow = allWars.slice(0, 10);

    // ---------------- Format Output ----------------
    const lines = warsToShow.map((w, idx) => {
      return `*No.${idx + 1}*  (${w.type}) ⚔️ vs ${w.opponent} | ${w.result} (${w.stars})`;
    });

    const message = `📜 *Last 10 Wars for ${clanTagUC}:*\n\n` + lines.join("\n");
  ...
  try {
    ...
    async function cocFetch(endpoint) {
      ...
      if (!res.ok) {
        const text = await res.text();
        let msg = text;
        try { msg = JSON.parse(text).reason || text; } catch {}
        throw new Error(msg);
      }
  ...
  if (warState === "inWar") {
    ...
    if (!logs.length) {
      msg += "Leader wasn't selected this member in last war\n\n";
      continue;
    }
    // Try API fallback
    const warLogsApi = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/warlog?limit=1`);
    let apiHandled = false;
    if (warLogsApi?.reason !== "accessDenied" && warLogsApi?.items?.length) {
      ...
      status =
        usedCount === 0 && unusedCount === 2 ? "(unused attacks in last war)" :
        usedCount === 1 ? "(used 1 attack in last war)" :
        usedCount === 2 ? "(used 2 attacks in last war)" :
        "(not in last war)";
      apiHandled = true;
    }
  ...
  msg += status + "\n\n";
      ...
      // If normal notInWar, show CWL
      else {
        const league = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
        const rounds = league.rounds || [];
        let found = false, state = '', myClan = null, warData = null, roundIndex = -1;
          ...
          for (let i = rounds.length - 1; i >= 0; i--) {
            ...
            for (const warTag of round.warTags) {
              if (warTag && warTag !== '#0') {
                ...
                if (side && war.state === s) {
                  found = true; state = war.state;
                  myClan = side === 'clan' ? war.clan : war.opponent;
                  warData = war; roundIndex = i+1; break;
                }
              }
            }
            if (found) break;
        ...
        if (found && myClan) {
          msg += `🏆 *CWL State: ${
            state === 'inWar' ? 'Live War' :
            state === 'warEnded' ? 'War Ended' :
          ...
          myClan.members.forEach((m, idx) => {
            ...
            if (state === 'inWar') {
              msg += m.attacks?.length
                ? "(cwl used attack in current war)\n"
                : "(cwl unused attack in current war)\n";
            } else if (state === 'warEnded') {
              msg += m.attacks?.length
                ? "(cwl used attack in last war)\n"
                : "(cwl unused attack in last war)\n";
            } else if (state === 'preparation') {
              msg += "(cwl preparation for next war)\n";
            }
            msg += "\n";
          });
          // Show last war ONLY if state is prep (as per your latest logic)
          if (state === 'preparation') {
            ...
            for (let i = rounds.length - 1; i >= 0; i--) {
              ...
              for (const warTag of round.warTags) {
                if (warTag && warTag !== '#0') {
                  const w = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
                  if (
                    (w.state === 'warEnded') &&
                    ((w.clan && tagsEqual(w.clan.tag, clanTag)) ||
                    (w.opponent && tagsEqual(w.opponent.tag, clanTag)))
                  ) {
                    const c = (w.clan && tagsEqual(w.clan.tag, clanTag)) ? w.clan : w.opponent;
                    lastBlock += "--- Last CWL War Attacks ---\n";
                    c.members.forEach((mem, idx2) => {
                      lastBlock += `${idx2 + 1}. ${mem.name} ${mem.tag}\n`;
                      lastBlock += mem.attacks?.length
                        ? "(cwl used attack in last war)\n"
                        : "(cwl unused attack in last war)\n";
                      lastBlock += "\n";
                    });
            ...
            msg += lastBlock || "--- Last CWL War Attacks Not found ---\n";
          }
        } else {
    ...
    // ---- PLAYER DETAIL (normal war / cwl war) ----
    if (!isNaN(args[0])) {
      ...
      if (warState !== "notInWar") {
        ...
        if (warState === "inWar") {
          ...
          if (wm) {
          if (warState === "inWar" && !wm) {
  msg += "The leader hasn't selected this member to be in current war\n";
}

            msg += "Current Normal War:\n";
            msg += wm.attacks?.map(
              (atk, idx) => `Attack ${idx + 1} → vs ${warData.opponent.members.find(o => o.tag === atk.defenderTag)?.name || "Unknown"} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`
            ).join("") || "Attack 1 → Not used\nAttack 2 → Not used\n";
            msg += "\n";
          } else {
            msg += "Not in current normal war roster.\n\n";
          }
        }
        if (warLogs.length) {
          msg += `📜 *War History:*\n`;
          ...
          for (const [warKey, logs] of Object.entries(grouped)) {
            ...
            const lastTwo = logs.filter(l => !l.isFromLive).slice(-2).sort((a, b) => a.order - b.order);
            lastTwo.length
              ? lastTwo.forEach((atk, i) => {
                msg += atk.destructionPercentage > 0 || atk.stars > 0
                  ? `Attack${lastTwo.length > 1 ? " " + (i+1) : ""} → vs ${atk.oppName} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`
                  : `Attack${lastTwo.length > 1 ? " " + (i+1) : ""} → Not used\n`;
              })
      ...
      // ---- cwl -----
        ...
        // Get latest round with state-priority as above
        const league = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
        const rounds = league.rounds || [];
        let found = false, state = '', myClan = null, warData = null, roundIndex = -1;
        const statePriority = ['inWar', 'warEnded', 'preparation'];
        ...
        const player = myClan.members[playerNo - 1];
        if (!player) return await sock.sendMessage(jid, { text: "❌ Invalid player number." });
        
        // Utility functions
function normalizeTag(tag) {
  return tag ? tag.replace('#', '').toUpperCase().trim() : '';
}
function tagsEqual(a, b) {
  return a && b && a.replace('#','').toUpperCase() === b.replace('#','').toUpperCase();
}

// Side detection (fixes opponent showing as you in CWL)
const isUs = tagsEqual(warData.clan.tag, clanTag); // clanTag is your clan
const usClan = isUs ? warData.clan : warData.opponent;
const enemyClan = isUs ? warData.opponent : warData.clan;

let msg = `📌 *Player War Details for ${player.name} (${player.tag})*\n\n`;
msg += `🏰 ${usClan.name} (${usClan.tag}) vs ${enemyClan.name} (${enemyClan.tag})\n\n`;
msg += `Current CWL war (Round ${roundIndex})\n`;

async function getPlayerNameByTag(tag) {
  if (!tag) return "Unknown";
  try {
    const playerData = await cocFetch(`/players/${encodeURIComponent(tag)}`);
...
if (player.attacks?.length) {
  for (const atk of player.attacks) {
    // Always get defender name by tag, so bench/swap/leave also handled
    const defenderName = await getPlayerNameByTag(atk.defenderTag);
    msg += `Attack - ${defenderName} - ${atk.stars} ⭐ - ${atk.destructionPercentage}%\n`;
  }
} else {
  msg += "Attack - Not used\n";
}
        ...
        // Show war history
        if (DB.playerWarLogs?.[player.tag]) {
          const logs = DB.playerWarLogs[player.tag].filter(l => !l.isFromLive).slice(-10).reverse();
          if (logs.length) {
            msg += `\n📜 *War History:*\n`;
            let grouped = {};
            ...
            for (const [warKey, logs] of Object.entries(grouped)) {
              const sample = logs[0];
              msg += `No.${idx++} 🏰 ${sample.myClan} (${sample.clanTag}) vs ${sample.oppClan} (${sample.oppClanTag})${sample.cwlRound ? ` (Round ${sample.cwlRound})` : ""}\n`;
              logs.forEach((atk, i) => {
                msg += atk.destructionPercentage > 0 || atk.stars > 0
                  ? `Attack${logs.length > 1 ? " " + (i+1) : ""} → vs ${atk.oppName} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`
                  : `Attack${logs.length > 1 ? " " + (i+1) : ""} → Not used\n`;
              });
              msg += "\n";
            }
          }
...
async function runGetWarLogs() {
  ...
  try {
    // DB load karna async hona chahiye, await laga ke
    DB = await loadDB();
    ...
    for (const clanTag of clans) {
      try {
        ...
        // Spawn getlogwar.js and wait for finish
        await new Promise((resolve, reject) => {
        ...
        // Spawn cwlgetwar.js and wait for finish
        await new Promise((resolve, reject) => {
          const proc = spawn("node", ["cwlgetwar.js"], { stdio: "inherit" });
          proc.on("close", (code) => {
            if (code === 0) resolve();
            else reject(new Error(`cwlgetwar.js exited with code ${code}`));
          });
          proc.on("error", reject);
        });

        console.log(`War finalized for clan ${clanTag}`);
      } catch (e) {
        console.error(`Error while processing clan ${clanTag}: ${e.message}`);
      }
    }
  } catch (err) {
    console.error("runGetWarLogs fatal error:", err.message);
  } finally {
