...
HANDLERS.removeadmin = async ({ sock, jid, args, sender }) => {
  ...
  if (DB.admins[adminJid]) {
    ...
    return sock.sendMessage(jid, { text: `❌ ${adminJid} admin not is in admin list` });
  }
};
...
// ✅ Message handling with spam prevention
if (msg.key.fromMe) {
  // 👉 Bot ka apna self-message
  const botNumber = msg.key.participant || msg.key.remoteJid;

  if (!OWNER_JIDS.includes(botNumber)) {
    logger.info(`❌ Blocked self-message from non-owner JID: ${botNumber}`);
    return;
  }

  if (DB.lastMessages[botNumber] === text) {
    logger.info(`⚠️ Ignored repeated self-message from ${botNumber}`);
    return;
  }

  DB.lastMessages[botNumber] = text;
  await saveDB();
  logger.info(`✅ Accepted self-message from ${botNumber}: ${text}`);

} else {
  // 👉 Normal incoming messages
  if (!OWNER_JIDS.includes(sender)) {
    logger.info(`❌ Blocked message from non-owner: ${sender}`);
    return;
  }

  if (DB.lastMessages[sender] === text) {
    logger.info(`⚠️ Ignored repeated owner message from ${sender}`);
    return;
  }

  DB.lastMessages[sender] = text;
  await saveDB();
  logger.info(`✅ Accepted owner message from ${sender}: ${text}`);
}
