...
HANDLERS.removeclan = async function ({ sock, jid, sender }) {
  ...
  try {
    ...
    // Helper for CWL API via node-fetch or axios
    async function cocFetch(endpoint) {
    ...
    if (!args[0]) {
      ...
      // ------------------------- ✅ If normal war NOT active, CWL block -----------------------------
      else {
        // --- CWL, latest inWar or warEnded or prep as fallback ---
        ...
        let msg = '';
        if (found && myClan) {
          msg += `🏆 *CWL State: ${state.toUpperCase()} (Round ${roundIndex})*\nClan: ${myClan.name} (${myClan.tag})\n\n`;
          myClan.members.forEach((m, idx) => {
            msg += `${idx + 1}. ${m.name} ${m.tag}\n`;
            const n = m.attacks?.length || 0;
            msg += n === 0 ? "(UNUSED attack)\n"
              : n === 1 ? "(USED 1 attack)\n"
              : "(USED 2 attacks)\n";
            msg += "\n";
          });
          // Show last war ONLY if state is prep
                ...
                if (warTag && warTag !== '#0') {
                  ...
                  if (
                    ...
                    c.members.forEach(mem => {
                      lastBlock += `${mem.name} ${mem.tag} `;
                      lastBlock += mem.attacks?.length ? `(USED attack)\n` : "(UNUSED attack)\n";
                    });
                    break;
                  }
                }
    ...
    if (!isNaN(args[0])) {
      ...
      // Normal war active — keep your old logic
      if (warState !== "notInWar") {
        if (!player) {
        ...
        if (warState === "inWar") {
          ...
          else if (lastLogs.length === 0) msg += `1. The user wasn't attacked in last war\n\n`;
        }
        if (warLogs.length) {
          msg += `📜 *War History:*\n`;
          let grouped = {};
      ...
      // CWL player info (only if notInWar in normal)
      else {
        // Latest active CWL
        const league = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
        const rounds = league.rounds || [];
