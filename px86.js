...
if (!isNaN(args[0])) {
  ...
  // ================= CWL WAR =================
    ...
    const league = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
    const rounds = league.rounds || [];
    let found = false, state = '', myClan = null, roundIndex = -1, cwlWar = null;
    const statePriority = ['inWar', 'warEnded', 'preparation'];
    ...
    for (const s of statePriority) {
      for (let i = rounds.length - 1; i >= 0; i--) {
        ...
        for (const warTag of round.warTags) {
          if (warTag && warTag !== '#0') {
            const war = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
            if ((war.clan && tagsEqual(war.clan.tag, clanTag)) || (war.opponent && tagsEqual(war.opponent.tag, clanTag))) {
              if (war.state === s) {
                found = true;
                state = war.state;
                cwlWar = war;
    ...
    let msg = `📌 *Player War Details for ${player.name} (${player.tag})*\n\n`;
    msg += `🏰 ${cwlWar.clan.name} (${cwlWar.clan.tag}) vs ${cwlWar.opponent.name} (${cwlWar.opponent.tag})\n\n`;

    if (state === "inWar") {
      msg += `🔥 *Current CWL War (Round ${roundIndex}):*\n`;
      if (player.attacks?.length) {
        player.attacks.forEach((atk) => {
          const opp = cwlWar.opponent.members.find(o => o.tag === atk.defenderTag);
