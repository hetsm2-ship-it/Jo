...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  ...
  try {
    ...
    // Find the most relevant CWL war for a clan (state priority)
    async function findCwlWarForClan(tag) {
      try {
        const league = await cocFetch(`/clans/${encodeURIComponent(tag)}/currentwar/leaguegroup`);
        const rounds = league?.rounds || [];
        ...
        for (const state of statePriority) {
          for (let i = rounds.length - 1; i >= 0; i--) {
            ...
            for (const warTag of round.warTags) {
              ...
              try {
                const war = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(warTag)}`);
                // which side is our clan?
                let side = null;
                if (war?.clan && tagsEqual(war.clan.tag, tag)) side = 'clan';
                if (war?.opponent && tagsEqual(war.opponent.tag, tag)) side = 'opponent';
                if (side && war.state === state) {
                  return {
                    found: true,
                    war,
                    mySide: side, // 'clan' or 'opponent'
                    roundIndex: i + 1,
                    state,
                  };
                }
              } catch (e) {
                // continue scanning other warTags
              }
            }
          }
        }
    ...
    // small util to build War History from DB.playerWarLogs
    function buildPlayerHistory(playerTag) {
      const logs = (DB.playerWarLogs?.[playerTag] || []).filter(l => !l.isFromLive);
      if (!logs.length) return [];
      // group by warKey or clanTag_oppClanTag
      const grouped = {};
      for (const l of logs) {
        const key = l.warId || `${l.clanTag}_${l.oppClanTag}`; // warId optional
        if (!grouped[key]) grouped[key] = [];
        grouped[key].push(l);
      }
      // produce array of groups sorted latest-first by time if available
      const groups = Object.values(grouped).map(arr => arr.sort((a,b) => (a.time||'').localeCompare(b.time||'')));
      // sort groups by last log time desc
      groups.sort((A,B) => {
        const ta = A[A.length-1]?.time || '';
        const tb = B[B.length-1]?.time || '';
        return tb.localeCompare(ta);
      });
      return groups;
    }

    // ---------- CASE A: cminfo (no args) => show clan members + statuses ----------
    if (!args[0]) {
      const clanData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}`);
      const warData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
      const warState = warData?.state || 'notInWar';

      // If normal war active (inWar / preparation / warEnded)
        ...
        for (const [i, m] of (clanData.memberList || []).entries()) {
          ...
          if (warState === 'inWar') {
            if (!wm) {
              ...
              status = used === 0 ? "(unused attacks in current war)"
                     : used === 1 ? "(used 1 attack in current war)"
                     : "(used 2 attacks in current war)";
            }
          } else if (warState === 'preparation' || warState === 'warEnded') {
            // try DB first then API fallback
            const logs = DB.playerWarLogs?.[m.tag] || [];
            let resolved = false;
            if (logs.length) {
              const lastTwo = logs.filter(l => !l.isFromLive).slice(-2);
              const usedCount = lastTwo.filter(l => l.destructionPercentage > 0).length;
              status = usedCount === 0 ? "(unused attacks in last war)"
                     : usedCount === 1 ? "(used 1 attack in last war)"
                     : "(used 2 attacks in last war)";
              resolved = true;
            } else {
              // API fallback for last war
              try {
                const warLogsApi = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/warlog?limit=1`);
                if (warLogsApi?.items?.length) {
                  const lastWar = warLogsApi.items[0];
                  const playerInLastWar = lastWar?.clan?.members?.find(x => x.tag === m.tag);
                  if (playerInLastWar) {
                    const usedCount = (playerInLastWar.attacks || []).filter(a => a.destructionPercentage > 0).length;
                    status = usedCount === 0 ? "(unused attacks in last war)"
                           : usedCount === 1 ? "(used 1 attack in last war)"
                           : "(used 2 attacks in last war)";
                    resolved = true;
            ...
            if (!resolved) status = "(not in last war / no record)";
          } else {
      ...
      // else normal notInWar => check CWL fallback
      const cwl = await findCwlWarForClan(clanTag);
      if (!cwl.found) {
        return await sock.sendMessage(jid, { text: `ℹ️ No active normal war nor CWL war found for ${clanTag}` });
      }

      // Build CWL message
      const myClan = cwl.mySide === 'clan' ? cwl.war.clan : cwl.war.opponent;
      let msg = `🏆 *CWL State: ${cwl.state === 'inWar' ? 'Live War' : cwl.state === 'warEnded' ? 'War Ended' : 'Preparation'} (Round ${cwl.roundIndex})*\nClan: ${myClan.name} (${myClan.tag})\n\n`;
      for (const [idx, m] of (myClan.members || []).entries()) {
        msg += `${idx+1}. ${m.name} ${m.tag}\n`;
        if (cwl.state === 'inWar') {
          msg += m.attacks?.length ? "(CWL used attack in current war)\n" : "(CWL unused attack in current war)\n";
        } else if (cwl.state === 'warEnded') {
          msg += m.attacks?.length ? "(CWL used attack in last war)\n" : "(CWL unused attack in last war)\n";
        } else {
          msg += "(CWL preparation / not selected)\n";
        }
        msg += "\n";
      }

      // If preparation show last ended round summary (best-effort)
      if (cwl.state === 'preparation') {
        msg += "--- Last finished CWL war (best-effort) ---\n";
        // scan rounds to find last warEnded
        try {
          const league = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
          const rounds = league?.rounds || [];
          let foundLast = false;
          for (let i = rounds.length - 1; i >= 0 && !foundLast; i--) {
            const round = rounds[i];
            if (!Array.isArray(round.warTags)) continue;
            for (const wtag of round.warTags) {
              if (!wtag || wtag === '#0') continue;
              try {
                const w = await cocFetch(`/clanwarleagues/wars/${encodeURIComponent(wtag)}`);
                if (w.state === 'warEnded' && ((w.clan && tagsEqual(w.clan.tag, clanTag)) || (w.opponent && tagsEqual(w.opponent.tag, clanTag)))) {
                  const c = tagsEqual(w.clan.tag, clanTag) ? w.clan : w.opponent;
                  c.members.forEach((mem, ix) => {
                    msg += `${ix+1}. ${mem.name} ${mem.tag}\n`;
                    msg += mem.attacks?.length ? "(CWL used attack in last war)\n" : "(CWL unused attack in last war)\n";
                    msg += "\n";
                  });
                  foundLast = true;
                  break;
          ...
          if (!foundLast) msg += "No recent finished CWL war found.\n";
        } catch (e) {
          msg += "Could not fetch last CWL war details.\n";
        }
      }
    ...
    // ---------- CASE B: cminfo <player_no> => player details ----------
    if (!isNaN(args[0])) {
      ...
      if (warState !== 'notInWar') {
        ...
        if (warState === 'inWar') {
          ...
          if (wm) {
            ...
            // Attack 1
            if (wm.attacks?.[0]) {
              const oppName = (warData.opponent.members.find(o => o.tag === wm.attacks[0].defenderTag)?.name) || "Unknown";
              msg += `Attack 1 → vs ${oppName} → ${wm.attacks[0].destructionPercentage}% (${wm.attacks[0].stars}⭐)\n`;
            } else {
              msg += "Attack 1 → Not used\n";
            }
            // Attack 2 (if normal war)
            if (!warData.isCwl) {
              if (wm.attacks?.[1]) {
                const oppName2 = (warData.opponent.members.find(o => o.tag === wm.attacks[1].defenderTag)?.name) || "Unknown";
                msg += `Attack 2 → vs ${oppName2} → ${wm.attacks[1].destructionPercentage}% (${wm.attacks[1].stars}⭐)\n`;
              } else {
        ...
        // Always append war history from DB (finalized logs)
        const groups = buildPlayerHistory(player.tag);
        if (groups.length) {
          msg += "📜 *War History:*\n";
          let idx = 1;
          for (const logs of groups) {
            const sample = logs[0];
            msg += `No.${idx++} 🏰 ${sample.myClan || sample.clanTag || '?'} (${sample.clanTag||''}) vs ${sample.oppClan || sample.oppClanTag || '?'} (${sample.oppClanTag||''})\n`;
            const lastTwo = logs.slice(-2).sort((a,b) => (a.order||0)-(b.order||0));
            if (!lastTwo.length) {
              msg += "Attack 1 → Not used\nAttack 2 → Not used\n";
            } else {
              lastTwo.forEach((atk, i) => {
                if (!atk || (atk.destructionPercentage === 0 && atk.stars === 0)) {
                  msg += `Attack ${i+1} → Not used\n`;
                } else {
                  msg += `Attack ${i+1} → vs ${atk.oppName || atk.opp || 'Unknown'} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`;
                }
              });
            }
            msg += "\n";
          ...
          msg += "📜 *War History:* No saved logs ❌\n";
        }
        return await sock.sendMessage(jid, { text: msg.trim() });
      }

      // ---------- fallback to CWL player info ----------
      const cwl = await findCwlWarForClan(clanTag);
      if (!cwl.found) {
        return await sock.sendMessage(jid, { text: "❌ No active normal war nor CWL found for this clan." });
      }
      const war = cwl.war;
      // determine our side and enemy
      const isUsClan = tagsEqual(war.clan.tag, clanTag);
      const usClan = isUsClan ? war.clan : war.opponent;
      const enemyClan = isUsClan ? war.opponent : war.clan;
      const cwlPlayer = (usClan.members || [])[playerNo - 1];
      if (!cwlPlayer) return await sock.sendMessage(jid, { text: "❌ Invalid player number for CWL roster." });

      let msg = `📌 *Player War Details for ${cwlPlayer.name} (${cwlPlayer.tag})*\n\n`;
      msg += `🏰 ${usClan.name} (${usClan.tag}) vs ${enemyClan.name} (${enemyClan.tag})\n\n`;
      if (cwl.state === 'inWar') {
        msg += `🔥 Current CWL war (Round ${cwl.roundIndex})\n`;
        if (cwlPlayer.attacks?.length) {
          for (const atk of cwlPlayer.attacks) {
            const defender = (enemyClan.members || []).find(o => o.tag === atk.defenderTag);
            msg += `Attack → vs ${defender?.name || 'Unknown'} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`;
          }
        } else {
          msg += "Attack → Not used\n";
        }
        ...
        msg += `🏁 CWL war ended (Round ${cwl.roundIndex}) — shown attacks if any\n\n`;
        if (cwlPlayer.attacks?.length) {
          for (const atk of cwlPlayer.attacks) {
            const defender = (enemyClan.members || []).find(o => o.tag === atk.defenderTag);
            msg += `Attack → vs ${defender?.name || 'Unknown'} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`;
        ...
        // preparation
        msg += `⚠️ CWL state: ${cwl.state} (Round ${cwl.roundIndex})\n`;
        msg += cwlPlayer.attacks?.length ? "Used attacks in previous round(s)\n\n" : "No attacks recorded yet\n\n";
      }

      // show history from DB
      const groups = buildPlayerHistory(cwlPlayer.tag);
      if (groups.length) {
        msg += "📜 *War History:*\n";
        let idx = 1;
        for (const logs of groups) {
          const sample = logs[0];
          msg += `No.${idx++} 🏰 ${sample.myClan || sample.clanTag || '?'} (${sample.clanTag||''}) vs ${sample.oppClan || sample.oppClanTag || '?'} (${sample.oppClanTag||''})${sample.cwlRound ? ` (Round ${sample.cwlRound})` : ""}\n`;
          logs.forEach((atk, i) => {
            msg += (atk.destructionPercentage === 0 && atk.stars === 0)
              ? `Attack ${i+1} → Not used\n`
              : `Attack ${i+1} → vs ${atk.oppName || atk.opp || 'Unknown'} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`;
          });
          msg += "\n";
        }
      } else {
    ...
    try { await sock.sendMessage(jid, { text: "❌ Error fetching clan info." }); } catch (e) {}
  }
};
