...
// Bas neeche war history print hoga.
        ...
        if (warLogs.length) {
          ...
          for (const logs of Object.values(grouped)) {
            const sample = logs[0];
            msg += `No.${idx++} 🏰 ${sample.myClan} (${sample.clanTag}) vs ${sample.oppClan} (${sample.oppClanTag})${
              sample.cwlRound ? ` (Round ${sample.cwlRound})` : ""
            }\n`;
            logs.forEach((atk, i) => {
  // ✅ Agar CWL war hai (cwlRound present), to numbering hatao aur (CWL) + Round show karo
  const isCWL = !!atk.cwlRound;
  if (i === 0) {
    msg += `${isCWL ? "(CWL) " : ""}🏰 ${atk.myClan} (${atk.clanTag}) vs ${atk.oppClan} (${atk.oppClanTag})${
      isCWL ? ` (Round ${atk.cwlRound})` : ""
    }\n`;
  }
