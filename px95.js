...
// Bas neeche war history print hoga.
        ...
        // 📜 Always show history (even if CWL war ended/prep)
        const warLogs = (DB.playerWarLogs?.[player.tag] || [])
  .slice(-20)
  .reverse();
...
if (warLogs.length) {
  ...
  for (const logs of Object.values(grouped)) {
    ...
    // Attacks
    logs.sort((a, b) => a.order - b.order).forEach((atk, i) => {
      if (atk.destructionPercentage === 0 && atk.stars === 0) {
        msg += isCWL ? `Attack → Not used\n` : `Attack ${i + 1} → Not used\n`;
      } else {
        msg += isCWL
          ? `Attack → vs ${atk.oppName} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`
          : `Attack ${i + 1} → vs ${atk.oppName} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`;
      }
    });
