...
HANDLERS.warlog = async ({ sock, jid, sender, isGroup }) => {
  ...
  try {
    ...
    if (!args[0]) {
      ...
      // ================= NORMAL WAR (active or prep/ended) =================
      ...
      // ================= CWL (if no normal war active) =================
        ...
        if (found && myClan) {
          msg += `🏆 *CWL State: ${
            state === "inWar" ? "Live War" : state === "warEnded" ? "War Ended" : state === "preparation" ? "Preparation" : state
          } (Round ${roundIndex})*\nClan: ${myClan.name} (${myClan.tag})\n\n`;
            ...
            for (let i = rounds.length - 1; i >= 0; i--) {
              ...
              for (const warTag of round.warTags) {
                if (warTag && warTag !== "#0") {
                  ...
                  if (
                    w.state === "warEnded" &&
                    ((w.clan && tagsEqual(w.clan.tag, clanTag)) || (w.opponent && tagsEqual(w.opponent.tag, clanTag)))
                  ) {
                    ...
                    c.members.forEach((mem, idx2) => {
                      ...
                      lastBlock += mem.attacks?.length ? "(cwl used attack in last war)\n" : "(cwl unused attack in last war)\n";
                      lastBlock += "\n";
                    });
                    break;
                  }
    ...
    if (!isNaN(args[0])) {
      ...
      const clanData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}`);
      let warData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
      ...
      if (warState && warState !== "notInWar") {
        ...
        if (warState === "inWar") {
          msg += `🏰 ${warData?.clan?.name || "?"} (${warData?.clan?.tag || "?"}) vs ${
          ...
          if (wm) {
            ...
            msg +=
              wm.attacks?.map(
                (atk, idx) =>
                  `Attack ${idx + 1} → vs ${
                    warData.opponent.members.find((o) => o.tag === atk.defenderTag)?.name || "Unknown"
                  } → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`
        ...
        // 📜 Always show War History
        const warLogs = (DB.playerWarLogs?.[player.tag] || []).filter((l) => !l.isFromLive).slice(-10).reverse();
        if (warLogs.length) {
          msg += "📜 *War History:*\n";
          let grouped = {};
      ...
      // ================= CWL PLAYER =================
        ...
        if (!found || !myClan || !myClan.members) {
          return await sock.sendMessage(jid, { text: "❌ No CWL war found." });
        }
...
// ✅ Sirf live war me header aur current attacks show karna hai
if (state === "inWar") {
  ...
  if (player.attacks?.length) {
    player.attacks.forEach((atk) => {
      const opp = warData.opponent.members.find((o) => o.tag === atk.defenderTag);
...
// ✅ Agar state warEnded / preparation / notInWar hai toh kuch aur header/attacks mat dikhana.
// Bas neeche war history print hoga.

// --- War history: include both live and non-live so CWL snapshots are detected ---
// DEBUG MODE: verbose console output to trace why cwlRound detection might fail
const DEBUG = true;

const rawPlayerLogs = DB.playerWarLogs?.[player.tag] || [];
if (DEBUG) console.log('=== WAR HISTORY DEBUG START ===', { playerTag: player.tag, rawPlayerLogsLen: rawPlayerLogs.length });

// take a slice we will work with
const warLogs = rawPlayerLogs.slice(-50).reverse();
if (DEBUG) {
  console.log('warLogs (used) length:', warLogs.length);
  warLogs.forEach((l, i) => {
    console.log(`[warLogs ${i}] warKey=${l.warKey} warId=${l.warId} cwlRound=${JSON.stringify(l.cwlRound)} typeof=${typeof l.cwlRound} isFromLive=${l.isFromLive} unused=${l.unused} order=${l.order} oppName=${JSON.stringify(l.oppName)}`);
  });
}
...
if (warLogs.length) {
  ...
  // group by normalized key (trim to avoid whitespace differences)
  let grouped = {};
  for (const log of warLogs) {
    const rawKey = log.warId || `${log.clanTag}_${log.oppClanTag}`;
    const key = (rawKey === undefined || rawKey === null) ? String(rawKey) : String(rawKey).trim();
    if (!grouped[key]) grouped[key] = [];
    grouped[key].push(log);
  }
  ...
  if (DEBUG) {
    ...
    console.log('Grouped keys count:', keys.length, 'keys:', keys);
    keys.forEach(k => {
      console.log(`--- Group "${k}" length=${grouped[k].length} ---`);
      grouped[k].forEach((l, idx) => {
        console.log(`  [${idx}] warKey=${l.warKey} warId=${l.warId} cwlRound=${JSON.stringify(l.cwlRound)} typeof=${typeof l.cwlRound} isFromLive=${l.isFromLive} order=${l.order} unused=${l.unused}`);
      });
    });
  }
  ...
  let idx = 1;
  for (const logs of Object.values(grouped)) {
    // robust CWL detection: find any entry with cwlRound that is not null/undefined
    const cwlLogs = logs.filter(l => l.cwlRound !== undefined && l.cwlRound !== null);
    const isCWL = cwlLogs.length > 0;
    const sample = cwlLogs[0] || logs[0];

    if (DEBUG) {
      console.log('Processing group. totalLogs=', logs.length, 'cwlLogsCount=', cwlLogs.length);
      if (cwlLogs.length) {
        console.log('  cwlRounds found in group:', cwlLogs.map(x => ({ value: x.cwlRound, type: typeof x.cwlRound, warKey: x.warKey })));
      } else {
        console.log('  No cwlRound found in this group.');
      }
      console.log('  sample used for header:', { sampleWarKey: sample.warKey, sampleCwlRound: sample.cwlRound, sampleType: typeof sample.cwlRound });
    }

    // Header
    msg += `No.${idx++} ${isCWL ? "(CWL) " : ""}🏰 ${sample.myClan} (${sample.clanTag}) vs ${sample.oppClan} (${sample.oppClanTag})`;
    if (isCWL) msg += ` (Round ${sample.cwlRound})`;
    msg += "\n";

    // Attacks — keep order stable; for CWL do not show numbering
    logs.sort((a, b) => (a.order || 0) - (b.order || 0)).forEach((atk, i) => {
      const notUsed = ((atk.destructionPercentage === 0 && atk.stars === 0) || atk.unused);
      if (notUsed) {
        msg += isCWL ? `Attack → Not used\n` : `Attack ${i + 1} → Not used\n`;
      } else {
        const oppName = (atk.oppName || atk.oppName === "" ? atk.oppName : "Unknown");
        msg += isCWL
          ? `Attack → vs ${oppName} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`
          : `Attack ${i + 1} → vs ${oppName} → ${atk.destructionPercentage}% (${atk.stars}⭐)\n`;
      }
    });
...
if (DEBUG) {
  console.log('=== WAR HISTORY DEBUG END ===');
  // quick summary: how many groups had CWL
  const summaryGroups = Object.values((DB.playerWarLogs?.[player.tag] || []).slice(-50).reverse().reduce((acc, log) => {
    const k = (log.warId || `${log.clanTag}_${log.oppClanTag}`) + '';
    (acc[k] = acc[k] || []).push(log);
    return acc;
  }, {}));
  const groupSummary = summaryGroups.map(g => ({
    keySample: (g[0] && (g[0].warId || `${g[0].clanTag}_${g[0].oppClanTag}`)),
    len: g.length,
    hasCWL: g.some(x => x.cwlRound !== undefined && x.cwlRound !== null),
    cwlRounds: g.filter(x => x.cwlRound !== undefined && x.cwlRound !== null).map(x => ({ val: x.cwlRound, type: typeof x.cwlRound }))
  }));
  console.log('Group summary:', JSON.stringify(groupSummary, null, 2));
}
