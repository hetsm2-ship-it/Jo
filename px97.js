...
const warLogs = (DB.playerWarLogs?.[player.tag] || []).slice(-50).reverse();
console.log(`[DEBUG] Using ${warLogs.length} logs for war history grouping...`);
...
if (warLogs.length) {
  ...
  console.log(`[DEBUG] Grouped into ${Object.keys(grouped).length} wars:`, Object.keys(grouped));
  ...
  for (const logs of Object.values(grouped)) {
    ...
    console.log(`[DEBUG] Group ${idx}: isCWL=${isCWL} cwlRounds=`, cwlLogs.map(x => x.cwlRound));
    ...
    logs.sort((a, b) => (a.order || 0) - (b.order || 0)).forEach((atk, i) => {
      ...
      msg += isCWL
        ? `Attack → ${notUsed ? "Not used" : `vs ${atk.oppName} → ${atk.destructionPercentage}% (${atk.stars}⭐)`}\n`
        : `Attack ${i + 1} → ${notUsed ? "Not used" : `vs ${atk.oppName} → ${atk.destructionPercentage}% (${atk.stars}⭐)`}\n`;
    });
