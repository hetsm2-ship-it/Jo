...
// --- War history: FORCE CWL DETECTION from db.json ---
const playerLogsRaw = DB.playerWarLogs?.[player.tag] || [];
const warLogs = playerLogsRaw.slice(-50).reverse(); // get last 50 logs

if (warLogs.length) {
  msg += "📜 *War History:*\n";

  // Group by warId or fallback key
  const grouped = {};
  for (const log of warLogs) {
  ...
  for (const group of Object.values(grouped)) {
    // ✅ EXTREME FORCE: get first log that has cwlRound (accept number OR string)
    const cwlLog = group.find(l => l.cwlRound !== undefined && l.cwlRound !== null && l.cwlRound !== "" && l.cwlRound !== "null" && l.cwlRound !== "undefined");
    const isCWL = Boolean(cwlLog);
    const roundNumber = isCWL ? Number(cwlLog.cwlRound) || cwlLog.cwlRound : null;
    const sample = cwlLog || group[0];

    // HEADER
    msg += `No.${idx++} ${isCWL ? "(CWL) " : ""}🏰 ${sample.myClan} (${sample.clanTag}) vs ${sample.oppClan} (${sample.oppClanTag})`;
    if (isCWL && roundNumber) msg += ` (Round ${roundNumber})`;
    msg += "\n";

    // Sort attacks by order, then print
    group.sort((a, b) => (a.order || 0) - (b.order || 0)).forEach(atk => {
      ...
      if (isCWL) {
        ...
        // Normal war: keep numbering
        const attackIndex = group.indexOf(atk) + 1;
        msg += `Attack ${attackIndex} → ${notUsed ? "Not used" : `vs ${atk.oppName || "Unknown"} → ${atk.destructionPercentage}% (${atk.stars}⭐)`}\n`;
      }
    });
