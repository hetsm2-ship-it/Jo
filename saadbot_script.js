...
let DB = {
  ...
  lastWarlogPlayers: {},
  playerWarLogs: {},   // 🔥 always define here
  attendanceLogs: {}   // ✅ add attendance logs
};
...
const loadDB = () => {
  ...
  // ✅ always ensure these exist after loading
  if (!DB.playerWarLogs) DB.playerWarLogs = {};
...
async function handleWarNotifications(sock) {
  try {
    const seen = new Set(
      Object.values(DB.userClans || {}).map(d =>
        typeof d === "string" ? d : d?.clanTag
      ).filter(Boolean) // null/undefined skip
    );

    for (const clanTag of seen) {
      try {
        const war = await cocFetch(
          `/clans/${encodeURIComponent(clanTag)}/currentwar`
        );
        if (war.error || !war.state) continue;

        // 🎯 Safe filter (skip null/undefined entries)
        const targets = Object.entries(DB.userClans)
          .filter(([, data]) => {
            if (!data) return false; // removeclan case
        ...
        if (war.state === "inWar") {
          ...
          if (!endTime) {
            console.error("❌ Invalid war.endTime:", war.endTime);
            continue;
          }
          ...
          const minuteMilestones = [{ m: 15, key: "inwar15m" }];

          const timeLeft = formatTimeLeft(diffMs + 10000);

          // ✅ Hour milestones
          for (const ms of milestones) {
            const targetMs = ms.h * 60 * 60 * 1000 + 16000; // +16s buffer
            const notifyKey = `${baseKey}:${ms.key}`;

            if (!DB.lastWarNotificationSent[notifyKey] && diffMs <= targetMs) {
              console.log(`🚀 Sending notification: ${ms.key}`);
              await sendWarNotification(sock, war, userTargets, groupTargets, formatTimeLeft(diffMs + 16000));
          ...
          for (const ms of minuteMilestones) {
            const targetMs = ms.m * 60 * 1000 + 16000; // +16s buffer
            const notifyKey = `${baseKey}:${ms.key}`;
    ...
    console.error("handleWarNotifications failed:", e.message);
  }
}

// ✅ Helper to send war update
...
async function setClan(jid, clanTag) {
  ...
  if (oldClan) {
    ...
    if (!DB.removedClanLogs[oldTag]) {
      const backup = {};
      for (const [playerTag, logs] of Object.entries(DB.playerWarLogs)) {
        const related = logs.filter(l => l.clanTag === oldTag);
        if (related.length) backup[playerTag] = related;
      }
  ...
  if (DB.removedClanLogs[clanTag]) {
    for (const [playerTag, logs] of Object.entries(DB.removedClanLogs[clanTag])) {
      if (!DB.playerWarLogs[playerTag]) DB.playerWarLogs[playerTag] = [];
    ...
    console.log(`♻️ Restored old logs for clan ${clanTag}.`);
    delete DB.removedClanLogs[clanTag]; // cleanup after restoring
  }

  if (typeof saveDB === "function") saveDB();
  return `✅ Clan ${clanTag} set successfully.`;
}
